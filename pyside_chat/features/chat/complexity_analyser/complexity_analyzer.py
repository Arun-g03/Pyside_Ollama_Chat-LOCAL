...
class RequestComplexityAnalyzer:
    ...
    def analyze_complexity(self, request: str, conversation_history: List[Dict] = None) -> ComplexityMetrics:
        ...
        context_dependency = self._analyze_context_dependency(
        ...
        factors = {
            ...
            # Normalize to 0-1
            'constraint_count': min(constraint_count / 10, 1.0),
    ...
    def _analyze_reasoning_depth(self, text: str) -> float:
        ...
        step_indicators = len(re.findall(
            r'\b(?:first|second|third|then|next|finally|step|phase)\b', text_lower))
        score += step_indicators * 0.1

        # Check for conditional logic
        conditional_indicators = len(re.findall(
            r'\b(?:if|else|when|unless|provided|assuming)\b', text_lower))
    ...
    def _analyze_ambiguity(self, text: str) -> float:
        ...
        ambiguous_words = len(re.findall(
            r'\b(?:maybe|perhaps|possibly|might|could|would|should|depends|unclear|vague)\b', text_lower))
        ...
        subjective_words = len(re.findall(
            r'\b(?:best|better|good|bad|better|worse|prefer|like|dislike)\b', text_lower))
    ...
    def _count_constraints(self, text: str) -> int:
        ...
        constraint_count += len(re.findall(
            r'\b(?:only|just|exactly|precisely)\b', text_lower))
        constraint_count += len(re.findall(
            r'\b(?:must|should|need to|have to|required)\b', text_lower))
        constraint_count += len(re.findall(
            r'\b(?:format|structure|template)\b', text_lower))
    ...
    def _analyze_context_dependency(self, text: str, conversation_history: List[Dict] = None) -> float:
        ...
        context_indicators = len(re.findall(
            r'\b(?:previous|earlier|before|last time|continue|follow up|extend)\b', text_lower))
        score += context_indicators * 0.2

        # Check for pronouns that need context
        pronoun_count = len(re.findall(
            r'\b(?:it|this|that|these|those|they|them)\b', text_lower))
        score += pronoun_count * 0.05

        # Check conversation history length
        if conversation_history:
            history_length = len(conversation_history)
            # Cap at 0.3 for very long histories
            score += min(history_length / 20.0, 0.3)
    ...
    def _analyze_output_complexity(self, text: str) -> float:
        ...
        format_indicators = len(re.findall(
            r'\b(?:json|xml|yaml|csv|html|markdown|code|function|class)\b', text_lower))
    ...
    def _generate_recommendations(self, level: ComplexityLevel, factors: Dict[str, float]) -> List[str]:
        ...
        if factors['constraint_count'] > 5:
            recommendations.append(
                "Consider relaxing some constraints for better results")
