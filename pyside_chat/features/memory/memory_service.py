...
class MemoryClassifier:
    ...
    def classify_message(message: str, role: str = "user") -> Dict:
        ...
        try:
            ...
            relationship_matches = words.intersection(
            ...
            LoggingHelpers.log_exception_with_context(
                "classify_message", e, {"message": message, "role": role})
...
class PronounNormalizer:
        ...
        try:
            ...
            for contraction, replacement in PronounNormalizer.FIRST_PERSON_CONTRACTIONS.items():
                normalized_text = re.sub(
                    r'\b' + re.escape(contraction) + r'\b', replacement, normalized_text, flags=re.IGNORECASE)

            # Handle individual pronouns
            for pronoun, replacement in PronounNormalizer.FIRST_PERSON_PRONOUNS.items():
                normalized_text = re.sub(
                    r'\b' + re.escape(pronoun) + r'\b', replacement, normalized_text, flags=re.IGNORECASE)

            return normalized_text
        except Exception as e:
            LoggingHelpers.log_exception_with_context(
                "normalize_pronouns", e, {"text": text, "user_name": user_name})
            return text
    ...
    def should_normalize(text: str) -> bool:
        ...
        try:
            ...
            first_person_words = {'i', 'me', 'my', 'myself',
                                  'mine', 'we', 'us', 'our', 'ours', 'ourselves'}
...
class MemoryRetriever:
    ...
    def calculate_relevance(query: str, memory_entry: LongTermMemoryEntry) -> float:
        ...
        try:
            ...
            if memory_entry.key:
                entry_keywords.update(re.findall(
                    r'\b\w+\b', memory_entry.key.lower()))
            if memory_entry.value:
                entry_keywords.update(re.findall(
                    r'\b\w+\b', memory_entry.value.lower()))
            if memory_entry.summary:
                entry_keywords.update(re.findall(
                    r'\b\w+\b', memory_entry.summary.lower()))
            ...
            LoggingHelpers.log_exception_with_context(
                "calculate_relevance", e, {"query": query, "memory_entry": str(memory_entry)})
            return 0.0
    ...
    def get_relevant_memories(query: str, memories: List[LongTermMemoryEntry],
        ...
        try:
            ...
            LoggingHelpers.log_exception_with_context(
                "get_relevant_memories", e, {"query": query, "memories_count": len(memories)})
            return []
        ...
        try:
            if os.path.exists(self.stm_file):
                ...
                LoggingHelpers.log_file_operation(
                    "load", self.stm_file, False, FileNotFoundError("File not found"))
        except Exception as e:
...
class LongTermMemoryService:
        ...
        try:
            if os.path.exists(self.ltm_file):
                with open(self.ltm_file, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                    self.entries = [LongTermMemoryEntry(
                        **entry) for entry in data]
                LoggingHelpers.log_file_operation("load", self.ltm_file, True)
                LoggingHelpers.log_info_with_context(
                    "Loaded LTM entries", {"count": len(self.entries)})
            else:
                LoggingHelpers.log_file_operation(
                    "load", self.ltm_file, False, FileNotFoundError("File not found"))
        except Exception as e:
    ...
    def get_entries(self, type_filter: Optional[str] = None) -> List[LongTermMemoryEntry]:
        ...
        try:
            ...
            LoggingHelpers.log_exception_with_context(
                "get_entries", e, {"type_filter": type_filter})
            return []
    ...
    def _save(self):
        ...
        try:
            ...
            with open(self.ltm_file, 'w', encoding='utf-8') as f:
                json.dump([asdict(entry) for entry in self.entries],
    ...
    def update_access_stats(self, entry: LongTermMemoryEntry):
        ...
        try:
            ...
            LoggingHelpers.log_exception_with_context(
                "update_access_stats", e, {"entry_type": entry.type})
...
class MemoryService(QObject):
            ...
            try:
                if SENTENCE_TRANSFORMERS_AVAILABLE:
                    ...
                    LoggingHelpers.log_service_initialization(
                        "SemanticSearchService (with embeddings)", True)
                else:
                    self.semantic_search = SemanticSearchService()  # This is the fallback service
                    LoggingHelpers.log_service_initialization(
                        "SemanticSearchService (fallback mode)", True)
            except Exception as e:
                LoggingHelpers.log_service_initialization(
                    "SemanticSearchService", False, e)
    ...
    def add_memory(self, content: str, conversation_id: str, importance: float = 0.5,
        ...
        try:
            memory_id = hashlib.md5(
                f"{content}{conversation_id}{datetime.now().isoformat()}".encode()).hexdigest()
            ...
            if self.semantic_search:
                try:
                    ...
                    logger.warning(
                        f"Failed to add memory to semantic search: {e}")

            LoggingHelpers.log_memory_operation(
                "add_memory", memory_type, True)
            return memory_id
        except Exception as e:
            LoggingHelpers.log_memory_operation(
                "add_memory", memory_type, False, e)
            return ""
    ...
    def get_relevant_memories(self, query: str, limit: int = 10, use_semantic: bool = True) -> List[MemoryEntry]:
        ...
        try:
            ...
            relevant_ltm = MemoryRetriever.get_relevant_memories(
                query, ltm_entries, limit)

            for entry, relevance in relevant_ltm:
                memory_entry = MemoryEntry(
                    id=hashlib.md5(
                        f"{entry.type}{entry.key}{entry.value}".encode()).hexdigest(),
                    content=entry.summary or f"{entry.key}: {entry.value}" if entry.key and entry.value else str(
                        entry),
                    conversation_id="ltm",
                    timestamp=entry.timestamp,
            ...
            if use_semantic and self.semantic_search:
                try:
                    semantic_results = self.semantic_search.search_semantic(
                        query, max_results=limit//2)
                    ...
                    LoggingHelpers.log_exception_with_context(
                        "semantic_search", e, {"query": query})
                    logger.warning(
                        f"Semantic search failed, using fallback: {e}")

            # Sort by relevance and limit
            relevant_memories.sort(key=lambda x: x.metadata.get(
                "relevance", 0.0), reverse=True)
            return relevant_memories[:limit]
        except Exception as e:
            LoggingHelpers.log_exception_with_context(
                "get_relevant_memories", e, {"query": query, "limit": limit})
            return []
    ...
    def intelligent_add_message(self, message: Dict) -> Dict:
        ...
        try:
            ...
            if role == "user" and PronounNormalizer.should_normalize(content):
                normalized_content = PronounNormalizer.normalize_pronouns(
                    content)
            ...
            if self.semantic_search:
                try:
                    memory_id = hashlib.md5(
                        f"{content}{datetime.now().isoformat()}".encode()).hexdigest()
                    self.semantic_search.add_memory(
                        memory_id=memory_id,
                        content=content,
                    ...
                    logger.warning(
                        f"Failed to add message to semantic search: {e}")
            ...
            LoggingHelpers.log_exception_with_context(
                "intelligent_add_message", e, {"message": message})
            return {
                "classified": False,
                "memory_type": "conversation",
                "importance": 0.3,
    ...
    def extract_facts_from_message(self, message: str) -> Dict[str, str]:
        ...
        try:
            ...
            LoggingHelpers.log_exception_with_context(
                "extract_facts_from_message", e, {"message": message})
            return {}
    ...
    def get_context_messages(self, current_query: str = "") -> List[Dict]:
        ...
        try:
            ...
            LoggingHelpers.log_exception_with_context(
                "get_context_messages", e, {"current_query": current_query})
            return []
    ...
    def summarize_conversation(self, conversation_messages: List[Dict], conversation_id: str) -> str:
        ...
        try:
            ...
            user_messages = [msg["content"]
                             for msg in conversation_messages if msg.get("role") == "user"]
            assistant_messages = [
                msg["content"] for msg in conversation_messages if msg.get("role") == "assistant"]

            summary = f"Conversation {conversation_id}: {len(user_messages)} user messages, {len(assistant_messages)} assistant responses"

            # Add to long-term memory
            self.add_summary(summary, importance=0.6, tags=[
                             "conversation", "summary"])

            return summary
        except Exception as e:
            LoggingHelpers.log_exception_with_context("summarize_conversation", e, {
                                                      "conversation_id": conversation_id})
            return ""
        ...
        try:
            if memory_type is None:
                ...
                if memory_type == "short_term":
                    ...
                    self.ltm_service.entries = [
                        entry for entry in self.ltm_service.entries if entry.type != memory_type]
                    self.ltm_service._save()

                LoggingHelpers.log_memory_operation(
                    "clear_memory", memory_type, True)
        except Exception as e:
            LoggingHelpers.log_memory_operation(
                "clear_memory", memory_type or "all", False, e)
        ...
        try:
            ...
            if os.path.exists(self.stm_service.stm_file):
                with open(self.stm_service.stm_file, 'r', encoding='utf-8') as f:
                    ...
                    if stm_data:
                        LoggingHelpers.log_warning_with_context("STM file not empty after clear", {
                                                                "file": self.stm_service.stm_file, "data_length": len(stm_data)})
            ...
            if os.path.exists(self.ltm_service.ltm_file):
                with open(self.ltm_service.ltm_file, 'r', encoding='utf-8') as f:
                    ...
                    if ltm_data:
                        LoggingHelpers.log_warning_with_context("LTM file not empty after clear", {
                                                                "file": self.ltm_service.ltm_file, "data_length": len(ltm_data)})
        except Exception as e:
            LoggingHelpers.log_exception_with_context(
                "_verify_memory_files_cleared", e, {})
    ...
    def delete_memory(self, memory_id: str):
        ...
        try:
            ...
            self.ltm_service._save()
            LoggingHelpers.log_memory_operation(
                "delete_memory", "specific", True)
        except Exception as e:
            LoggingHelpers.log_memory_operation(
                "delete_memory", "specific", False, e)
    ...
    def get_memory_stats(self) -> Dict:
        ...
        try:
            ...
            average_importance = total_importance / \
                importance_count if importance_count > 0 else 0.0
    ...
    def set_max_context_messages(self, max_messages: int):
        ...
        try:
            ...
            LoggingHelpers.log_configuration_change(
                "max_context_messages", "previous", max_messages)
        except Exception as e:
            LoggingHelpers.log_exception_with_context("set_max_context_messages", e, {
    ...
    def search_memories(self, query: str, memory_type: str = None) -> List[MemoryEntry]:
        ...
        try:
            ...
            relevant_entries = MemoryRetriever.get_relevant_memories(
                query, ltm_entries)
            ...
            LoggingHelpers.log_exception_with_context(
                "search_memories", e, {"query": query, "memory_type": memory_type})
            return []
    ...
    def cleanup_memory_entries(self):
        ...
        try:
            ...
            if removed_count > 0:
                self.ltm_service._save()
                LoggingHelpers.log_info_with_context("Cleaned up memory entries", {
                                                     "removed_count": removed_count})
        except Exception as e:
            LoggingHelpers.log_exception_with_context(
                "cleanup_memory_entries", e, {})
