...
DEFAULT_PERSISTENT_THREAD_CONFIG = {
    'chat_streaming': {
        ...
        'max_wait_time': 30.0,  # Max time to wait for available thread (seconds)
        'idle_timeout': 300.0,  # Time before idle thread is cleaned up (seconds)
        ...
        for thread_type, thread_config in config.items():
            ...
            for key in required_keys:
                if key not in thread_config:
                    print(f"Missing required key '{key}' in {thread_type} configuration")
                    return False
...
def get_config_summary(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    try:
        total_threads = sum(thread_config['size'] for thread_config in config.values())
        total_max_wait = sum(thread_config['max_wait_time'] for thread_config in config.values())
    ...
    for config_type in configs:
        ...
        if validate_persistent_thread_config(config):
            ...
            for thread_type, details in summary['thread_details'].items():
                print(f"  {thread_type}: {details['size']} threads - {details['description']}")
        else:
