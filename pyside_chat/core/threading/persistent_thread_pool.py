...
class PersistentThreadPool(QObject):
    ...
    def initialize_pool(self, thread_type: str, worker_class: type, **kwargs):
        ...
        try:
            ...
            logger.debug(
                f"[ID:PTP003] Initializing {thread_type} pool with {pool_size} threads")
            ...
            logger.debug(
                f"[ID:PTP004] {thread_type} pool initialized with {len(self.idle_threads[thread_type])} idle threads")
            return True

        except Exception as e:
            logger.error(
                f"[ID:PTP005] Error initializing {thread_type} pool: {e}")
    ...
    def get_thread(self, thread_type: str, timeout: Optional[float] = None) -> Optional[QThread]:
        ...
        try:
            ...
            with lock:
                ...
                if self.idle_threads[thread_type]:
                    ...
                    if hasattr(thread, 'worker') and thread.worker:
                        if hasattr(thread.worker, '_configuration'):
                            ...
                            streaming_id = thread.worker._configuration.get(
                                'streaming_message_id', 'None')
                            logger.debug(
                                f"[ID:PTP007A] Retrieved {thread_type} thread {thread.objectName()} - config keys: {config_keys}, streaming_id: {streaming_id}")
                        else:
                            logger.debug(
                                f"[ID:PTP007A] Retrieved {thread_type} thread {thread.objectName()} - no configuration")
                    ...
                    logger.debug(
                        f"[ID:PTP007] Retrieved {thread_type} thread: {thread.objectName()}")
                ...
                current_active = len(
                    [t for t in self.active_threads.values() if t['type'] == thread_type])

                if current_active < max_size:
                    worker_class = self.pool_config[thread_type]['worker_class']
                    if worker_class:
                        thread = self._create_active_thread(
                            thread_type, worker_class)
                        if thread:
                            logger.debug(
                                f"[ID:PTP008] Created new {thread_type} thread: {thread.objectName()}")
                            self.thread_busy.emit(thread_type)
                            return thread

                # Wait for a thread to become available
                logger.debug(
                    f"[ID:PTP009] Waiting for {thread_type} thread (timeout: {max_wait_time}s)")
                start_time = time.time()
                ...
                while time.time() - start_time < max_wait_time:
                    ...
                    if self.idle_threads[thread_type]:
                        ...
                        if hasattr(thread, 'worker') and thread.worker:
                            if hasattr(thread.worker, '_configuration'):
                                ...
                                streaming_id = thread.worker._configuration.get(
                                    'streaming_message_id', 'None')
                                logger.debug(
                                    f"[ID:PTP010A] Retrieved {thread_type} thread {thread.objectName()} after wait - config keys: {config_keys}, streaming_id: {streaming_id}")
                            else:
                                logger.debug(
                                    f"[ID:PTP010A] Retrieved {thread_type} thread {thread.objectName()} after wait - no configuration")
                        ...
                        logger.debug(
                            f"[ID:PTP010] Retrieved {thread_type} thread after wait: {thread.objectName()}")
                        self.thread_busy.emit(thread_type)
                        return thread

                logger.warning(
                    f"[ID:PTP011] Timeout waiting for {thread_type} thread")
                return None

        except Exception as e:
            logger.error(
                f"[ID:PTP012] Error getting {thread_type} thread: {e}")
            logger.error(traceback.format_exc())
            return None
    ...
    def return_thread(self, thread: QThread):
        ...
        try:
            ...
            if thread_name not in self.active_threads:
                logger.warning(
                    f"[ID:PTP013] Thread {thread_name} not found in active threads")
                return
            ...
            if hasattr(thread, 'worker') and thread.worker:
                if hasattr(thread.worker, '_configuration'):
                    ...
                    streaming_id = thread.worker._configuration.get(
                        'streaming_message_id', 'None')
                    logger.debug(
                        f"[ID:PTP013A] Resetting {thread_type} thread {thread_name} - config keys: {config_keys}, streaming_id: {streaming_id}")
                else:
                    logger.debug(
                        f"[ID:PTP013A] Resetting {thread_type} thread {thread_name} - no configuration")
            ...
            logger.debug(
                f"[ID:PTP014] Returned {thread_type} thread to pool: {thread_name}")
            self.thread_available.emit(thread_type)

        except Exception as e:
            logger.error(
                f"[ID:PTP015] Error returning thread {thread.objectName()}: {e}")
            logger.error(traceback.format_exc())
    ...
    def _create_idle_thread(self, thread_type: str, worker_class: type) -> Optional[QThread]:
        ...
        try:
            ...
            thread.setObjectName(
                f"{thread_type.capitalize()}Thread_{id(thread)}")
            ...
            thread_monitor.register_thread(
                thread, thread_type, {'status': 'idle'})

            logger.debug(
                f"[ID:PTP016] Created idle {thread_type} thread: {thread.objectName()}")
            return thread

        except Exception as e:
            logger.error(
                f"[ID:PTP017] Error creating idle {thread_type} thread: {e}")
            return None
    ...
    def _create_active_thread(self, thread_type: str, worker_class: type) -> Optional[QThread]:
        ...
        try:
            ...
            thread_monitor = get_global_thread_monitor()
            thread_monitor.register_thread(
                thread, thread_type, {'status': 'active'})

            logger.debug(
                f"[ID:PTP018] Created active {thread_type} thread: {thread.objectName()}")
            return thread

        except Exception as e:
            logger.error(
                f"[ID:PTP019] Error creating active {thread_type} thread: {e}")
            return None
                ...
                if len(idle_threads) > min_size:
                    ...
                    for _ in range(threads_to_remove):
                        if idle_threads:
                            ...
                            logger.debug(
                                f"[ID:PTP020] Cleaning up idle {thread_type} thread: {thread.objectName()}")
    ...
    def shutdown(self):
        ...
        try:
            ...
            logger.error(
                f"[ID:PTP025] Error during PersistentThreadPool shutdown: {e}")
