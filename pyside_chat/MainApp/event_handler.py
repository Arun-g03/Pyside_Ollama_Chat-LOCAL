...
class EventHandler:
    ...
    def setup_connections(self):
        ...
        try:
            ...
            logger.info("All signal connections established successfully")
    ...
    def _on_worker_finished(self):
        ...
        if self._tts_finished:
            logger.debug("Both worker and TTS finished, cleaning up")
            self._cleanup_worker_thread()
        else:
            logger.debug("Worker finished but TTS still running, waiting for TTS")
            # TTS will call cleanup when it finishes
    
    def _on_tts_finished(self):
        """Handle TTS completion"""
        logger.debug("TTS finished signal received")
    ...
    def _handle_tts_finished_delayed(self):
        ...
        try:
            ...
            if hasattr(self, 'worker_thread') and self.worker_thread and self.worker_thread.isRunning():
                logger.debug("Worker thread still running, waiting for it to finish")
                # Don't clean up yet - let the worker finish naturally
                # The worker will call cleanup when it finishes
                return
            else:
                logger.debug("Worker thread not running, cleaning up immediately")
                self._cleanup_worker_thread()
        except Exception as e:
            logger.error(f"Error in delayed TTS finished handling: {e}")
            ...
            try:
                ...
                if self.worker_thread and self.worker_thread.isRunning():
                    ...
                    if not self.worker_thread.wait(2000):  # 2 second timeout
                        logger.warning("Worker thread did not quit within timeout, marking for deletion")
                        # Don't use terminate() as it can cause crashes
                        # Instead, just mark it for deletion and let it finish naturally
                        self.worker_thread.deleteLater()
                    else:
                        logger.debug("Worker thread quit successfully")
                        self.worker_thread.deleteLater()
                else:
                    logger.debug("Worker thread not running, cleaning up")
