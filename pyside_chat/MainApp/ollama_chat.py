...
from pyside_chat.MainApp.service_manager import ServiceManager
from pyside_chat.MainApp.ui_manager import UIManager
from pyside_chat.MainApp.event_handler import EventHandler
from pyside_chat.MainApp.app_lifecycle import AppLifecycleManager
...
class OllamaChat(QMainWindow):
    ...
    def __init__(self):
        try:
            ...
            logger.info("[ID:0285] OllamaChat initialization completed successfully")
            
        except Exception as e:
            logger.error(f'[ID:0284] ❌ Exception in OllamaChat.__init__: {e}')
            traceback.print_exc()
            raise
    ...
    def _setup_ui(self):
        ...
        try:
            ...
            logger.info("[ID:0283] UI setup completed successfully")
            
        except Exception as e:
            logger.error(f"[ID:0282] Error setting up UI: {e}")
            raise
