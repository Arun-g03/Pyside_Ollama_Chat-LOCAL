    ...
    def run(self):
        try:
            ...
            self.download_progress.emit(
                f"Starting download of {self.model_name}...")
            ...
            if success:
                self.download_completed.emit(
                    True, f"Successfully downloaded {self.model_name}")
            else:
                self.download_completed.emit(
                    False, f"Failed to download {self.model_name}")
...
class CoquiModelDialog(QDialog):
    ...
    def create_speaker_panel(self):
        ...
        self.speaker_info_label = QLabel(
            "Select a model first to see available speakers")
            ...
            if self.coqui_service.load_model(model_name):
                ...
                if speakers:
                    ...
                    self.speaker_info_label.setText(
                        f"Found {len(speakers)} speakers for model '{model_name}'")
                else:
                    self.speaker_info_label.setText(
                        f"Model '{model_name}' has no multiple speakers (single voice)")
                    # Add a default speaker item
                    item = QListWidgetItem("default")
                    self.speaker_list.addItem(item)

                self.log_status(
                    f"Loaded {len(speakers) if speakers else 1} speakers for model '{model_name}'")
                ...
                self.log_status(
                    f"Failed to load speakers for model '{model_name}'")
    ...
    def on_speaker_selected(self, item):
        ...
        self.speaker_info_label.setText(
            f"Selected speaker: {self.selected_speaker}")
        self.update_selection_button()
    ...
    def accept_selection(self):
        ...
        if self.selected_model and self.selected_speaker:
            self.model_selected.emit(
                self.selected_model, self.selected_speaker)
            self.accept()
        else:
            QMessageBox.warning(self, "Selection Required",
                                "Please select both a model and a speaker.")
