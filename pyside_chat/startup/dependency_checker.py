    ...
    def run_install_dependencies(self) -> bool:
        ...
        try:
            ...
            install_script = os.path.join(
                current_dir, "install_dependencies.py")

            if not os.path.exists(install_script):
                print(
                    f"[ID:0264] Error: install_dependencies.py not found at {install_script}")
                return False

            print("[ID:0263] Installing dependencies...")
            result = subprocess.run([
            ...
            print(f"[ID:0262] Failed to install dependencies: {e}")
            return False
        except Exception as e:
            print(f"[ID:0261] Error installing dependencies: {e}")
            return False
...
def check_and_install_dependencies(auto_install: bool = True, verbose: bool = False) -> bool:
    ...
    """
    if verbose:
        print("[ID:0260] Checking dependencies...")
    ...
    if not needs_fixing:
        if verbose:
            print("[ID:0259] All dependencies are working correctly!")
        return True

    if verbose:
        print(
            f"[ID:0258] Dependency issues found: {checker.get_dependency_summary()}")
        if checker.broken_imports:
            print(f"[ID:0257] Missing: {', '.join(checker.broken_imports)}")
        if checker.version_conflicts:
            print(
                f"[ID:0256] Version conflicts: {len(checker.version_conflicts)}")

    if auto_install:
        if verbose:
            print("[ID:0255] Installing missing dependencies...")
        success = checker.run_install_dependencies()

        if success:
            if verbose:
                print("[ID:0254] Re-checking dependencies...")
            ...
            if not final_check:
                if verbose:
                    print("[ID:0253] All dependencies are now working!")
                return True
            else:
                if verbose:
                    print("[ID:0252] Some dependencies may still have issues")
                return False
        else:
            if verbose:
                print("[ID:0251] Failed to install dependencies automatically")
            return False
    else:
        if verbose:
            print("[ID:0250] Run 'python pyside_chat/services/start_up/install_dependencies.py' to install missing dependencies")
        return False


if __name__ == "__main__":
    # Test the dependency checker
    print("[ID:0249] 🔍 Testing Dependency Checker...")
    checker = DependencyChecker()
    needs_fixing = checker.run_comprehensive_check()
    ...
    for imp in checker.optional_broken:
        print(f"[ID:0243] ⚠️  {imp}")

    print(f"[ID:0242] \nVersion conflicts ({len(checker.version_conflicts)}):")
    for conflict in checker.version_conflicts:
        print(f"[ID:0241] ⚠️  {conflict}")

    print(
        f"[ID:0240] \nOverall status: {'✅ All good' if not needs_fixing else '❌ Needs fixing'}")
    print(
        f"[ID:0239] Can start application: {'✅ Yes' if not needs_fixing else '❌ No'}")
    if checker.optional_broken:
        print(
            f"[ID:0238] Note: {len(checker.optional_broken)} optional features unavailable")
