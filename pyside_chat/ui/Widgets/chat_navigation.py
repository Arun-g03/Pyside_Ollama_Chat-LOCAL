...
class ChatNavigationWidget(QWidget):
    ...
    # Signals
    # Emitted when a conversation is selected (filepath)
    ...
    def setup_connections(self):
        ...
        if self.summarization_service:
            self.summarization_service.summarization_completed.connect(
                self.on_summarization_completed)
            self.summarization_service.summarization_failed.connect(
                self.on_summarization_failed)
    ...
    def show_context_menu(self, position):
        ...
        open_action.triggered.connect(
            lambda: self.conversation_selected.emit(filepath))

        # Rename action
        rename_action = menu.addAction("Rename")
        rename_action.triggered.connect(
            lambda: self.rename_conversation(filepath))
        ...
        try:
            ...
            if not metadata.ai_generated_name:
                ...
                generate_name_action.triggered.connect(
                    lambda: self.trigger_name_generation(filepath))
        except Exception as e:
        ...
        delete_action.triggered.connect(
            lambda: self.delete_conversation(filepath))
        ...
        try:
            ...
            if ok and new_filename and new_filename != old_display_name:
                ...
                new_filepath = os.path.join(
                    os.path.dirname(filepath), new_filename)
                ...
                if self.conversation_manager.rename_conversation(filepath, new_filepath):
                    ...
                    QMessageBox.warning(
                        self, "Error", "Failed to rename conversation.")

        except Exception as e:
            QMessageBox.warning(
                self, "Error", f"Failed to rename conversation: {str(e)}")
    ...
    def delete_conversation(self, filepath: str):
        ...
        try:
            ...
            try:
                _, metadata = self.conversation_manager.load_conversation(
                    filepath)
                display_name = metadata.get_display_name()
            except:
            ...
            QMessageBox.warning(
                self, "Error", f"Failed to delete conversation: {str(e)}")
    ...
    def clear_all_conversations(self):
        ...
        if reply == QMessageBox.Yes:
            try:
                ...
                QMessageBox.warning(
                    self, "Error", f"Failed to clear all conversations: {str(e)}")
        ...
        try:
            ...
            if filepath in self.name_generation_cooldown:
                time_since_last = current_time - \
                    self.name_generation_cooldown[filepath]
                if time_since_last < self.cooldown_duration:
                    print(
                        f"Name generation on cooldown for {filepath} ({self.cooldown_duration - time_since_last:.1f}s remaining)")
                    return
    ...
    def on_summarization_completed(self, filepath: str, generated_name: str) -> None:
        ...
        try:
            ...
            new_filepath = self.conversation_manager.update_conversation_name(
