...
class LoggingHelpers:
    ...
    def log_exception_with_context(operation: str, exception: Exception, context: Dict = None):
        ...
        logger.error(
            f"[ID:0900] Exception in {operation}: {str(exception)}{context_str}")
        logger.error(
            f"[ID:0901] Exception traceback: {traceback.format_exc()}")
    ...
    @staticmethod
    def log_error(message: str, print_to_terminal: bool = False):
    ...
    def log_network_request(url: str, method: str, status_code: int = None, error: Exception = None):
        ...
        if error:
            logger.error(
                f"[ID:0906] Network request failed - {method} {url}: {str(error)}")
        elif status_code:
            logger.info(
                f"[ID:0907] Network request - {method} {url}: {status_code}")
        else:
    ...
    def log_file_operation(operation: str, filepath: str, success: bool, error: Exception = None):
        ...
        if success:
            logger.debug(
                f"[ID:0909] File operation successful - {operation}: {filepath}")
        else:
            logger.error(
                f"[ID:0910] File operation failed - {operation}: {filepath} - {str(error)}")
    ...
    def log_audio_operation(operation: str, success: bool, error: Exception = None, details: str = ""):
        ...
        if success:
            logger.debug(
                f"[ID:0911] Audio operation successful - {operation}{' - ' + details if details else ''}")
        else:
            logger.error(
                f"[ID:0912] Audio operation failed - {operation} - {str(error)}")
    ...
    def log_memory_operation(operation: str, memory_type: str, success: bool, error: Exception = None):
        ...
        if success:
            logger.debug(
                f"[ID:0913] Memory operation successful - {operation} ({memory_type})")
        else:
            logger.error(
                f"[ID:0914] Memory operation failed - {operation} ({memory_type}) - {str(error)}")
    ...
    def log_ui_operation(component: str, operation: str, success: bool, error: Exception = None):
        ...
        if success:
            logger.debug(
                f"[ID:0915] UI operation successful - {component}.{operation}")
        else:
            logger.error(
                f"[ID:0916] UI operation failed - {component}.{operation} - {str(error)}")
    ...
    def log_performance_metric(operation: str, duration: float, context: str = ""):
        ...
        context_str = f" - Context: {context}" if context else ""
        logger.debug(
            f"[ID:0917] Performance - {operation}: {duration:.3f}s{context_str}")
    ...
    def log_critical_error(component: str, error: Exception, recovery_action: str):
        ...
        logger.critical(
            f"[ID:0921] Critical error in {component}: {str(error)}")
    ...
    def log_fact_extraction_start(query: str):
        ...
        logger.debug(
            f"[ID:0923] Fact extraction started for query: {query[:100]}...")

    @staticmethod
    def log_fact_extraction_end(facts_count: int):
        """Log fact extraction end"""
        logger.debug(
            f"[ID:0924] Fact extraction completed - {facts_count} facts extracted")
    ...
    def log_fact_storage_summary(total_facts: int, stored_facts: int):
        ...
        logger.info(
            f"[ID:0929] Fact storage summary: {stored_facts}/{total_facts} facts stored")
    ...
    def log_memory_result(query: str, memory_count: int):
        ...
        logger.debug(
            f"[ID:0931] Memory search for '{query[:50]}...' returned {memory_count} results")

    @staticmethod
    def log_memory_ltm_status(ltm_count: int, stm_count: int):
        """Log memory status"""
        logger.debug(
            f"[ID:0932] Memory status - LTM: {ltm_count}, STM: {stm_count}")

    @staticmethod
    def log_llm_call(model: str, prompt_length: int):
        """Log LLM call"""
        logger.debug(
            f"[ID:0933] LLM call - Model: {model}, Prompt length: {prompt_length}")

    @staticmethod
    def log_llm_response(model: str, response_length: int):
        """Log LLM response"""
        logger.debug(
            f"[ID:0934] LLM response - Model: {model}, Response length: {response_length}")

    @staticmethod
    def log_json_extraction(json_data: Dict):
        """Log JSON extraction"""
        logger.debug(
            f"[ID:0935] JSON extraction successful: {len(json_data)} fields")
    ...
    def log_message_sent_end(message_length: int, response_length: int):
        ...
        logger.debug(
            f"[ID:0937] Message exchange completed - Sent: {message_length}, Received: {response_length}")
    ...
    def log_service_initialization(service_name: str, success: bool, error: Exception = None):
        ...
        if success:
            logger.info(
                f"[ID:0939] Service initialized successfully: {service_name}")
        else:
            error_msg = f" - Error: {str(error)}" if error else ""
            logger.error(
                f"[ID:0940] Service initialization failed: {service_name}{error_msg}")
...
class ThreadMonitor(QObject):
    ...
    def __init__(self):
        ...
        logger.debug(
            f"[ID:0800] ThreadMonitor created - ID: {self._monitor_id}")
    ...
    def register_thread(self, thread: QThread, thread_type: str = "unknown"):
        ...
        try:
            ...
            logger.debug(
                f"[ID:0801] Registered thread: {thread_name} (ID: {thread_id})")
            self.thread_started.emit(thread_name)

        except Exception as e:
            logger.error(f"[ID:0802] Error registering thread: {e}")
            logger.error(
                f"[ID:0803] Thread registration error traceback: {traceback.format_exc()}")
    ...
    def unregister_thread(self, thread_name: str):
        ...
        try:
            if thread_name in self.active_threads:
                ...
                logger.debug(
                    f"[ID:0804] Unregistered thread: {thread_name} (Duration: {thread_info['duration']:.2f}s)")
                self.thread_finished.emit(thread_name)

        except Exception as e:
            logger.error(
                f"[ID:0805] Error unregistering thread {thread_name}: {e}")
            logger.error(
                f"[ID:0806] Thread unregistration error traceback: {traceback.format_exc()}")
    ...
    def _on_thread_started(self, thread_name: str):
        ...
        try:
            ...
            logger.error(
                f"[ID:0808] Error handling thread started for {thread_name}: {e}")
    ...
    def _on_thread_finished(self, thread_name: str):
        ...
        try:
            if thread_name in self.active_threads:
                ...
                logger.debug(
                    f"[ID:0809] Thread finished: {thread_name} (Duration: {thread_info['duration']:.2f}s)")

        except Exception as e:
            logger.error(
                f"[ID:0810] Error handling thread finished for {thread_name}: {e}")
    ...
    def get_thread_stats(self) -> Dict:
        ...
        try:
            ...
            completed_threads = [
                t for t in self.thread_history if 'duration' in t]
            avg_duration = 0.0
            if completed_threads:
                avg_duration = sum(t['duration']
                                   for t in completed_threads) / len(completed_threads)
            ...
            logger.error(
                f"[ID:0813] Thread stats error traceback: {traceback.format_exc()}")
            return {
                'active_threads': 0,
                'completed_threads': 0,
    ...
    def cleanup(self):
        ...
        try:
            logger.debug(
                f"[ID:0814] Cleaning up ThreadMonitor - ID: {self._monitor_id}")
            ...
            logger.error(
                f"[ID:0818] ThreadMonitor cleanup error traceback: {traceback.format_exc()}")
...
class ThreadSafeLogger:
    ...
    def log_thread_context(message: str, thread: Optional[QThread] = None):
        ...
        try:
            ...
            if thread:
                ...
                logger.debug(
                    f"[ID:0819] {message} - Current: {thread_name} (ID: {thread_id}), Target: {target_thread_name} (ID: {target_thread_id})")
            else:
                logger.debug(
                    f"[ID:0820] {message} - Thread: {thread_name} (ID: {thread_id})")
    ...
    def log_thread_safety_check(operation: str, current_thread: QThread, target_thread: QThread):
        ...
        try:
            ...
            if current_thread == target_thread:
                logger.debug(
                    f"[ID:0822] Thread-safe operation: {operation} - Same thread: {current_name}")
            else:
                logger.warning(
                    f"[ID:0823] Cross-thread operation: {operation} - Current: {current_name}, Target: {target_name}")
    ...
    def log_thread_operation(operation: str, thread_name: str, details: str = ""):
        ...
        try:
            ...
            logger.debug(
                f"[ID:0825] Thread operation: {operation} on {thread_name}{details_str}")
