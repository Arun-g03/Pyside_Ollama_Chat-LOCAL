...
class SummarizationService(QObject):
    ...
    # Signals
    # Emits (filepath, generated_name)
    summarization_completed = Signal(str, str)
    # Emits (filepath, error_message)
    summarization_failed = Signal(str, str)

    def __init__(self, ollama_service: OllamaService):
        super().__init__()
        self.ollama_service = ollama_service
        self.summarization_model = "Auto"  # Use auto mode to select smallest model
        # Reduced minimum messages for testing
        self.min_messages_for_summarization = 3
    ...
    def generate_chat_name(self, conversation: List[Dict], filepath: str) -> None:
        ...
        try:
            logger.info(
                f"[ID:0064] Starting name generation for conversation: {filepath}")
            logger.info(
                f"[ID:0063] Conversation has {len(conversation)} messages, minimum required: {self.min_messages_for_summarization}")

            # Check if we have enough messages to summarize
            if len(conversation) < self.min_messages_for_summarization:
                logger.debug(
                    f"[ID:0062] Not enough messages ({len(conversation)}) for summarization")
                return
            ...
            if not user_messages:
                logger.debug(
                    "[ID:0061] No user messages found for summarization")
                return

            # Check if the conversation has enough substance for meaningful naming
            if not self._has_enough_substance(user_messages):
                logger.debug(
                    "[ID:0060] Conversation lacks sufficient substance for meaningful naming")
                return

            logger.info(
                f"[ID:0059] Found {len(user_messages)} user messages for summarization")
            ...
            if self.summarization_model == "Auto":
                ...
                if not available_models:
                    logger.warning(
                        "[ID:0056] No models available for summarization")
                    self.summarization_failed.emit(
                        filepath, "No models available")
                    return
                ...
                logger.info(
                    f"[ID:0055] Auto-selected model for summarization: {chosen_model}")
            else:
                chosen_model = self.summarization_model

            logger.info(
                f"[ID:0054] Sending summarization request to model: {chosen_model}")
            ...
            logger.info(
                f"[ID:0051] Cleaned generated name: '{generated_name}'")
            ...
            if generated_name:
                ...
                self.summarization_failed.emit(
                    filepath, "Failed to generate valid name")
...
Generate a concise, meaningful name that accurately describes what this conversation is about. If the conversation lacks substance or clear topic, respond with "NO_TOPIC". Return only the name or "NO_TOPIC", nothing else."""
        ...
        logger.debug(
            f"[ID:0046] Created summarization prompt with {len(sample_messages)} sample messages")
    ...
    def _clean_generated_name(self, response: str) -> Optional[str]:
        ...
        for prefix in prefixes_to_remove:
            if name.lower().startswith(prefix.lower()):
                ...
                logger.debug(
                    f"[ID:0040] Removed prefix '{prefix}', result: '{name}'")
        ...
        if len(name) < 3:
            logger.debug(
                f"[ID:0037] Name too short ({len(name)} chars): '{name}'")
            return None

        if len(name) > 50:
            logger.debug(
                f"[ID:0036] Name too long ({len(name)} chars): '{name}'")
            return None
...
Respond with only "YES" if the conversation has enough substance for naming, or "NO" if it lacks substance (e.g., mostly greetings, random thoughts, or no clear topic)."""
            ...
            if self.summarization_model == "Auto":
                ...
                if not available_models:
                    logger.warning(
                        "[ID:0032] No models available for quality evaluation")
                    return False
                ...
                logger.debug(
                    f"[ID:0031] Auto-selected model for quality evaluation: {chosen_model}")
            else:
                chosen_model = self.summarization_model
            ...
            logger.debug(
                f"[ID:0030] Quality evaluation response: '{response}'")

            # Check for positive evaluation
            if "YES" in response:
                logger.debug(
                    "[ID:0029] AI determined conversation has sufficient substance")
                return True
            else:
                logger.debug(
                    "[ID:0028] AI determined conversation lacks sufficient substance")
                return False

        except Exception as e:
            logger.error(
                f"[ID:0027] Error evaluating conversation quality: {str(e)}")
    ...
    def _fallback_quality_check(self, user_messages: List[str]) -> bool:
        ...
        logger.debug(
            f"[ID:0026] Fallback check: {substantial_messages} substantial messages out of {len(user_messages)} total")
...
Respond with only "YES" if the name is suitable, or "NO" if it's not suitable (e.g., too generic, random, or inappropriate)."""
            ...
            if self.summarization_model == "Auto":
                ...
                if not available_models:
                    logger.warning(
                        "[ID:0025] No models available for name quality evaluation")
                    return False
                ...
                logger.debug(
                    f"[ID:0024] Auto-selected model for name quality evaluation: {chosen_model}")
            else:
                chosen_model = self.summarization_model
            ...
            response = response.strip().upper()
            logger.debug(
                f"[ID:0023] Name quality evaluation response: '{response}'")
