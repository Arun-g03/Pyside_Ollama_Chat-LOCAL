...
class MemoryTab(QWidget):
    ...
    def create_settings_tab(self):
        ...
        self.include_memories_checkbox = QCheckBox(
    ...
    def create_memories_tab(self):
        ...
        self.memory_type_filter.addItems(
            ["All Types", "conversation", "summary", "fact", "preference", "skill", "relationship"])
    ...
    def refresh_overview(self):
        ...
        self.context_messages_label.setText(
            f"Max Context Messages: {stats['max_context_messages']}")

        # Add semantic search information
        semantic_stats = stats.get('semantic_search', {})
        if semantic_stats:
            model_status = "Loaded" if semantic_stats.get(
            ...
            self.semantic_count_label.setText(
                f"Vectorized Memories: {vectorized_count}")
        ...
        for i, (memory_type, count) in enumerate(stats['memory_types'].items()):
            self.memory_types_table.setItem(
                i, 0, QTableWidgetItem(memory_type))
        ...
        recent_entries = sorted(
            all_entries, key=lambda x: x['timestamp'], reverse=True)[:10]
    ...
    def refresh_memories(self):
        ...
        scope = self.memory_scope_filter.currentText() if hasattr(
            self, 'memory_scope_filter') else "Short-Term Memory"
        if scope == "Short-Term Memory":
            ...
            for i, memory in enumerate(memories):
                ...
                self.memories_table.setItem(
                    i, 1, QTableWidgetItem(memory.get("content", "")))
                self.memories_table.setItem(
                    i, 2, QTableWidgetItem(memory.get("role", "")))
            ...
            for i, entry in enumerate(ltm_entries):
                ...
                content = entry.summary if entry.type == "summary" else entry.value
                self.memories_table.setItem(
                    i, 1, QTableWidgetItem(content or ""))
                self.memories_table.setItem(i, 2, QTableWidgetItem(entry.type))
                self.memories_table.setItem(
                    i, 3, QTableWidgetItem(str(entry.importance)))
                # Handle potential None tags
                tags = entry.tags if entry.tags is not None else []
                self.memories_table.setItem(
                    i, 4, QTableWidgetItem(", ".join(tags)))
                self.memories_table.setItem(
                    i, 5, QTableWidgetItem(str(entry.access_count)))
                self.memories_table.setItem(
                    i, 6, QTableWidgetItem(entry.timestamp))
    ...
    def refresh_summaries(self):
        ...
        for summary in summaries:
            ...
            item.setData(Qt.UserRole, summary.timestamp)
    ...
    def search_memories(self):
        ...
        for i, memory in enumerate(results):
            ...
            self.memories_table.setItem(
                i, 1, QTableWidgetItem(memory.content[:50] + "..."))
            self.memories_table.setItem(
                i, 2, QTableWidgetItem(memory.memory_type))
            self.memories_table.setItem(
                i, 3, QTableWidgetItem(f"{memory.importance:.2f}"))
            self.memories_table.setItem(
                i, 4, QTableWidgetItem(", ".join(memory.tags)))
            # Access count not available for old memories
            self.memories_table.setItem(i, 5, QTableWidgetItem("-"))
            ...
            try:
                ...
                if memory:
                    ...
                    self.memory_details_text.setText(
                        "Memory not found or no longer available.")
            except Exception as e:
                self.memory_details_text.setText(
                    f"Error loading memory details: {str(e)}")
    ...
    def show_summary_details(self):
        ...
        if current_item:
            ...
            summaries = self.memory_service.ltm_service.get_entries('summary')
            summary = next(
                (s for s in summaries if s.timestamp == summary_timestamp), None)
    ...
    def summarize_current_conversation(self):
        ...
        QMessageBox.information(
            self, "Info", "Summarize functionality is connected to the conversation service.")
    ...
    def _summarize_with_service(self):
        ...
        if hasattr(self, 'conversation_service') and self.conversation_service:
            ...
            QMessageBox.warning(
                self, "Error", "Conversation service not connected.")
    ...
    def clear_all_memories(self):
        ...
        if reply == QMessageBox.Yes:
            ...
            self.refresh_data()
            QMessageBox.information(
                self, "Success", "All memories cleared successfully!")
    ...
    def cleanup_memory_entries(self):
        ...
        if reply == QMessageBox.Yes:
            try:
                ...
                self.refresh_data()
                QMessageBox.information(
                    self, "Success", "Memory entries cleaned up successfully!")
            except Exception as e:
                QMessageBox.critical(
                    self, "Error", f"Failed to cleanup memory entries: {str(e)}")
