...
class ConversationService(QObject):
    ...
    # Emits the saved filepath when auto-save completes
    auto_save_completed = Signal(str)

    def __init__(self, history_dir: str = "User_history/Chat_history", memory_service=None):
        super().__init__()
        self.history_dir = history_dir
        self.memory_service = memory_service
        # List of message dicts: {"role": ..., "content": ..., "id": ...}
        self.conversation = []
        self.metadata = {}
    ...
    def add_message(self, role: str, content: str, message_id: Optional[str] = None, thought: str = "", personality: Optional[str] = None):
        ...
        if not content or not content.strip():
            logger.warning(
                f"[EMPTY_MESSAGE] Attempted to add empty {role} message - ignoring")
            return None
        ...
        logger.debug(
            f"[ADD_MSG] Emitting conversation_updated signal with {len(self.conversation)} messages")
        self.conversation_updated.emit(self.conversation)
        logger.debug(
            f"[ID:0019] DEBUG: Added message to conversation: {message}", print_to_terminal=True)
        logger.debug(
            f"[ID:0018] DEBUG: Total conversation length: {len(self.conversation)}")
        logger.debug(
            f"[ID:0017] DEBUG: Conversation contents: {[msg.get('role', 'unknown') for msg in self.conversation]}")
    ...
    def start_streaming_message(self, role: str = "assistant", personality: Optional[str] = None) -> str:
        ...
        logger.debug(
            "[CS_STREAM_RESET] Reset _streaming_message_id before starting new streaming message")
        message_id = self._get_next_message_id()
        message = {
            "role": role,
            "content": "",
        ...
        self.conversation_updated.emit(self.conversation)
        logger.debug(
            f"[CS_STREAM_START] Started new streaming message: {message_id}")
        return message_id
    ...
    def update_streaming_message(self, chunk: str, append: bool = True, msg_id: str = None, chunk_index: int = None) -> bool:
        ...
        try:
            logger.debug(
                f"[CS_DEBUG] update_streaming_message called with chunk: '{chunk[:50]}...', append: {append}, msg_id: {msg_id}, chunk_index: {chunk_index}")

            # Find the streaming message
            target_id = msg_id if msg_id else self._streaming_message_id
            if not target_id:
                logger.error(
                    f"[CS_DEBUG] No target message ID found for streaming update")
                logger.error(
                    f"[CS_DEBUG] Available message IDs: {[msg.get('id') for msg in self.conversation]}")
                return False
            ...
            if not target_message:
                logger.error(
                    f"[CS_DEBUG] Target message {target_id} not found in conversation")
                logger.error(
                    f"[CS_DEBUG] Available message IDs: {[msg.get('id') for msg in self.conversation]}")
                logger.error(
                    f"[CS_DEBUG] This indicates a streaming message ID mismatch!")
                return False

            # Log the message state before update
            old_content_length = len(target_message.get('content', ''))
            logger.debug(
                f"[CS_DEBUG] Before update - Message {target_id}: content_length={old_content_length}, is_streaming={target_message.get('is_streaming')}")
            ...
            logger.debug(
                f"[CS_DEBUG] After update - Message {target_id}: content_length={new_content_length}, chunk_index={chunk_index}")
            logger.debug(
                f"[CS_DEBUG] Content change: +{len(chunk)} chars (from {old_content_length} to {new_content_length})")
            ...
            logger.error(
                f"[CS_DEBUG] Error traceback: {traceback.format_exc()}")
            return False

    def finalize_streaming_message(self) -> bool:
        """Finalize the current streaming message"""
        logger.debug(
            f"[STREAM_FINALIZE] Attempting to finalize streaming message: {self._streaming_message_id}")
        logger.debug(
            f"[STREAM_FINALIZE] Current conversation has {len(self.conversation)} messages")

        # Debug: Log all messages to see their state
        for i, msg in enumerate(self.conversation):
            logger.debug(
                f"[STREAM_FINALIZE] Message {i}: id={msg.get('id')}, role={msg.get('role')}, is_streaming={msg.get('is_streaming')}, content_len={len(msg.get('content', ''))}")

        if not self._streaming_message_id:
            logger.warning(
                "[STREAM_FINALIZE] No streaming message to finalize")
            return False
        ...
        for msg in self.conversation:
            if msg.get('id') == self._streaming_message_id and msg.get('is_streaming', False):
                ...
                logger.debug(
                    f"[STREAM_FINALIZE] Finalized streaming message: {msg['id']}")
                self._streaming_message_id = None
                self.conversation_updated.emit(self.conversation)
                return True
        logger.warning(
            f"[STREAM_FINALIZE] Streaming message id {self._streaming_message_id} not found or already finalized")
        self._streaming_message_id = None
        return False
    ...
    def get_messages(self) -> List[Dict]:
        ...
        logger.debug(
            f"[ID:0016] DEBUG: get_messages called, returning {len(self.conversation)} messages")
        logger.debug(
            f"[ID:0015] DEBUG: Message roles: {[msg.get('role', 'unknown') for msg in self.conversation]}")
        ...
        for msg in self.conversation:
            if msg.get("role") == "assistant" and 'thought' in msg and not msg['thought'] and '<think>' in msg['content'] and '</think>' in msg['content']:
                ...
                if match:
                    ...
                    msg['content'] = re.sub(
                        r'<think>.*?</think>', '', msg['content'], flags=re.DOTALL).lstrip('\n')
        # --- END MIGRATION LOGIC ---
    ...
    def clear_conversation(self):
        ...
        self.conversation_updated.emit(self.conversation)
        logger.debug(
            "[CONVERSATION_SERVICE] Conversation cleared and state reset")
    ...
    def _add_to_memory(self, role: str, content: str):
        ...
        if role == "user":
            ...
            importance = min(0.8, 0.3 + (len(content) / 1000) * 0.5)
        elif role == "assistant":
            # Longer assistant responses are more important
            importance = min(0.7, 0.2 + (len(content) / 2000) * 0.5)
    ...
    def load_conversation(self, filename: str) -> List[Dict]:
        ...
        try:
            if self.conversation_manager:
                ...
                logger.debug(
                    f"[LOAD_DEBUG] Loading conversation from filepath: {filepath}")
                ...
                for message in self.conversation:
                    if message.get("is_streaming", False):
                        logger.debug(
                            f"[LOAD_FIX] Finalizing streaming message {message.get('id')} from loaded conversation")
                ...
                logger.debug(
                    f"[LOAD_FIX] Loaded conversation with {len(self.conversation)} messages, max_id: {max_id}")
                logger.debug(
                    f"[LOAD_FIX] Emitting conversation_updated signal with {len(self.conversation)} messages")
                self.conversation_updated.emit(self.conversation)
                ...
                logger.warning(
                    "[LOAD_DEBUG] Conversation manager not available, using fallback loading")
                filepath = os.path.join(self.history_dir, filename)
            ...
            logger.error(
                f"[LOAD_ERROR] Failed to load conversation {filename}: {e}")
            import traceback
