...
class ConversationService(QObject):
    ...
    def add_message(self, role: str, content: str, message_id: Optional[str] = None):
        ...
        logger.debug(f"[ID:0020] DEBUG: Added message to conversation: {message}")
        logger.debug(f"[ID:0019] DEBUG: Total conversation length: {len(self.conversation)}")
        logger.debug(f"[ID:0018] DEBUG: Conversation contents: {[msg.get('role', 'unknown') for msg in self.conversation]}")
    ...
    def get_messages(self) -> List[Dict]:
        ...
        logger.debug(f"[ID:0017] DEBUG: get_messages called, returning {len(self.conversation)} messages")
        logger.debug(f"[ID:0016] DEBUG: Message roles: {[msg.get('role', 'unknown') for msg in self.conversation]}")
