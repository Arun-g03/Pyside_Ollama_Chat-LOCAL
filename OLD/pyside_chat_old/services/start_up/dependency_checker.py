    ...
    def run_install_dependencies(self) -> bool:
        ...
        try:
            ...
            if not os.path.exists(install_script):
                print(f"[ID:0264] Error: install_dependencies.py not found at {install_script}")
                return False
...
def check_and_install_dependencies(auto_install: bool = True, verbose: bool = False) -> bool:
    ...
    if verbose:
        print(f"[ID:0258] Dependency issues found: {checker.get_dependency_summary()}")
        if checker.broken_imports:
            print(f"[ID:0257] Missing: {', '.join(checker.broken_imports)}")
        if checker.version_conflicts:
            print(f"[ID:0256] Version conflicts: {len(checker.version_conflicts)}")
...
if __name__ == "__main__":
    ...
    print(f"[ID:0239] Can start application: {'✅ Yes' if not needs_fixing else '❌ No'}")
    if checker.optional_broken:
        print(f"[ID:0238] Note: {len(checker.optional_broken)} optional features unavailable")
