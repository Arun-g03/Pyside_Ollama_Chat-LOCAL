...
try:
    ...
    logger.debug(
        "[ID:0016] Official Ollama library not available, using requests")
...
class OllamaService(QObject):
            ...
            if OLLAMA_LIBRARY_AVAILABLE:
                try:
                    ...
                    if hasattr(ollama, 'set_host'):
                        ...
                        logger.info(
                            f"[ID:0017] Official Ollama client initialized with host: {self.base_url}")
                    else:
                        logger.info(
                            f"[ID:0017] Official Ollama client available but set_host not supported")
                except Exception as e:
                    logger.warning(
                        f"[ID:0018] Failed to initialize official Ollama client: {e}")

            LoggingHelpers.log_service_initialization("OllamaService", True)
        except Exception as e:
            LoggingHelpers.log_service_initialization(
                "OllamaService", False, e)
            raise
    ...
    def _get_models_with_library(self) -> List[str]:
        ...
        try:
            ...
            logger.debug(
                f"[ID:0019] Using official library to get models from: {self.base_url}")
            ...
            if hasattr(models, 'models'):
                ...
                for model in models.models:
                    ...
                    if hasattr(model, 'name'):
                        ...
                        model_name = model_dict.get('name') or model_dict.get(
                            'model') or model_dict.get('id')
                        if model_name:
                ...
                logger.warning(
                    f"[ID:0020A] Unknown models response format: {type(models)}")
            ...
            LoggingHelpers.log_info_with_context(
                "Successfully retrieved models using official library", {"count": len(model_names)})
            return model_names

        except Exception as e:
            logger.debug(
                f"[ID:0020] Official library failed, falling back to requests: {e}")
            LoggingHelpers.log_exception_with_context(
                "get_models_with_library", e, {"base_url": self.base_url})
    ...
    def _get_models_with_requests(self) -> List[str]:
        ...
        try:
            LoggingHelpers.log_network_request(
                f"{self.base_url}/api/tags", "GET")
            logger.debug(
                f"[ID:0013] DEBUG: Attempting to connect to Ollama at: {self.base_url}/api/tags")

            response = requests.get(
                f"{self.base_url}/api/tags", timeout=5)  # Add timeout
            logger.debug(
                f"[ID:0012] DEBUG: Response status: {response.status_code}")

            LoggingHelpers.log_network_request(
                f"{self.base_url}/api/tags", "GET", response.status_code)
            ...
            if response.status_code == 200:
                try:
                    ...
                    self.model_list_updated.emit(model_names)
                    LoggingHelpers.log_info_with_context(
                        "Successfully retrieved models using requests", {"count": len(model_names)})
                    return model_names
                except json.JSONDecodeError as e:
                    LoggingHelpers.log_exception_with_context("JSON parsing in get_models", e, {
                ...
                logger.debug(
                    f"[ID:0010] DEBUG: Error response: {response.text}")
            ...
            logger.debug(
                f"[ID:0009] DEBUG: Connection error: {e}", print_to_terminal=True)
            LoggingHelpers.log_network_request(
                f"{self.base_url}/tags", "GET", error=e)
            error_msg = "Cannot connect to Ollama. Make sure it's running on localhost:11434"
            self.model_operation_error.emit(error_msg)
            return []
        except requests.exceptions.Timeout as e:
            logger.debug(
                f"[ID:0008] DEBUG: Timeout error: {e}", print_to_terminal=True)
            LoggingHelpers.log_network_request(
                f"{self.base_url}/tags", "GET", error=e)
            ...
            logger.debug(
                f"[ID:0007] DEBUG: Unexpected error: {e}", print_to_terminal=True)
            LoggingHelpers.log_exception_with_context(
                "get_models", e, {"base_url": self.base_url})
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            LoggingHelpers.log_network_request(
                f"{self.base_url}/api/tags", "GET", error=e)
            return False
    ...
    def _send_chat_message_with_library(self, model: str, messages: List[Dict],
        ...
        try:
            ...
            if session_variables:
                try:
                    session_commands = self._build_session_commands(
                        session_variables)
                    if session_commands:
                        session_system_message = "\n".join(session_commands)
                        request_data["messages"].insert(
                            0, {"role": "system", "content": session_system_message})
                        logger.debug(
                            f"[ID:0021] SESSION: Sending session commands: {session_commands}")
                except Exception as e:
                    LoggingHelpers.log_exception_with_context("session_variables processing", e, {
            ...
            if self.cancellation_requested:
                LoggingHelpers.log_debug(
                    "Request cancelled before library call")
                return

            logger.debug(
                f"[ID:0022] ====================SENDING TO OLLAMA (LIBRARY)====================\n{json.dumps(request_data, indent=2)}\n=========================================================")
            ...
            try:
                ...
                LoggingHelpers.log_exception_with_context(
                    "Library chat request", e, {"model": model})
                # Fall back to requests method
                logger.debug(
                    f"[ID:0023] Library failed, falling back to requests: {e}")
                yield from self._send_chat_message_with_requests(model, messages, temperature, stream, session_variables)

        except Exception as e:
            LoggingHelpers.log_exception_with_context(
                "send_chat_message_with_library", e, {"model": model})
            yield f"Error: {e}"
    ...
    def _send_chat_message_with_requests(self, model: str, messages: List[Dict],
        ...
        try:
                ...
                try:
                    ...
                    if session_commands:
                        ...
                        data["messages"].insert(
                            0, {"role": "system", "content": session_system_message})
                        logger.debug(
                            f"[ID:0005] SESSION: Sending session commands: {session_commands}")
                except Exception as e:
                    LoggingHelpers.log_exception_with_context("session_variables processing", e, {
                                                              "session_variables": session_variables})
            ...
            if self.cancellation_requested:
                LoggingHelpers.log_debug(
                    "Request cancelled before network call")
                return

            logger.debug(
                f"[ID:0003] ====================SENDING TO OLLAMA====================\n{json.dumps(data, indent=2)}\n=========================================================")
            ...
            try:
                        ...
                        for line in response.iter_lines(decode_unicode=True):
                            if line and not self.cancellation_requested:
                                try:
                                    ...
                                    LoggingHelpers.log_exception_with_context(
                                        "JSON parsing in streaming response", e, {"line": line})
                                    continue
                                except Exception as e:
                                    LoggingHelpers.log_exception_with_context(
                                        "Streaming response processing", e, {"line": line})
                                    continue
                    else:
                        # Non-streaming response
                        try:
                            ...
                            LoggingHelpers.log_exception_with_context(
                                "JSON parsing in non-streaming response", e, {"response_text": response.text})
                ...
                LoggingHelpers.log_exception_with_context(
                    "send_chat_message_with_requests", e, {"model": model})
                yield f"Error: {e}"
    ...
    def _pull_model_thread(self, model_name: str) -> None:
        ...
        try:
            LoggingHelpers.log_thread_operation(
                "pull_model_thread", f"pull {model_name}", True)

            if OLLAMA_LIBRARY_AVAILABLE:
                # Try with official library first
                try:
                    logger.info(
                        f"[ID:0024] Using official library to pull model: {model_name}")
                    ...
                    logger.debug(
                        f"[ID:0025] Library pull failed, falling back to subprocess: {e}")
            ...
            if return_code:
                ...
                LoggingHelpers.log_thread_operation(
                    "pull_model_thread", f"pull {model_name}", False, Exception(error_msg))
                self.model_operation_error.emit(error_msg)
            else:
                LoggingHelpers.log_thread_operation(
                    "pull_model_thread", f"pull {model_name}", True)
            ...
            LoggingHelpers.log_thread_operation(
                "pull_model_thread", f"pull {model_name}", False, e)
            self.model_operation_error.emit(
                "Ollama command not found. Is it installed and in your PATH?")
        except Exception as e:
            LoggingHelpers.log_thread_operation(
                "pull_model_thread", f"pull {model_name}", False, e)
    ...
    def _remove_model_thread(self, model_name: str) -> None:
        ...
        try:
            LoggingHelpers.log_thread_operation(
                "remove_model_thread", f"remove {model_name}", True)

            if OLLAMA_LIBRARY_AVAILABLE:
                # Try with official library first
                try:
                    logger.info(
                        f"[ID:0026] Using official library to remove model: {model_name}")
                    ...
                    logger.debug(
                        f"[ID:0027] Library remove failed, falling back to subprocess: {e}")
            ...
            if return_code:
                ...
                LoggingHelpers.log_thread_operation(
                    "remove_model_thread", f"remove {model_name}", False, Exception(error_msg))
                self.model_operation_error.emit(error_msg)
            else:
                LoggingHelpers.log_thread_operation(
                    "remove_model_thread", f"remove {model_name}", True)
            ...
            LoggingHelpers.log_thread_operation(
                "remove_model_thread", f"remove {model_name}", False, e)
            self.model_operation_error.emit(
                "Ollama command not found. Is it installed and in your PATH?")
        except Exception as e:
            LoggingHelpers.log_thread_operation(
    ...
    def _update_model_thread(self, model_name: str) -> None:
        ...
        try:
            LoggingHelpers.log_thread_operation(
                "update_model_thread", f"update {model_name}", True)

            if OLLAMA_LIBRARY_AVAILABLE:
                # Try with official library first (update is same as pull)
                try:
                    logger.info(
                        f"[ID:0028] Using official library to update model: {model_name}")
                    ollama.pull(model_name)  # Update is same as pull
                    LoggingHelpers.log_thread_operation(
                        "update_model_thread", f"update {model_name}", True)
                    self.model_operation_finished.emit(
                        f"Successfully updated model: {model_name}")
                    return
                except Exception as e:
                    logger.debug(
                        f"[ID:0029] Library update failed, falling back to subprocess: {e}")
            ...
            if return_code:
                ...
                LoggingHelpers.log_thread_operation(
                    "update_model_thread", f"update {model_name}", False, Exception(error_msg))
                self.model_operation_error.emit(error_msg)
            else:
                LoggingHelpers.log_thread_operation(
                    "update_model_thread", f"update {model_name}", True)
            ...
            LoggingHelpers.log_thread_operation(
                "update_model_thread", f"update {model_name}", False, e)
            self.model_operation_error.emit(
                "Ollama command not found. Is it installed and in your PATH?")
        except Exception as e:
            LoggingHelpers.log_thread_operation(
    ...
    def _extract_system_prompt(self, messages: List[Dict]) -> str:
        ...
        try:
            ...
            LoggingHelpers.log_exception_with_context(
                "extract_system_prompt", e, {"messages_count": len(messages)})
            return ""
    ...
    def _build_session_commands(self, session_variables: Dict) -> List[str]:
        ...
        try:
            ...
            LoggingHelpers.log_exception_with_context("build_session_commands", e, {
                                                      "session_variables": session_variables})
            return []
    ...
    def cleanup(self):
        ...
        try:
            ...
            if self._active_operations:
                logger.debug(
                    f"[DEBUG] Waiting for {len(self._active_operations)} active operations to complete")
