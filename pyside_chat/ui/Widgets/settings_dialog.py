...
class SettingsDialog(QDialog):
    ...
    def load_current_settings(self):
        ...
        if not self.ui_ready:
            logger.debug(f"[ID:0232] SETTINGS: UI not ready yet, skipping settings load",print_to_terminal=False)
            return
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"[ID:0231] SETTINGS: UI elements deleted during load: {e}",print_to_terminal=True)
                return
            ...
            logger.debug(f"[ID:0230] SETTINGS: Error loading settings: {e}",print_to_terminal=True)
