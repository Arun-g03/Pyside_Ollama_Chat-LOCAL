...
class ThreadingIntegration(QObject):
    ...
    def start_chat_streaming(self, context_messages: List[Dict], chosen_model: str, temperature: float, config_manager) -> bool:
        ...
        try:
            logger.debug(
                f"[ID:TI002] Starting chat streaming for model: {chosen_model}")
    ...
    def process_message_spell_check(self, message: str, callback) -> str:
        ...
        try:
            logger.debug(
                f"[ID:TI006] Processing message spell check: {message[:50]}...")
            return self.threading_service.process_message(message, "spell_check", callback)

        except Exception as e:
            logger.error(
                f"[ID:TI007] Error processing message spell check: {e}")
            return ""
    ...
    def process_message_formatting(self, message: str, callback) -> str:
        ...
        try:
            logger.debug(
                f"[ID:TI008] Processing message formatting: {message[:50]}...")
            return self.threading_service.process_message(message, "formatting", callback)

        except Exception as e:
            logger.error(
                f"[ID:TI009] Error processing message formatting: {e}")
            return ""
    ...
    def process_file_operation(self, file_path: str, operation: str, **kwargs) -> str:
        ...
        try:
            logger.debug(
                f"[ID:TI010] Processing file operation: {operation} on {file_path}")
...
class EventBusThreadingBridge:
    ...
    def start_chat_streaming(self, context_messages: List[Dict], chosen_model: str, temperature: float, config_manager) -> bool:
        ...
        try:
            logger.debug(
                f"[ID:TB010] Starting chat streaming via bridge - Model: {chosen_model}")
            return self.threading_integration.start_chat_streaming(context_messages, chosen_model, temperature, config_manager)

        except Exception as e:
            logger.error(
                f"[ID:TB011] Error starting chat streaming via bridge: {e}")
            return False
    ...
    def stop_chat_streaming(self):
        ...
        try:
            ...
            logger.error(
                f"[ID:TB013] Error stopping chat streaming via bridge: {e}")
    ...
    def get_threading_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"[ID:TB014] Error getting threading status via bridge: {e}")
            return {}
