...
def example_persistent_thread_pool():
    ...
    persistent_pool.initialize_pool(
        'chat_streaming', DummyWorker, size=background_threads)
    persistent_pool.initialize_pool(
        'monitoring', DummyWorker, size=ui_update_threads)
...
def example_memory_optimization():
    ...
    if recommendations.memory_gb < 8.0:
        ...
        print(
            f"Low memory system ({recommendations.memory_gb:.1f} GB), using conservative thread count: {thread_count}")
    elif recommendations.memory_gb < 16.0:
        # Medium memory system
        thread_count = recommendations.memory_safe_threads
        print(
            f"Medium memory system ({recommendations.memory_gb:.1f} GB), using memory-safe thread count: {thread_count}")
    else:
        # High memory system
        thread_count = recommendations.max_worker_threads
        print(
            f"High memory system ({recommendations.memory_gb:.1f} GB), using max thread count: {thread_count}")
...
def example_dynamic_adjustment():
    ...
    if cpu_percent > 80 or memory_percent > 80:
        ...
        print(
            f"High system load detected, reducing threads from {base_threads} to {adjusted_threads}")
    elif cpu_percent < 30 and memory_percent < 50:
        # Low load - can use more threads
        adjusted_threads = min(base_threads + 2, get_pool_thread_count('max'))
        print(
            f"Low system load detected, increasing threads from {base_threads} to {adjusted_threads}")
    else:
        # Normal load - use recommended threads
        adjusted_threads = base_threads
        print(
            f"Normal system load, using recommended threads: {adjusted_threads}")
