...
class VoiceProcessManager(QObject):
    ...
    def start_voice_process(self) -> bool:
        ...
        try:
            ...
            if self.voice_process and self.voice_process.is_alive():
                logger.debug(
                    "[ID:0521] Stopping existing voice process before starting new one")
            ...
            logger.debug(
                f"[ID:0522] Creating voice process - PID: {self.voice_process.pid if self.voice_process.is_alive() else 'not started'}")
            ...
            logger.debug(
                f"[ID:0524] Voice process started successfully - PID: {self.voice_process.pid}")

            # Start the monitoring thread
            self.monitor_thread = VoiceProcessMonitor(
                self.response_queue, self)
            ...
            logger.debug(
                "[ID:0526] Voice process and monitor started successfully")
            return True

        except Exception as e:
            logger.error(f"[ID:0527] Error starting voice process: {e}")
            logger.error(
                f"[ID:0528] Voice process start error traceback: {traceback.format_exc()}")
            return False
        ...
        try:
            ...
            if hasattr(self, 'monitor_thread') and self.monitor_thread:
                ...
                try:
                    ...
                    logger.debug(
                        f"[ID:0531] Monitor thread stats before cleanup: {stats}")

                    if self.monitor_thread.isRunning():
                        logger.debug(
                            "[ID:0532] Waiting for monitor thread to finish")
                        # 2 second timeout
                        if not self.monitor_thread.wait(2000):
                            logger.warning(
                                "[ID:0533] Monitor thread did not finish within timeout")
                        else:
                            logger.debug(
                                "[ID:0534] Monitor thread finished successfully")

                    self.monitor_thread.deleteLater()

                except Exception as e:
                    logger.error(
                        f"[ID:0535] Error stopping monitor thread: {e}")
                    logger.error(
                        f"[ID:0536] Monitor thread stop error traceback: {traceback.format_exc()}")
                finally:
            ...
            if self.voice_process and self.voice_process.is_alive():
                logger.debug(
                    f"[ID:0537] Stopping voice process - PID: {self.voice_process.pid}")
                ...
                try:
                    ...
                    if self.voice_process.is_alive():
                        logger.warning(
                            "[ID:0538] Voice process did not stop naturally, terminating")
                        self.voice_process.terminate()
                        self.voice_process.join(timeout=2.0)

                        if self.voice_process.is_alive():
                            logger.error(
                                "[ID:0539] Voice process could not be terminated, killing")
                            self.voice_process.kill()

                    logger.debug(
                        "[ID:0540] Voice process stopped successfully")

                except Exception as e:
                    logger.error(
                        f"[ID:0541] Error stopping voice process: {e}")
                    logger.error(
                        f"[ID:0542] Voice process stop error traceback: {traceback.format_exc()}")
            ...
            logger.error(
                f"[ID:0545] Voice process cleanup error traceback: {traceback.format_exc()}")
    ...
    def send_command(self, command: str, data: Any = None):
        ...
        try:
            if not self.is_running or not self.voice_process or not self.voice_process.is_alive():
                logger.warning(
                    f"[ID:0546] Cannot send command '{command}' - voice process not running")
                return False
            ...
            logger.error(
                f"[ID:0549] Send command error traceback: {traceback.format_exc()}")
            return False
    ...
    def _handle_response(self, response: Dict[str, Any]):
        ...
        try:
            ...
            logger.debug(
                f"[ID:0550] Handling voice process response: {response_type}")

            if response_type == "voice_input_received":
                logger.debug(
                    f"[EMIT] voice_process_manager.py: voice_input_received.emit({data!r}) from id={id(self)}")
                ...
                logger.warning(
                    f"[ID:0552] Unknown response type: {response_type}")

        except Exception as e:
            logger.error(
                f"[ID:0553] Error handling voice process response: {e}")
            logger.error(
                f"[ID:0554] Response handling error traceback: {traceback.format_exc()}")
    ...
    def _handle_monitor_error(self, error_message: str):
        ...
        logger.error(
            f"[ID:0556] Monitor error occurred in thread: {QThread.currentThread().objectName()}")
...
class VoiceProcessMonitor(QThread):
    ...
    def __init__(self, response_queue: mp.Queue, parent=None):
        ...
        logger.debug(
            f"[ID:0500] VoiceProcessMonitor created - ID: {self._monitor_id}")

    def run(self):
        """Monitor the response queue"""
        try:
            logger.debug(
                f"[ID:0501] VoiceProcessMonitor started - Thread: {QThread.currentThread().objectName()}")
                ...
                try:
                    ...
                    try:
                        ...
                        if response_count % 10 == 0:  # Log every 10th response
                            logger.debug(
                                f"[ID:0502] VoiceProcessMonitor processed {response_count} responses")
                    ...
                    logger.error(
                        f"[ID:0503] Error in voice process monitor loop: {e}")
                    logger.error(
                        f"[ID:0504] VoiceProcessMonitor loop error traceback: {traceback.format_exc()}")
                    self.monitor_error.emit(f"Monitor loop error: {str(e)}")
                    break

            logger.debug(
                f"[ID:0505] VoiceProcessMonitor finished - Total responses: {response_count}")

        except Exception as e:
            logger.error(f"[ID:0506] Error in voice process monitor: {e}")
            logger.error(
                f"[ID:0507] VoiceProcessMonitor error traceback: {traceback.format_exc()}")
    ...
    def stop(self):
        ...
        try:
            logger.debug(
                f"[ID:0508] Stopping VoiceProcessMonitor - ID: {self._monitor_id}")
            self.running = False
            ...
            logger.debug(
                f"[ID:0509] VoiceProcessMonitor stop completed - ID: {self._monitor_id}")

        except Exception as e:
            logger.error(f"[ID:0510] Error stopping VoiceProcessMonitor: {e}")
            logger.error(
                f"[ID:0511] VoiceProcessMonitor stop error traceback: {traceback.format_exc()}")
        ...
        while True:
            try:
                ...
                try:
                    ...
                    logger.debug(
                        f"Error processing events in voice worker: {e}")
