...
class BarEQWidget(QWidget):
    ...
    DEFAULT_NUM_BARS = 24
    # milliseconds (60 FPS) - Options: 33ms (30 FPS), 16ms (60 FPS), 8ms (120 FPS)
    DEFAULT_ANIMATION_INTERVAL = 20
    ...
    def _animate(self):
        ...
        for i in range(self.num_bars):
            ...
            self._bar_values[i] += (target - current) * \
        ...
        if len(validated_values) != self.num_bars:
            if len(validated_values) > self.num_bars:
                ...
                validated_values.extend(
                    [self.DEFAULT_IDLE_VALUE] * (self.num_bars - len(validated_values)))
    ...
    def _calculate_bar_geometry(self, widget_width, widget_height):
        ...
        bar_width = max(
            4, int((widget_width * (1 - total_gap_ratio)) / num_bars))
        total_width = num_bars * bar_width + (num_bars + 1) * gap
        start_x = (widget_width - total_width) // 2 + \
            gap  # Centered, first bar after initial gap
    ...
    def _create_bar_gradient(self, x, y, width, height, is_reflection=False, bar_index=0):
        ...
        if is_reflection:
            ...
            gradient.setColorAt(0.0, QColor(
                colors[0][0], colors[0][1], colors[0][2], 120))
            gradient.setColorAt(0.5, QColor(
                colors[1][0], colors[1][1], colors[1][2], 60))
    ...
    def _draw_bar(self, painter, x, y, width, height, value, max_height, bar_index=0):
        ...
        # Draw main bar
        main_gradient = self._create_bar_gradient(
            x, y, width, bar_height, bar_index=bar_index)
        ...
        # Draw reflection
        if bar_height > 0:
            reflection_gradient = self._create_bar_gradient(
                x, y, width, bar_height, is_reflection=True, bar_index=bar_index)
    ...
    def paintEvent(self, event):
        ...
        for i, value in enumerate(self._bar_values):
            x = geometry['start_x'] + i * \
                (geometry['bar_width'] + geometry['gap'])
            self._draw_bar(
                painter,
                x,
                geometry['base_y'],
