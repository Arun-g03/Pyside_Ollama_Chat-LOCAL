...
class StreamingUpdateTask(QRunnable, QObject):
    ...
    def __init__(self, target: Callable, callback: Optional[Callable] = None, task_id: Optional[str] = None):
        ...
        logger.debug(
            f"[DEBUG] StreamingUpdateTask created - ID: {self.task_id}")

    def run(self):
        """Execute the streaming update task."""
        try:
            logger.debug(
                f"[DEBUG] StreamingUpdateTask started - ID: {self.task_id}")
            ...
            logger.debug(
                f"[DEBUG] StreamingUpdateTask completed - ID: {self.task_id}")

        except Exception as e:
            error_msg = f"Streaming update error: {str(e)}"
            logger.error(f"[DEBUG] {error_msg}")
            logger.error(
                f"[DEBUG] StreamingUpdateTask traceback: {traceback.format_exc()}")
    ...
    def _execute_target(self):
        ...
        try:
            ...
            logger.error(f"[DEBUG] {error_msg}")
            logger.error(
                f"[DEBUG] Target execution traceback: {traceback.format_exc()}")
            self.update_error.emit(self.task_id, error_msg)
...
class MessageProcessingTask(QRunnable, QObject):
    ...
    def __init__(self, message: str, task_type: str = "spell_check"):
        ...
        logger.debug(
            f"[ID:TR001] MessageProcessingTask created - Type: {task_type}")

    def run(self):
        """Execute the message processing task."""
        try:
            logger.debug(
                f"[ID:TR002] MessageProcessingTask started - Type: {self.task_type}")
            ...
            logger.debug(
                f"[ID:TR003] MessageProcessingTask completed - Type: {self.task_type}")

        except Exception as e:
            error_msg = f"Message processing error: {str(e)}"
            logger.error(f"[ID:TR004] {error_msg}")
            logger.error(
                f"[ID:TR005] MessageProcessingTask traceback: {traceback.format_exc()}")
    ...
    def _format_message(self) -> Dict[str, Any]:
        ...
        formatted = re.sub(
            r'([.!?])\s+([a-z])', lambda m: m.group(1) + ' ' + m.group(2).upper(), formatted)
    ...
    def _analyze_sentiment(self) -> Dict[str, Any]:
        ...
        positive_words = {'good', 'great', 'excellent',
                          'amazing', 'wonderful', 'love', 'like', 'happy', 'joy'}
        negative_words = {'bad', 'terrible', 'awful',
                          'hate', 'dislike', 'sad', 'angry', 'frustrated'}
...
class FileProcessingTask(QRunnable, QObject):
    ...
    # operation, file_path, error_message
    ...
    def __init__(self, file_path: str, operation: str, **kwargs):
        ...
        logger.debug(
            f"[ID:TR006] FileProcessingTask created - Operation: {operation}, File: {file_path}")

    def run(self):
        """Execute the file processing task."""
        try:
            logger.debug(
                f"[ID:TR007] FileProcessingTask started - Operation: {self.operation}")
            ...
            logger.debug(
                f"[ID:TR008] FileProcessingTask completed - Operation: {self.operation}")

        except Exception as e:
            error_msg = f"File processing error: {str(e)}"
            logger.error(f"[ID:TR008] {error_msg}")
            logger.error(
                f"[ID:TR009] FileProcessingTask traceback: {traceback.format_exc()}")
...
class DataProcessingTask(QRunnable, QObject):
    ...
    def __init__(self, data: Any, operation: str, **kwargs):
        ...
        logger.debug(
            f"[ID:TR010] DataProcessingTask created - Operation: {operation}")

    def run(self):
        """Execute the data processing task."""
        try:
            logger.debug(
                f"[ID:TR011] DataProcessingTask started - Operation: {self.operation}")
            ...
            logger.debug(
                f"[ID:TR013] DataProcessingTask completed - Operation: {self.operation}")

        except Exception as e:
            error_msg = f"Data processing error: {str(e)}"
            logger.error(f"[ID:TR012] {error_msg}")
            logger.error(
                f"[ID:TR013] DataProcessingTask traceback: {traceback.format_exc()}")
    ...
    def _handle_model_operation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cancellation_requested = self.kwargs.get(
            ...
            logger.debug(
                f"[DEBUG] Handling model operation: {operation} for model: {model_name}")

            # Try with official library first
            try:
                if operation == "pull":
                    logger.info(
                        f"[DEBUG] Using official library to pull model: {model_name}")
                    ...
                    logger.info(
                        f"[DEBUG] Using official library to remove model: {model_name}")
                    ...
                    logger.info(
                        f"[DEBUG] Using official library to update model: {model_name}")
                ...
                logger.debug(
                    f"[DEBUG] Library operation failed, falling back to subprocess: {e}")
            ...
            if return_code:
                ...
                if success_callback:
                    success_callback(
                        f"Successfully {operation}d model: {model_name}")
                return {"success": True, "operation": operation, "model": model_name}
...
class CalculationTask(QRunnable, QObject):
    ...
    def __init__(self, calculation_type: str, parameters: Dict[str, Any]):
        ...
        logger.debug(
            f"[ID:TR014] CalculationTask created - Type: {calculation_type}")

    def run(self):
        """Execute the calculation task."""
        try:
            logger.debug(
                f"[ID:TR015] CalculationTask started - Type: {self.calculation_type}")
            ...
            if self.calculation_type == "fibonacci":
                ...
                result = {
                    "error": f"Unknown calculation type: {self.calculation_type}"}

            self.result_ready.emit(self.calculation_type, result)

            logger.debug(
                f"[ID:TR018] CalculationTask completed - Type: {self.calculation_type}")

        except Exception as e:
            error_msg = f"Calculation error: {str(e)}"
            logger.error(f"[ID:TR016] {error_msg}")
            logger.error(
                f"[ID:TR017] CalculationTask traceback: {traceback.format_exc()}")
