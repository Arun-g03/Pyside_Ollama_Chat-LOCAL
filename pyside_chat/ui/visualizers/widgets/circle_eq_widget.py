...
class CircleEQWidget(QWidget):
    ...
    # Number of circular sections (increased from 12 for more detail)
    DEFAULT_NUM_SECTIONS = 24
    ...
    def _animate(self):
        ...
        for i in range(self.num_sections):
            ...
            self._section_values[i] += (target - current) * \
        ...
        if len(validated_values) != self.num_sections:
            if len(validated_values) > self.num_sections:
                ...
                validated_values.extend(
                    [self.DEFAULT_IDLE_VALUE] * (self.num_sections - len(validated_values)))
    ...
    def _create_section_gradient(self, center_x, center_y, inner_radius, outer_radius, angle_start, angle_span, value, section_index):
        ...
        # First 33% - bass (sections 0-7)
        bass_end = int(self.num_sections * 0.33)
        # Next 34% - mid (sections 8-15)
        mid_end = int(self.num_sections * 0.67)
        ...
        gradient.setColorAt(0.0, QColor(
            colors[0][0], colors[0][1], colors[0][2], 200))
        gradient.setColorAt(0.7, QColor(
            colors[1][0], colors[1][1], colors[1][2], 150))
    ...
    def paintEvent(self, event):
        ...
        for i, value in enumerate(self._section_values):
            ...
            painter.drawPie(
                ...
                # Convert to Qt angle units
                int(angle_start * 180 / math.pi * 16),
