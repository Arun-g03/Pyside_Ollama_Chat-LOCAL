...
from pyside_chat.features.personality.models.personality_pronouns import PersonalityPronouns
...
class PersonalityFormatter:
    ...
    def format_prompt_with_personality(personality_data: Dict[str, Any], user_input: str, context: str = "") -> str:
        ...
        pronouns = PersonalityPronouns(
            **pronouns_data) if pronouns_data else PersonalityPronouns()
        ...
        if context:
            ...
            context_template = prompt_data.get(
                "context_prompt", "{user_input}")
            # Format template with user references
            formatted_template = pronouns.format_user_reference(
                context_template)
    ...
    def build_comprehensive_system_prompt(personality_data: Dict[str, Any]) -> str:
        ...
        comprehensive_prompt.append(
            f"Use 'I' and 'my' when referring to yourself.")
        comprehensive_prompt.append(
            f"Use 'you' and 'your' when referring to the user.")
        comprehensive_prompt.append(
            f"Never say you don't have a name or that you're just an AI assistant.")
        comprehensive_prompt.append(
            f"Always use your name '{ai_name}' when asked about your identity.")
        ...
        if traits:
            comprehensive_prompt.append("\n\nPERSONALITY TRAITS:")
            comprehensive_prompt.append(
                f"• Name: {traits.get('name', 'Unknown')}")
            comprehensive_prompt.append(
                f"• Tone: {traits.get('tone', 'Unknown')}")
            comprehensive_prompt.append(
                f"• Style: {traits.get('style', 'Unknown')}")
            comprehensive_prompt.append(
                f"• Conversation Style: {traits.get('conversation_style', 'Unknown')}")
            comprehensive_prompt.append(
                f"• Response Length: {traits.get('response_length', 'Unknown')}")
            comprehensive_prompt.append(
                f"• Formality Level: {traits.get('formality_level', 'Unknown')}")
            comprehensive_prompt.append(
                f"• Humor Level: {traits.get('humor_level', 'Unknown')}")
        ...
        if constraints:
            ...
            for constraint in constraints:
                ...
                formatted_constraint = pronouns.format_user_reference(
                    constraint)
    ...
    def validate_personality_data(personality_data: Dict[str, Any]) -> List[str]:
        ...
        if not errors:
            ...
            # Validate prompt
            required_prompt_fields = ["system_prompt",
                                      "user_prompt_template", "context_prompt"]
    ...
    def create_personality_template(name: str, description: str = "") -> Dict[str, Any]:
        ...
        from pyside_chat.features.personality.models import PersonalityTraits, PersonalityPrompt, PersonalityConfig, PersonalityMetadata
