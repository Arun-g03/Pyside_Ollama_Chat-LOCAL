...
class AppLifecycleManager:
    ...
    def initialize_application(self):
        ...
        try:
            ...
            if not self._verify_ollama_installation():
                logger.warning(
                    "[ID:VERIFY_FAILED] Ollama verification failed - app will not proceed")
                return False
            ...
            logger.info(
                "[ID:0230] Application initialization completed successfully")
            return True

        except Exception as e:
            logger.error(
                f"[ID:0229] Error during application initialization: {e}")
    ...
    def handle_show_event(self):
        ...
        try:
            ...
            logger.info(
                "[ID:0228] Application show event handled successfully")
    ...
    def handle_close_event(self, event):
        ...
        try:
            ...
            close_ollama_on_exit = self.service_manager.config_manager.get(
                "close_ollama_on_exit", True)

            if close_ollama_on_exit and self.ollama_process and self.ollama_started_by_app:
                logger.info(
                    "[ID:0226B] Closing Ollama app.exe on application exit (config enabled, started by app)")
                self._stop_ollama_process()
            elif not close_ollama_on_exit and self.ollama_process and self.ollama_started_by_app:
                logger.info(
                    "[ID:0226C] Keeping Ollama app.exe running on application exit (config disabled, started by app)")
            elif self.ollama_process and not self.ollama_started_by_app:
                logger.info(
                    "[ID:0226D] Not closing Ollama app.exe (was not started by this app)")
            else:
    ...
    def show_ollama_connection_error(self, context="general", force_show=False):
        ...
        logger.info(
            f"[ID:ERROR_DIALOG] Showing Ollama error dialog - context: {context}, force_show: {force_show}, error_shown: {self.ollama_error_shown}")

        if not force_show and self.ollama_error_shown:
            logger.info(
                "[ID:ERROR_DIALOG] Skipping error dialog (already shown and not forced)")
            return
        if not force_show:
        ...
        logger.info(
            f"[ID:ERROR_DIALOG] Creating message box with title: {title}")
        msg_box = QMessageBox(self.main_window)
        ...
        open_browser_button = msg_box.addButton(
            "Open Browser", QMessageBox.ActionRole)
        custom_path_button = msg_box.addButton(
            "Specify Custom Path", QMessageBox.ActionRole)

        # Show the dialog and handle button clicks
        logger.info(
            "[ID:ERROR_DIALOG] About to show message box with four buttons")
        result = msg_box.exec()
        logger.info(
            f"[ID:ERROR_DIALOG] Message box closed with result: {result}")
        ...
        if clicked_button == close_button:
            logger.info(
                "[ID:CLOSE_APP] User clicked Close - shutting down app")
            # Close the application
            self.main_window.close()

        elif clicked_button == recheck_button:
            logger.info(
                "[ID:RECHECK] User clicked Recheck - verifying Ollama installation")
            ...
            if self._verify_ollama_installation():
                logger.info(
                    "[ID:RECHECK] Ollama verification successful - proceeding with app initialization")
                ...
                if not self.main_window.isVisible():
                    logger.info(
                        "[ID:RECHECK] Showing main window after successful verification")
                    self.main_window.show()
                return  # Success, don't show error dialog again
            else:
                logger.warning(
                    "[ID:RECHECK] Ollama verification failed - showing error dialog again")
            ...
            try:
                ...
                logger.info(
                    f"[ID:OPEN_BROWSER] Browser open result: {success}")
                if not success:
                    logger.warning(
                        "[ID:OPEN_BROWSER] Failed to open browser, trying alternative method")
                ...
                error_msg.setInformativeText(
                    f"Please manually visit: {download_link}")
                error_msg.exec()
            ...
            logger.warning(
                f"[ID:ERROR_DIALOG] Unexpected button result: {result}")
    ...
    def show_ollama_startup_dialog(self):
        ...
        logger.info(
            "[ID:0253] Startup dialog called but Ollama is started automatically")
    ...
    def force_show_ollama_error(self, context="not_installed"):
        ...
        logger.info(
            f"[ID:FORCE_ERROR] Force showing Ollama error dialog - context: {context}")
        self.show_ollama_connection_error(context, force_show=True)
    ...
    def get_available_models(self) -> List[str]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"[ID:MODEL_QUERY] Found {len(models)} available models")
                return models
            else:
                logger.warning(
                    f"[ID:MODEL_QUERY] Error fetching models: HTTP {response.status_code}")
                return []
        except Exception as e:
    ...
    def send_test_message(self, model_name: str, message: str = "Hello, this is a test message. Please respond with a simple greeting.") -> tuple[bool, str]:
        ...
        try:
            logger.info(
                f"[ID:TEST_MSG] Sending test message to model: {model_name}")
            ...
            if response.status_code == 200:
                data = response.json()
                ai_response = data.get('message', {}).get(
                    'content', 'No response content')
                logger.info(
                    f"[ID:TEST_MSG] Successfully received response from {model_name}")
                return True, ai_response
            else:
                logger.warning(
                    f"[ID:TEST_MSG] Error sending message: HTTP {response.status_code}")
    ...
    def _ensure_ollama_running(self):
        ...
        try:
            ...
            if not self._is_ollama_installed():
                logger.warning(
                    "[ID:0234] Ollama is not installed or not accessible")

                # Try to find where Ollama should be
                ollama_executable = self._find_ollama_executable()
                logger.info(
                    f"[ID:0234A] Ollama executable path: {ollama_executable}")
                ...
                logger.info(
                    "[ID:0234B] About to show error dialog for not_installed context")
                self.show_ollama_connection_error(
                    "not_installed", force_show=True)
    ...
    def _is_ollama_installed(self) -> bool:
        ...
        try:
            ...
            if os.path.exists(ollama_executable) and os.access(ollama_executable, os.X_OK):
                logger.info(
                    f"[ID:OLLAMA_VERSION] Ollama executable found: {ollama_executable}")
                return True
            ...
            if not os.path.isabs(ollama_executable):
                ...
                if full_path and os.path.exists(full_path) and os.access(full_path, os.X_OK):
                    logger.info(
                        f"[ID:OLLAMA_VERSION] Ollama executable found in PATH: {full_path}")
                    return True

            logger.warning(
                f"[ID:OLLAMA_VERSION] Ollama executable not found: {ollama_executable}")
            return False

        except Exception as e:
            logger.error(
                f"[ID:OLLAMA_VERSION] Error checking Ollama installation: {e}")
            return False
    ...
    def _find_ollama_executable(self) -> str:
        ...
        try:
            logger.info(
                "[ID:OLLAMA_PATH_DEBUG] Starting Ollama app.exe executable search...")

            # First check for custom path from config
            custom_path = self.service_manager.config_manager.get(
                "custom_ollama_path", "")
            if custom_path and os.path.exists(custom_path):
                logger.info(
                    f"[ID:OLLAMA_PATH] Using custom Ollama path: {custom_path}")
                return custom_path
            ...
            if ollama_path:
                logger.info(
                    f"[ID:OLLAMA_PATH] Found Ollama app.exe in PATH: {ollama_path}")
                return ollama_path
            ...
            possible_paths = [
                ...
                r"C:\Users\{}\AppData\Local\Programs\Ollama\ollama app.exe".format(
                    os.getenv('USERNAME', '')),
                r"C:\Users\{}\AppData\Roaming\Ollama\ollama app.exe".format(
                    os.getenv('USERNAME', '')),
                r"C:\Users\{}\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\ollama app.exe".format(
                    os.getenv('USERNAME', '')),
            ]

            logger.info(
                f"[ID:OLLAMA_PATH_DEBUG] Checking {len(possible_paths)} possible paths for ollama app.exe...")
            for i, path in enumerate(possible_paths):
                exists = os.path.exists(path)
                logger.info(
                    f"[ID:OLLAMA_PATH_DEBUG] Path {i+1}: {path} - Exists: {exists}")
                if exists:
                    logger.info(
                        f"[ID:OLLAMA_PATH] Found Ollama app.exe at: {path}")
                    return path

            # If not found, return the expected name for PATH fallback
            logger.warning(
                "[ID:OLLAMA_PATH] Ollama app.exe not found in common locations, trying PATH fallback")
            return "ollama app.exe"

        except Exception as e:
            logger.error(
                f"[ID:OLLAMA_PATH] Error finding Ollama app.exe executable: {e}")
    ...
    def _start_ollama_background(self) -> bool:
        ...
        try:
            ...
            progress = QProgressDialog(
                "Starting Ollama...", "Cancel", 0, 100, self.main_window)
            ...
            logger.info(
                f"[ID:OLLAMA_START] Starting Ollama with executable: {ollama_executable}")

            # Start Ollama app.exe process
            try:
                logger.info(
                    f"[ID:OLLAMA_START_DEBUG] Attempting to start Ollama app.exe: {ollama_executable}")
                ...
                logger.info(
                    f"[ID:OLLAMA_START_DEBUG] Started Ollama app.exe process with PID: {self.ollama_process.pid}")
                logger.info(
                    f"[ID:0237] Started Ollama app.exe process with PID: {self.ollama_process.pid}")
                ...
                logger.error(
                    f"[ID:OLLAMA_START_ERROR] Ollama app.exe not found: {ollama_executable}")
                progress.close()
                self.show_ollama_connection_error(
                    "not_installed", force_show=True)
                return False
                ...
                logger.error(
                    f"[ID:OLLAMA_START_ERROR] Failed to start Ollama app.exe: {e}")
                logger.error(
                    f"[ID:OLLAMA_START_ERROR] Error details: {type(e).__name__}: {str(e)}")
                progress.close()
                self.show_ollama_connection_error(
                    "startup_failed", force_show=True)
                return False
            ...
            def wait_for_ollama():
                ...
                while elapsed_time < max_wait_time:
                    ...
                    # Update progress
                    progress_value = min(
                        int((elapsed_time / max_wait_time) * 100), 95)
                    progress.setValue(progress_value)

                    # Update progress text to show elapsed time
                    progress.setLabelText(
                        f"Starting Ollama... ({elapsed_time}s)")
                    ...
                    if self.check_ollama_connection():
                        ...
                        QApplication.processEvents()
                        logger.info(
                            f"[ID:0239] Ollama started successfully after {elapsed_time} seconds")
                        return True
            ...
            if self.check_ollama_connection():
                ...
                logger.warning(
                    "[ID:0243] Ollama process started but not responding")
                return False
    ...
    def _verify_ollama_installation(self) -> bool:
        ...
        try:
            logger.info(
                "[ID:VERIFY] Starting Ollama installation verification...")
            ...
            if not self.is_ollama_running():
                logger.info(
                    "[ID:VERIFY] Ollama is installed but not running - attempting to start")
            ...
            if not self.check_ollama_connection():
                logger.warning(
                    "[ID:VERIFY] Ollama is running but connection test failed")
            ...
            try:
                ...
                if not models:
                    logger.warning(
                        "[ID:VERIFY] Ollama is running but no models available")
                    # This is not a critical failure, just a warning
                else:
                    logger.info(
                        f"[ID:VERIFY] Found {len(models)} available models")
            except Exception as e:
                logger.warning(f"[ID:VERIFY] Could not retrieve models: {e}")
                # This is not a critical failure, just a warning

            logger.info(
                "[ID:VERIFY] Ollama verification completed successfully")
            return True
    ...
    def _stop_ollama_process(self):
        ...
        if self.ollama_process:
            try:
                logger.info(
                    f"[ID:0245] Stopping Ollama app.exe process (PID: {self.ollama_process.pid})")
                ...
                try:
                    # Increased timeout for GUI app
                    self.ollama_process.wait(timeout=10)
                    logger.info(
                        "[ID:0245A] Ollama app.exe process terminated gracefully")
                except subprocess.TimeoutExpired:
                    logger.warning(
                        "[ID:0246] Ollama app.exe process did not terminate gracefully, force killing")
                    self.ollama_process.kill()
                    try:
                        self.ollama_process.wait(timeout=5)
                        logger.info(
                            "[ID:0246A] Ollama app.exe process force killed successfully")
                    except subprocess.TimeoutExpired:
                        logger.error(
                            "[ID:0246B] Failed to force kill Ollama app.exe process")
                except Exception as e:
                    logger.error(
                        f"[ID:0247A] Error during graceful termination: {e}")
                    # Try force kill as fallback
                    try:
                        self.ollama_process.kill()
                        self.ollama_process.wait(timeout=5)
                        logger.info(
                            "[ID:0247B] Ollama app.exe process force killed after graceful failure")
                    except Exception as kill_error:
                        logger.error(
                            f"[ID:0247C] Failed to force kill Ollama app.exe process: {kill_error}")

            except Exception as e:
                logger.error(
                    f"[ID:0247] Error stopping Ollama app.exe process: {e}")
            finally:
                self.ollama_process = None
                logger.info(
                    "[ID:0248] Ollama app.exe process cleanup completed")
    ...
    def _check_ollama_crash(self):
        ...
        try:
            ...
            if process_status is not None:
                logger.warning(
                    f"[ID:0257] Ollama process crashed (PID: {self.ollama_process.pid}, Exit code: {process_status})")
                ...
                try:
                    if self.ollama_process.stdout:
                        ...
                        if output:
                            logger.error(
                                f"[ID:0257A] Ollama process output before crash: {output}")
                except Exception as e:
                    logger.debug(
                        f"[ID:0257B] Could not read Ollama process output: {e}")

                # Check if we can still connect to Ollama
                connection_ok = self.check_ollama_connection()
                logger.info(
                    f"[ID:0258] Ollama connection check result: {connection_ok}")

                if not connection_ok:
                    logger.error(
                        "[ID:0258A] Ollama connection lost, attempting restart")

                    if self.ollama_restart_attempts < self.max_restart_attempts:
                        self.ollama_restart_attempts += 1
                        logger.info(
                            f"[ID:0259] Attempting to restart Ollama (attempt {self.ollama_restart_attempts}/{self.max_restart_attempts})")
                        ...
                        if self._start_ollama_background():
                            logger.info(
                                "[ID:0260] Ollama restarted successfully after crash")
                            self.ollama_restart_attempts = 0  # Reset counter on success
                        else:
                            logger.error(
                                f"[ID:0261] Failed to restart Ollama (attempt {self.ollama_restart_attempts})")
                    else:
                        logger.error(
                            f"[ID:0262] Maximum restart attempts ({self.max_restart_attempts}) reached, stopping auto-restart")
                        self.show_ollama_connection_error(
                            "crash", force_show=True)
                else:
                    logger.info(
                        "[ID:0258B] Ollama connection is still working despite process crash")

        except Exception as e:
            logger.error(f"[ID:0263] Error in Ollama crash detection: {e}")
            logger.error(
                f"[ID:0263A] Crash detection traceback: {traceback.format_exc()}")
    ...
    def set_close_ollama_on_exit(self, enabled: bool):
        ...
        self.service_manager.config_manager.set(
            "close_ollama_on_exit", enabled)
        logger.info(
            f"[ID:CLOSE_ON_EXIT] Set close_ollama_on_exit to: {enabled}")
    ...
    def force_close_ollama(self):
        ...
        logger.info(
            "[ID:FORCE_CLOSE] User requested to force close Ollama app.exe")
        self._stop_ollama_process()
    ...
    def _show_custom_path_dialog(self, context="not_installed"):
        ...
        try:
            ...
            current_path = self.service_manager.config_manager.get(
                "custom_ollama_path", "")
            ...
            if file_path:
                ...
                if self._validate_ollama_executable(file_path):
                    ...
                    self.service_manager.config_manager.set(
                        "custom_ollama_path", file_path)
                    self.service_manager.config_manager.save_config()

                    logger.info(
                        f"[ID:CUSTOM_PATH_SAVED] Custom Ollama path saved: {file_path}")
                    ...
                    success_msg.setText(
                        "Custom Ollama path saved successfully!")
                    ...
                    logger.info(
                        "[ID:CUSTOM_PATH_START] Attempting to start Ollama with custom path")
                    if self._start_ollama_background():
                        logger.info(
                            "[ID:CUSTOM_PATH_SUCCESS] Ollama started successfully with custom path")
                    else:
                        logger.warning(
                            "[ID:CUSTOM_PATH_FAIL] Failed to start Ollama with custom path")
                else:
                    ...
                    error_msg.setIcon(QMessageBox.Warning)
                    error_msg.setText(
                        "The selected file is not a valid Ollama executable.")
                    error_msg.setInformativeText(
                        "Please select the correct 'ollama app.exe' file.")
                    error_msg.exec()
                    # Re-show the error dialog after invalid selection
                    self.show_ollama_connection_error(context, force_show=True)
            else:
                logger.info(
                    "[ID:CUSTOM_PATH_CANCELLED] User cancelled custom path selection")
                # Re-show the error dialog after cancel
                self.show_ollama_connection_error(context, force_show=True)
        except Exception as e:
            logger.error(
                f"[ID:CUSTOM_PATH_ERROR] Error showing custom path dialog: {e}")
            error_msg = QMessageBox(self.main_window)
    ...
    def _validate_ollama_executable(self, file_path: str) -> bool:
        ...
        try:
            ...
            if not os.path.exists(file_path):
                logger.warning(
                    f"[ID:VALIDATE_PATH] File does not exist: {file_path}")
                return False

            # Check if it's an executable
            if not os.access(file_path, os.X_OK):
                logger.warning(
                    f"[ID:VALIDATE_PATH] File is not executable: {file_path}")
                return False
            ...
            if 'ollama' not in filename:
                logger.warning(
                    f"[ID:VALIDATE_PATH] Filename does not contain 'ollama': {filename}")
                return False
            ...
            if file_size < 1000000:  # Less than 1MB
                logger.warning(
                    f"[ID:VALIDATE_PATH] File seems too small for an executable: {file_size} bytes")
                return False

            logger.info(
                f"[ID:VALIDATE_PATH] Valid Ollama executable found: {file_path}")
            return True

        except Exception as e:
            logger.error(
                f"[ID:VALIDATE_PATH] Error validating executable: {e}")
            return False
    ...
    def clear_custom_ollama_path(self):
        ...
        self.service_manager.config_manager.save_config()
        logger.info(
            "[ID:CLEAR_CUSTOM_PATH] Custom Ollama path cleared, reverting to auto-detection")
