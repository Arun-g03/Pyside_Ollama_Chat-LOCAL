...
class StreamingAudioPlayer(QThread):
    ...
    def __init__(self, sample_rate=22050, channels=1, chunk_size=512):
        ...
        logger.debug(
            f"StreamingAudioPlayer initialized with sample_rate={sample_rate}, channels={channels}, chunk_size={chunk_size}")

    def __del__(self):
        """Destructor to ensure cleanup"""
        if not self._cleaned_up:
            logger.debug(
                "StreamingAudioPlayer destructor called - cleaning up")
            self.cleanup()
    ...
    def run(self):
        ...
        try:
                ...
                try:
                    ...
                    try:
                        ...
                        for i in range(device_count):
                            try:
                                device_info = self.pyaudio.get_device_info_by_index(
                                    i)
                                logger.debug(
                                    f"Device {i}: {device_info['name']} (max outputs: {device_info.get('maxOutputChannels', 0)})")
                            except Exception as e:
                                logger.warning(
                                    f"Could not get info for device {i}: {e}")
                    except Exception as e:
                        logger.warning(
                    ...
                    self.playback_error.emit(
                        f"PyAudio initialization failed: {str(e)}")
                    return
            ...
            try:
                ...
                logger.debug(
                    f"Using default output device: {default_device['name']}")
            except Exception as e:
            ...
            try:
                ...
                self.playback_error.emit(
                    f"Audio stream initialization failed: {str(e)}")
                return
                ...
                try:
                    if self.audio_queue:
                        ...
                        if audio_data is not None:  # None signals end of stream
                            logger.debug(
                                f"Processing audio chunk of size {len(audio_data)}")
                            processed_audio = self._process_audio_chunk(
                                audio_data)
                            logger.debug(
                                f"Writing {len(processed_audio.tobytes())} bytes to audio stream")
            ...
            import traceback
            logger.error(
                f"Audio playback error traceback: {traceback.format_exc()}")
            try:
                self.playback_error.emit(str(e))
            except Exception as emit_error:
                logger.error(
                    f"Error emitting playback error signal: {emit_error}")
        finally:
    ...
    def _process_audio_chunk(self, audio_chunk: np.ndarray) -> np.ndarray:
        ...
        try:
            logger.debug(
                f"Processing audio chunk: shape={audio_chunk.shape}, dtype={audio_chunk.dtype}")
            ...
            if audio_chunk.dtype != np.float32:
                try:
                    ...
                    logger.error(
                        f"Error converting audio chunk to float32: {e}")
                    return np.array([], dtype=np.float32)
            ...
            try:
                ...
                eq_bars = calculate_eq_bars(
                    audio_chunk, num_bars=24, sample_rate=self.sample_rate)
            ...
            try:
                ...
                if len(audio_chunk) > 0:
                    try:
                        ...
                        low_freq_energy = np.mean(
                            magnitude[:len(magnitude)//4])
                        mid_freq_energy = np.mean(
                            magnitude[len(magnitude)//4:len(magnitude)//2])
                        high_freq_energy = np.mean(
                            magnitude[len(magnitude)//2:])
                        freq_enhancement = (low_freq_energy * 0.3 + mid_freq_energy * 0.4 +
                                            high_freq_energy * 0.3) / np.max(magnitude) if np.max(magnitude) > 0 else 1.0
                        amplified_level *= (1.0 + freq_enhancement * 0.5)
                    except Exception as e:
                        logger.error(
                            f"Error calculating frequency enhancement: {e}")
                ...
                if self.audio_level_buffer:
                    avg_level = sum(self.audio_level_buffer) / \
                        len(self.audio_level_buffer)
                    ...
                    try:
                        ...
                        logger.debug(
                            f"Emitted audio level: {smoothed_level:.4f} (raw: {amplified_level:.4f})")
                    except Exception as e:
                        logger.error(f"Error emitting audio level: {e}")

                logger.debug(
                    f"Audio chunk processed: max={np.max(audio_chunk)}, min={np.min(audio_chunk)}, level={amplified_level:.4f}")
            ...
            import traceback
            logger.error(
                f"Audio chunk processing error traceback: {traceback.format_exc()}")
    ...
    def add_audio_chunk(self, audio_chunk: np.ndarray):
        ...
        try:
            if self.is_playing:
                logger.debug(
                    f"Adding audio chunk to queue (queue size: {len(self.audio_queue)})")
                self.audio_queue.append(audio_chunk)
            else:
                logger.warning(
                    "Audio player not playing, ignoring audio chunk")
        except Exception as e:
    ...
    def stop_playback(self):
        ...
        try:
            ...
            import traceback
            logger.error(
                f"Stop playback error traceback: {traceback.format_exc()}")
        ...
        try:
            ...
            if self.stream:
                try:
                    ...
                    logger.warning(
                        f"Error closing audio stream during cleanup: {e}")
            ...
            if self.pyaudio:
                try:
                    ...
                    term_thread = threading.Thread(
                        target=terminate_pyaudio, name="terminate_pyaudio")
