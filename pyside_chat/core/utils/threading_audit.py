...
class ThreadSafetyAuditor:
    ...
    def analyze_current_threads(self):
        ...
        try:
            ...
            logger.info(
                f"  Main thread: {len([t for t in active_threads if t.name == 'MainThread'])}")
    ...
    def detect_cross_thread_ui_operations(self):
        ...
        try:
            ...
            logger.info(
                f"Detected {len(self.cross_thread_operations)} potential cross-thread operations")
    ...
    def check_thread_safety_patterns(self):
        ...
        try:
            ...
            main_thread_count = len(
                [t for t in self.thread_info.values() if t['name'] == 'MainThread'])
            ...
            logger.info(
                f"Found {len(self.thread_safety_issues)} thread safety issues")
        ...
        try:
            ...
            if thread_breakdown.get('audio', 0) > 3:
                recommendations.append(
                    "Consider reducing the number of audio threads")

            if thread_breakdown.get('worker', 0) > 10:
                recommendations.append(
                    "Consider using a thread pool for worker threads")

            # Check for signal connection issues
            for issue in self.thread_safety_issues:
                if issue['type'] == 'excessive_signal_receivers':
                    recommendations.append(
                        "Review signal connections to avoid duplicates")
    ...
    def generate_report(self) -> str:
        ...
        if self.cross_thread_operations:
            ...
            for op in self.cross_thread_operations:
                report.append(
                    f"  Thread: {op['thread_name']} ({op['thread_type']})")
...
def quick_thread_check() -> Dict[str, Any]:
    ...
    try:
        ...
        worker_thread_count = len(
            [t for t in active_threads if 'worker' in t.name.lower()])
        voice_thread_count = len(
            [t for t in active_threads if 'voice' in t.name.lower()])
