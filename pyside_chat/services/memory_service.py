...
class ShortTermMemoryService:
    ...
    def _load(self):
        if os.path.exists(self.stm_file):
            try:
                ...
                logger.error(f"[ID:0142] Error loading STM file: {e}", print_to_terminal=True)
                self.messages = []
    ...
    def _save(self):
        try:
            ...
            logger.error(f"[ID:0141] Error saving STM file: {e}", print_to_terminal=True)
...
class LongTermMemoryService:
                ...
                with open(self.ltm_file, 'r', encoding='utf-8') as f:
                    ...
                    for entry_data in data:
                        try:
                            ...
                            logger.error(f"[ID:0140] Error loading LTM entry {entry_data}: {e}", print_to_terminal=True)
                            continue
            except Exception as e:
                logger.error(f"[ID:0139] Error loading LTM file: {e}", print_to_terminal=True)
                self.entries = []
    def add_entry(self, entry: LongTermMemoryEntry):
        logger.debug(f"[ID:0138] === LTM ADD ENTRY START ===", print_to_terminal=True)
        logger.debug(f"[ID:0137] Adding entry: type='{entry.type}', key='{entry.key}', value='{entry.value}'", print_to_terminal=True)
        logger.debug(f"[ID:0136] Current entries count: {len(self.entries)}", print_to_terminal=True)
        
        self.entries.append(entry)
        logger.debug(f"[ID:0135] Entry added to memory, new count: {len(self.entries)}", print_to_terminal=True)
        
        logger.debug("[ID:0134] Saving to disk...", print_to_terminal=True)
        self._save()
        logger.debug("[ID:0133] Save completed", print_to_terminal=True)
        logger.debug(f"[ID:0132] === LTM ADD ENTRY END ===", print_to_terminal=True)
    ...
    def _save(self):
        try:
            logger.debug(f"[ID:0131] Saving {len(self.entries)} entries to {self.ltm_file}", print_to_terminal=True)
            with open(self.ltm_file, 'w', encoding='utf-8') as f:
                data_to_save = [asdict(e) for e in self.entries]
                logger.debug(f"[ID:0130] Data to save: {data_to_save}", print_to_terminal=True)
                json.dump(data_to_save, f, indent=2, ensure_ascii=False)
            logger.debug("[ID:0129] LTM file saved successfully", print_to_terminal=True)
        except Exception as e:
            logger.error(f"[ID:0128] Error saving LTM file: {e}", print_to_terminal=True)
            logger.error(f"[ID:0127] Exception type: {type(e)}", print_to_terminal=True)
...
class MemoryService(QObject):
    ...
    def __init__(self, max_context_messages: int = 16):
        ...
        self.memory_file = os.path.join("pyside_chat", "User_history", "memory", "memories.json")
    ...
    def _load_memory(self):
        ...
        try:
            ...
            logger.error(f"[ID:0126] Error loading memory: {e}", print_to_terminal=True)
    ...
    def _save_memory(self):
        ...
        try:
            ...
            logger.error(f"[ID:0125] Error saving memory: {e}", print_to_terminal=True)
    
    def _on_embeddings_updated(self):
        """Handle embeddings updates from semantic search service"""
        logger.debug("[ID:0124] Semantic search embeddings updated", print_to_terminal=True)
    ...
    def add_memory(self, content: str, conversation_id: str, importance: float = 0.5, 
        ...
        logger.debug(f"[ID:0123] Added memory: {memory_id}")
        return memory_id
    
    def add_summary(self, summary: str, importance: float = 0.5, tags: List[str] = None):
        """Add a conversation summary to long-term memory"""
        logger.debug(f"[ID:0122] === ADD SUMMARY START ===", print_to_terminal=True)
        logger.debug(f"[ID:0121] Adding summary: '{summary[:50]}...'", print_to_terminal=True)
        ...
        try:
            ...
            if self.pronoun_normalizer.should_normalize(summary):
                ...
                logger.debug(f"[ID:0120] Normalized summary pronouns: '{summary[:50]}...' -> '{normalized_summary[:50]}...'", print_to_terminal=True)
            ...
            logger.debug(f"[ID:0119] Summary added to LTM", print_to_terminal=True)
            
            # Emit signal for UI updates
            self.summary_updated.emit(self.ltm.get_entries('summary'))
            
        except Exception as e:
            logger.error(f"[ID:0118] Error adding summary: {e}", print_to_terminal=True)
        finally:
            logger.debug(f"[ID:0117] === ADD SUMMARY END ===", print_to_terminal=True)
    ...
    def get_relevant_memories(self, query: str, limit: int = 10, use_semantic: bool = True) -> List[MemoryEntry]:
        ...
        if use_semantic and self.semantic_search.is_ready():
            logger.debug(f"[ID:0116] Using semantic search for query: '{query}'", print_to_terminal=True)
            ...
            for memory_id, similarity, memory_data in semantic_results:
                ...
                if memory:
                    ...
                    logger.debug(f"[ID:0115] Found relevant memory via semantic search: {memory_id} (similarity: {similarity:.3f})", print_to_terminal=True)
            
            return relevant_memories
        else:
            logger.debug(f"[ID:0114] Using keyword-based search for query: '{query}'", print_to_terminal=True)
    ...
    def intelligent_add_message(self, message: Dict) -> Dict:
        ...
        logger.debug(f"[ID:0113] === INTELLIGENT MESSAGE ADDITION START ===", print_to_terminal=True)
        logger.debug(f"[ID:0112] Processing message: {message}", print_to_terminal=True)
        
        # Always add to STM
        self.stm.add_message(message)
        logger.debug("[ID:0111] Message added to STM", print_to_terminal=True)
        ...
        if role == "user" and content.strip():
            ...
            logger.debug(f"[ID:0110] Message classification: {classification}", print_to_terminal=True)
            
            # If message should be stored in LTM, extract and store facts
            if classification["should_store_ltm"]:
                logger.debug("[ID:0109] Message qualifies for LTM storage", print_to_terminal=True)
                
                # Extract facts from the message
                facts = self.extract_facts_from_message(content)
                logger.debug(f"[ID:0108] Extracted facts: {facts}", print_to_terminal=True)
                ...
                logger.debug("[ID:0107] Message does not qualify for LTM storage", print_to_terminal=True)
        
        logger.debug(f"[ID:0106] === INTELLIGENT MESSAGE ADDITION END ===", print_to_terminal=True)
    ...
    def extract_facts_from_message(self, message: str) -> Dict[str, str]:
        ...
        for pattern in name_patterns:
            ...
            if match:
                ...
                if potential_name.lower() not in invalid_names and len(potential_name) > 1:
                    ...
                    logger.debug(f"[ID:0105] Extracted valid name: {potential_name}", print_to_terminal=True)
                    break
                else:
                    logger.debug(f"[ID:0104] Rejected invalid name: {potential_name}", print_to_terminal=True)
        ...
        for pattern in location_patterns:
            ...
            if match:
                ...
                if len(location) > 3 and not location.lower() in ['here', 'there', 'somewhere', 'anywhere']:
                    ...
                    facts["home"] = location
                    logger.debug(f"[ID:0103] Extracted location: {location}", print_to_terminal=True)
                    break
            ...
            if match:
                if "favorite" in pattern:
                    ...
                    if len(item) > 2 and not item.lower() in ['it', 'this', 'that', 'things', 'stuff']:
                        ...
                        logger.debug(f"[ID:0102] Extracted favorite {category}: {item}", print_to_terminal=True)
                else:
                    ...
                    if len(item) > 2 and not item.lower() in ['it', 'this', 'that', 'things', 'stuff']:
                        ...
                        logger.debug(f"[ID:0101] Extracted preference {action}: {item}", print_to_terminal=True)
        ...
        for key, value in facts.items():
            if self.pronoun_normalizer.should_normalize(value):
                ...
                logger.debug(f"[ID:0100] Normalized fact '{key}': '{value}' -> '{normalized_value}'", print_to_terminal=True)
    ...
    def get_user_info(self) -> Dict[str, str]:
        ...
        logger.debug(f"[ID:0099] Retrieved user info: {user_info}", print_to_terminal=True)
        return user_info
    ...
    def get_context_messages(self, current_query: str = "") -> List[Dict]:
        ...
        logger.debug(f"[ID:0098] === CONTEXT RETRIEVAL START ===", print_to_terminal=True)
        logger.debug(f"[ID:0097] Current query: '{current_query}'", print_to_terminal=True)
        
        # Get STM messages
        stm_msgs = self.stm.get_messages()
        logger.debug(f"[ID:0096] STM messages: {len(stm_msgs)}", print_to_terminal=True)
        
        # Get relevant LTM entries if we have a query
        relevant_ltm = []
        if current_query.strip():
            logger.debug("[ID:0095] Retrieving relevant LTM entries...", print_to_terminal=True)
            ...
            logger.debug(f"[ID:0094] Found {len(relevant_ltm)} relevant LTM entries", print_to_terminal=True)
            
            # Update access stats for retrieved memories
            for memory, relevance in relevant_ltm:
                self.ltm.update_access_stats(memory)
                logger.debug(f"[ID:0093] Retrieved memory: {memory.key} = {memory.value} (relevance: {relevance:.2f})", print_to_terminal=True)
        ...
        logger.debug(f"[ID:0092] Context messages: {context}", print_to_terminal=False)
        logger.debug(f"[ID:0091] === CONTEXT RETRIEVAL END ===", print_to_terminal=True)
        return context
        ...
        if memory_type:
            ...
            try:
                ...
                if os.path.exists(self.stm.stm_file):
                    ...
                    logger.debug(f"[ID:0090] Cleared short-term memory file: {self.stm.stm_file}", print_to_terminal=True)
                
                # Clear long-term memory file
                if os.path.exists(self.ltm.ltm_file):
                    with open(self.ltm.ltm_file, 'w', encoding='utf-8') as f:
                        json.dump([], f, indent=2, ensure_ascii=False)
                    logger.debug(f"[ID:0089] Cleared long-term memory file: {self.ltm.ltm_file}", print_to_terminal=True)
                ...
                if os.path.exists(self.memory_file):
                    with open(self.memory_file, 'w', encoding='utf-8') as f:
                        json.dump([], f, indent=2, ensure_ascii=False)
                    logger.debug(f"[ID:0088] Cleared memories file: {self.memory_file}", print_to_terminal=True)
                
                logger.debug("[ID:0087] All memory files cleared successfully", print_to_terminal=True)
                
                # Verify files are empty
                self._verify_memory_files_cleared()
                
            except Exception as e:
                logger.error(f"[ID:0086] Error clearing memory files: {e}", print_to_terminal=True)
        
        self._save_memory()
        self.memory_updated.emit(self.memories)
        logger.debug(f"[ID:0085] Memory cleared: type={memory_type if memory_type else 'all'}", print_to_terminal=True)
        ...
        try:
            ...
            if os.path.exists(self.stm.stm_file):
                with open(self.stm.stm_file, 'r', encoding='utf-8') as f:
                    ...
                    if stm_data:
                        logger.warning(f"[ID:0084] Short-term memory file not empty after clearing: {len(stm_data)} entries", print_to_terminal=True)
                    else:
                        logger.debug("[ID:0083] Short-term memory file verified as empty", print_to_terminal=True)
            ...
            if os.path.exists(self.ltm.ltm_file):
                with open(self.ltm.ltm_file, 'r', encoding='utf-8') as f:
                    ...
                    if ltm_data:
                        logger.warning(f"[ID:0082] Long-term memory file not empty after clearing: {len(ltm_data)} entries", print_to_terminal=True)
                    else:
                        logger.debug("[ID:0081] Long-term memory file verified as empty", print_to_terminal=True)
            ...
            if os.path.exists(self.memory_file):
                with open(self.memory_file, 'r', encoding='utf-8') as f:
                    ...
                    if memories_data:
                        logger.warning(f"[ID:0080] Memories file not empty after clearing: {len(memories_data)} entries", print_to_terminal=True)
                    else:
                        logger.debug("[ID:0079] Memories file verified as empty", print_to_terminal=True)
                        
        except Exception as e:
            logger.error(f"[ID:0078] Error verifying memory files: {e}", print_to_terminal=True)
    ...
    def add_fact(self, key: str, value: str, importance: float = 0.7, tags: List[str] = None):
        ...
        logger.debug(f"[ID:0077] === MEMORY SERVICE ADD FACT START ===", print_to_terminal=True)
        logger.debug(f"[ID:0076] Adding fact: key='{key}', value='{value}', importance={importance}", print_to_terminal=True)
        
        try:
            # Validate inputs
            if not key or not value:
                logger.debug(f"[ID:0075] Invalid fact data: key='{key}', value='{value}'", print_to_terminal=True)
                return
                
            if not isinstance(key, str) or not isinstance(value, str):
                logger.debug(f"[ID:0074] Fact key and value must be strings: key={type(key)}, value={type(value)}", print_to_terminal=True)
                return
            
            # Clean the inputs
            clean_key = key.strip()
            clean_value = value.strip()
            logger.debug(f"[ID:0073] Cleaned inputs: key='{clean_key}', value='{clean_value}'", print_to_terminal=True)
            ...
            if self.pronoun_normalizer.should_normalize(clean_value):
                ...
                logger.debug(f"[ID:0072] Normalized pronouns: '{clean_value}' -> '{normalized_value}'", print_to_terminal=True)
                clean_value = normalized_value
            
            if not clean_key or not clean_value:
                logger.debug("[ID:0071] Fact key or value is empty after cleaning", print_to_terminal=True)
                return
            
            # Check if this fact already exists to avoid duplicates
            logger.debug("[ID:0070] Checking for existing facts to avoid duplicates...", print_to_terminal=True)
            existing_facts = self.ltm.get_entries('fact')
            logger.debug(f"[ID:0069] Found {len(existing_facts)} existing facts", print_to_terminal=True)
            
            for existing in existing_facts:
                if existing.key == clean_key and existing.value == clean_value:
                    logger.debug(f"[ID:0068] Fact already exists: {clean_key} = {clean_value}", print_to_terminal=True)
                    return
            
            logger.debug("[ID:0067] No duplicate found, creating new fact entry", print_to_terminal=True)
            entry = LongTermMemoryEntry(
                type='fact', 
                key=clean_key, 
                value=clean_value, 
            ...
            logger.debug(f"[ID:0066] Created entry: {entry}", print_to_terminal=True)
            
            self.ltm.add_entry(entry)
            logger.debug(f"[ID:0065] Added fact to LTM: {clean_key} = {clean_value}", print_to_terminal=True)
            logger.debug(f"[ID:0064] Total LTM entries: {len(self.ltm.entries)}", print_to_terminal=True)
            
            # Clean up memory entries to resolve conflicts
            self.cleanup_memory_entries()
            
        except Exception as e:
            logger.error(f"[ID:0063] Error adding fact: {e}", print_to_terminal=True)
            logger.error(f"[ID:0062] Exception type: {type(e)}", print_to_terminal=True)
        finally:
            logger.debug(f"[ID:0061] === MEMORY SERVICE ADD FACT END ===", print_to_terminal=True)
    ...
    def set_max_context_messages(self, max_messages: int):
        ...
        self.stm._save()
        logger.debug(f"[ID:0060] Set max context messages to {max_messages}", print_to_terminal=True)
    ...
    def cleanup_memory_entries(self):
        ...
        logger.debug("[ID:0059] === MEMORY CLEANUP START ===", print_to_terminal=True)
        ...
        for key, entries in entries_by_key.items():
            if len(entries) == 1:
                ...
                logger.debug(f"[ID:0058] Found {len(entries)} entries for key '{key}': {[e.value for e in entries]}", print_to_terminal=True)
                ...
                if key in ['name', 'user_name']:
                    ...
                    if valid_names:
                        ...
                        logger.debug(f"[ID:0057] Selected best name: {best_name.value} (timestamp: {best_name.timestamp})", print_to_terminal=True)
                        cleaned_entries.append(best_name)
                    else:
                        # No valid names found, keep the most recent
                        entries.sort(key=lambda x: x.timestamp, reverse=True)
                        cleaned_entries.append(entries[0])
                        logger.debug(f"[ID:0056] No valid names found, keeping most recent: {entries[0].value}", print_to_terminal=True)
                else:
                    # For other keys, keep the most recent entry
                    entries.sort(key=lambda x: x.timestamp, reverse=True)
                    cleaned_entries.append(entries[0])
                    logger.debug(f"[ID:0055] Keeping most recent entry for '{key}': {entries[0].value}", print_to_terminal=True)
        ...
        logger.debug(f"[ID:0054] Memory cleanup complete: {old_count} -> {new_count} entries", print_to_terminal=True)
        logger.debug("[ID:0053] === MEMORY CLEANUP END ===", print_to_terminal=True)
