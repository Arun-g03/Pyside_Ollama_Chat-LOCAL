...
def check_dependencies(auto_install=True):
    ...
    try:
        ...
        logger.error(f"[ID:0374] Error checking dependencies: {str(e)}")
        logger.info(f"[ID:0373] ❌ Error checking dependencies: {str(e)}")
        return False
...
def main():
    try:
        ...
        if not args.skip_deps:
            logger.info("[ID:0372] 🔍 Checking dependencies...", print_to_terminal=True)
            if not check_dependencies(auto_install=not args.no_auto_install):
                logger.error("[ID:0371] ❌ Cannot start application due to missing dependencies.")
                sys.exit(1)
        else:
            logger.error("[ID:0370] ⚠️  Skipping dependency check (--skip-deps flag used)")
        
        logger.info("[ID:0369] 🚀 Starting QApplication...",print_to_terminal=True)
        app = QApplication(sys.argv)
        logger.info("[ID:0368] Application started",print_to_terminal=True)
        # Set application properties
        app.setApplicationName("Ollama Chat - Local LLM Chat Application")
        app.setApplicationVersion("0.4.0")
        
        logger.info("[ID:0367] 🏗️  Creating main window...",print_to_terminal=True)
        # Create and show the main window
        window = OllamaChat()
        logger.info("[ID:0366] 📱 Showing main window...",print_to_terminal=True)
        window.show()
        
        # Connect the application's aboutToQuit signal to ensure cleanup
        app.aboutToQuit.connect(window.close)
        
        logger.info("[ID:0365] 🔄 Starting application event loop...",print_to_terminal=True)
        # Start the application event loop
        result = app.exec()
        logger.info(f"[ID:0364] 👋 QApplication event loop exited with code: {result}",print_to_terminal=True)
        return result
        
    except Exception as e:
        logger.error(f"[ID:0363] Error in main: {str(e)}",print_to_terminal=True)
        raise
...
if __name__ == "__main__":
    try:
        ...
        logger.error(f"[ID:0362] Fatal error: {str(e)}",print_to_terminal=True)
        sys.exit(1)
