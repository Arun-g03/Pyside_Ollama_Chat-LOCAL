...
class ComplexityWidget(QWidget):
    ...
    def analyze_request(self, request: str, conversation_history=None, available_models=None):
        ...
        # Perform analysis
        self.current_metrics = self.analyzer.analyze_complexity(
        ...
        if available_models:
            recommended_model = self.analyzer.get_model_recommendation(
                self.current_metrics, available_models)
    ...
    def _update_display(self):
        ...
        color = color_map.get(self.current_metrics.level,
                              QColor(128, 128, 128))
        ...
        for metric_key, (bar, label) in self.metric_bars.items():
            if hasattr(self.current_metrics, metric_key):
                ...
                if metric_key in ['reasoning_depth', 'knowledge_breadth', 'output_complexity']:
                    ...
                    metric_color = self._get_color_for_value(
                        value, reverse=False)
                else:
        ...
        recommendations_text = "\n".join(
            f"• {rec}" for rec in self.current_metrics.recommendations)
