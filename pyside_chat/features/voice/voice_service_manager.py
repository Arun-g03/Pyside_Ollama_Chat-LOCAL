...
class VoiceServiceManager(QObject):
    ...
    # Emitted when voice service fails to initialize
    voice_service_error = Signal(str)
    ...
    def _initialize_voice_service(self):
        ...
        logger.info("Starting voice service initialization",
                    print_to_terminal=True)

        try:
            # Always use direct initialization to avoid wrapper signal duplication issues
            logger.info(
                "Using direct voice service initialization to avoid signal duplication", print_to_terminal=True)
            ...
            if voice_service:
                ...
                logger.info(
                    f"Voice service type: {service_type}", print_to_terminal=True)
                ...
                if self._cached_settings:
                    try:
                        ...
                        logger.debug(
                            f"Applied cached settings to voice service")
                ...
                if hasattr(voice_service, 'is_voice_available') and voice_service.is_voice_available():
                    logger.info(
                        "Voice service is ready, emitting ready signal", print_to_terminal=True)
                    self.voice_service_ready.emit()
                else:
                    logger.info(
                        "Voice service initialized but not ready yet, waiting for ready signal", print_to_terminal=True)
                ...
                for callback in self._ready_callbacks:
                    try:
                        ...
                        logger.error(
                            f"Error in voice service ready callback: {e}")
                self._ready_callbacks.clear()

            else:
                raise Exception(
                    "Failed to initialize voice service - no service available")

        except Exception as e:
            self._initialization_error = str(e)
            logger.error(
                f"Voice service initialization failed: {e}", print_to_terminal=True)
    ...
    def _on_voice_service_ready(self):
        ...
        logger.info("Voice service ready signal received from service",
                    print_to_terminal=True)
        self.voice_service_ready.emit()
    ...
    def _try_get_from_service_manager(self):
        ...
        try:
            ...
            logger.debug(
                f"Could not get voice service from service manager: {e}")
    ...
    def _try_direct_initialization(self):
        ...
        try:
            ...
            logger.info(
                "Trying direct VoiceService initialization (skipping wrapper)", print_to_terminal=True)
            from pyside_chat.features.voice.voice_service import VoiceService
            voice_service = VoiceService.get_instance()
            logger.info(
                "Direct voice service initialized successfully", print_to_terminal=True)
            return voice_service
        except Exception as e2:
            logger.error(
                f"Direct voice service initialization failed: {e2}", print_to_terminal=True)
            return None
    ...
    def is_ready(self) -> bool:
        ...
        logger.debug(
            f"Voice service ready check: initialized={self._voice_service_initialized}, service={self._voice_service is not None}, ready={ready}")
        return ready
