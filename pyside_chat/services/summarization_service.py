...
class SummarizationService(QObject):
    ...
    def generate_chat_name(self, conversation: List[Dict], filepath: str) -> None:
        ...
        """
        try:
            logger.info(f"[ID:0187] Starting name generation for conversation: {filepath}")
            logger.info(f"[ID:0186] Conversation has {len(conversation)} messages, minimum required: {self.min_messages_for_summarization}")
            
            # Check if we have enough messages to summarize
            if len(conversation) < self.min_messages_for_summarization:
                logger.debug(f"[ID:0185] Not enough messages ({len(conversation)}) for summarization")
                return
            
            # Extract user messages for summarization
            user_messages = [msg.get('content', '') for msg in conversation if msg.get('role') == 'user']
            if not user_messages:
                logger.debug("[ID:0184] No user messages found for summarization")
                return
            
            # Check if the conversation has enough substance for meaningful naming
            if not self._has_enough_substance(user_messages):
                logger.debug("[ID:0183] Conversation lacks sufficient substance for meaningful naming")
                return
            
            logger.info(f"[ID:0182] Found {len(user_messages)} user messages for summarization")
            logger.info(f"[ID:0181] User messages: {user_messages}")
            
            # Create summarization prompt
            prompt = self._create_summarization_prompt(user_messages)
            logger.info(f"[ID:0180] Summarization prompt: {prompt}")
            ...
            if self.summarization_model == "Auto":
                ...
                if not available_models:
                    logger.warning("[ID:0179] No models available for summarization")
                ...
                logger.info(f"[ID:0178] Auto-selected model for summarization: {chosen_model}")
            else:
                chosen_model = self.summarization_model
            
            logger.info(f"[ID:0177] Sending summarization request to model: {chosen_model}")
            logger.info(f"[ID:0176] Request messages: {messages}")
            ...
            logger.info(f"[ID:0175] Raw AI response: '{response}'")
            
            # Clean and validate the response
            generated_name = self._clean_generated_name(response)
            logger.info(f"[ID:0174] Cleaned generated name: '{generated_name}'")
            
            if generated_name:
                logger.info(f"[ID:0173] Generated chat name: {generated_name}")
                self.summarization_completed.emit(filepath, generated_name)
            else:
                logger.warning("[ID:0172] Failed to generate valid chat name")
                logger.warning(f"[ID:0171] Raw response was: '{response}'")
                self.summarization_failed.emit(filepath, "Failed to generate valid name")
                
        except Exception as e:
            logger.error(f"[ID:0170] Error generating chat name: {str(e)}")
...
Generate a concise, meaningful name that accurately describes what this conversation is about. If the conversation lacks substance or clear topic, respond with "NO_TOPIC". Return only the name or "NO_TOPIC", nothing else."""
        ...
        logger.debug(f"[ID:0169] Created summarization prompt with {len(sample_messages)} sample messages")
        logger.debug(f"[ID:0168] Sample messages: {sample_messages}")
        
        return prompt
    
    def _clean_generated_name(self, response: str) -> Optional[str]:
        """Clean and validate the generated name"""
        logger.debug(f"[ID:0167] Cleaning response: '{response}'")
        
        if not response:
            logger.debug("[ID:0166] Response is empty")
            return None
        
        # Clean the response
        name = response.strip()
        logger.debug(f"[ID:0165] After strip: '{name}'")
        
        # Remove <think> tags and their content
        import re
        name = re.sub(r'<think>.*?</think>', '', name, flags=re.DOTALL)
        name = name.strip()
        logger.debug(f"[ID:0164] After removing <think> tags: '{name}'")
        ...
        for prefix in prefixes_to_remove:
            if name.lower().startswith(prefix.lower()):
                ...
                logger.debug(f"[ID:0163] Removed prefix '{prefix}', result: '{name}'")
        
        # Remove quotes if present
        name = name.strip('"\'')
        logger.debug(f"[ID:0162] After quote removal: '{name}'")
        
        # Check for NO_TOPIC response
        if name.upper() == "NO_TOPIC":
            logger.debug("[ID:0161] AI indicated no clear topic found")
            return None
        
        # Validate the name length
        if len(name) < 3:
            logger.debug(f"[ID:0160] Name too short ({len(name)} chars): '{name}'")
            return None
        
        if len(name) > 50:
            logger.debug(f"[ID:0159] Name too long ({len(name)} chars): '{name}'")
            return None
        ...
        for generic in generic_responses:
            if generic in name_lower:
                logger.debug(f"[ID:0158] Name contains generic term '{generic}': '{name}'")
                return None
        
        # Use AI to evaluate name quality instead of hardcoded patterns
        if not self._ai_evaluate_name_quality(name):
            logger.debug(f"[ID:0157] AI determined name '{name}' is not suitable")
            return None
        
        logger.debug(f"[ID:0156] Final cleaned name: '{name}'")
        return name
...
Respond with only "YES" if the conversation has enough substance for naming, or "NO" if it lacks substance (e.g., mostly greetings, random thoughts, or no clear topic)."""
            ...
            if self.summarization_model == "Auto":
                ...
                available_models = self.ollama_service.get_models()
                if not available_models:
                    logger.warning("[ID:0155] No models available for quality evaluation")
                    return False
                ...
                logger.debug(f"[ID:0154] Auto-selected model for quality evaluation: {chosen_model}")
            else:
                chosen_model = self.summarization_model
            ...
            logger.debug(f"[ID:0153] Quality evaluation response: '{response}'")
            
            # Check for positive evaluation
            if "YES" in response:
                logger.debug("[ID:0152] AI determined conversation has sufficient substance")
                return True
            else:
                logger.debug("[ID:0151] AI determined conversation lacks sufficient substance")
                return False
                
        except Exception as e:
            logger.error(f"[ID:0150] Error evaluating conversation quality: {str(e)}")
    ...
    def _fallback_quality_check(self, user_messages: List[str]) -> bool:
        ...
        logger.debug(f"[ID:0149] Fallback check: {substantial_messages} substantial messages out of {len(user_messages)} total")
...
Respond with only "YES" if the name is suitable, or "NO" if it's not suitable (e.g., too generic, random, or inappropriate)."""
            ...
            if self.summarization_model == "Auto":
                ...
                available_models = self.ollama_service.get_models()
                if not available_models:
                    logger.warning("[ID:0148] No models available for name quality evaluation")
                    return False
                ...
                logger.debug(f"[ID:0147] Auto-selected model for name quality evaluation: {chosen_model}")
            else:
                chosen_model = self.summarization_model
            ...
            # Clean and evaluate response
            response = response.strip().upper()
            logger.debug(f"[ID:0146] Name quality evaluation response: '{response}'")
            
            # Check for positive evaluation
            if "YES" in response:
                logger.debug("[ID:0145] AI determined name is suitable")
                return True
            else:
                logger.debug("[ID:0144] AI determined name is not suitable")
                return False
                
        except Exception as e:
            logger.error(f"[ID:0143] Error evaluating name quality: {str(e)}")
