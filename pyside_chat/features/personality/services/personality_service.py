...
class PersonalityService:
    ...
    def create_custom_personality(self, name: str, traits: PersonalityTraits, prompt: PersonalityPrompt,
        ...
        try:
            ...
            if name in self.personalities and self.is_system_personality(name):
                logger.debug(
                    f"Cannot create custom personality '{name}' - name conflicts with system personality", print_to_terminal=True)
                return False

            # Create personality data
            personality_data = self.loader.create_personality_data(
                traits, prompt, config, metadata)

            # Ensure custom personalities are saved to the Custom folder
            custom_name = f"Custom.{name}" if not name.startswith(
                "Custom.") else name
            ...
            if self.loader.save_personality_to_file(custom_name, personality_data):
                ...
                filepath = self.loader.find_personality_file_by_name(
                    custom_name)
                if filepath:
            ...
            logger.debug(
                f"Error creating personality {name}: {e}", print_to_terminal=True)
            return False
    ...
    def delete_custom_personality(self, name: str) -> bool:
        ...
        if self.is_system_personality(name):
            logger.debug(
                f"Cannot delete system personality '{name}'", print_to_terminal=True)
            return False
        ...
        try:
            ...
            if self.loader.delete_personality_file(name):
                ...
                if self.current_personality == name:
                    if self.personalities:
                        self.current_personality = list(
                            self.personalities.keys())[0]
                    else:
            ...
            logger.debug(
                f"Error deleting personality {name}: {e}", print_to_terminal=True)
            return False
    ...
    def update_custom_personality(self, name: str, traits: PersonalityTraits, prompt: PersonalityPrompt,
        ...
        if self.is_system_personality(name):
            logger.debug(
                f"Cannot update system personality '{name}'", print_to_terminal=True)
            return False
        ...
        try:
            ...
            logger.debug(
                f"Error updating personality {name}: {e}", print_to_terminal=True)
            return False
    ...
    def refresh_personalities(self) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Error refreshing personalities: {e}", print_to_terminal=True)
            return False
    ...
    def build_comprehensive_system_prompt(self, memory_service=None) -> str:
        ...
        base_prompt = self.formatter.build_comprehensive_system_prompt(
            current_personality)
        ...
        if memory_service:
            try:
                ...
                if user_info:
                    ...
                    # Add preferences
                    preferences = {k: v for k, v in user_info.items() if k.startswith(
                        'favorite_') or k.startswith('preference_')}
                    if preferences:
                    ...
                    logger.debug(
                        f"Generated user context messages: {user_info}", print_to_terminal=True)
                    logger.debug(
                        f"New conversation: {is_new_conversation}", print_to_terminal=True)
                    logger.debug(
                        f"User context content: {context_text.strip()}", print_to_terminal=True)

            except Exception as e:
                logger.debug(
                    f"Error generating user context: {e}", print_to_terminal=True)
