...
class MessageFormatter:
    ...
    def detect_code_in_message(message: str) -> bool:
        ...
        code_patterns = [
            ...
            # JavaScript
            r'\b(var|let|const|function|if|else|for|while|try|catch|finally)\b',
            # Java
    ...
    def detect_code_type(message: str):
        ...
        """
        try:
            # Automatically detect the lexer based on code content
            lexer = guess_lexer(message)
            return lexer
        except Exception as e:
            logger.debug(
                f"Error detecting code type: {e}", print_to_terminal=True)
            return None
    ...
    def syntax_highlight_code(message: str, language: str = None) -> str:
        ...
        try:
            ...
            logger.debug(
                f"Error highlighting code: {e}", print_to_terminal=True)
    ...
    def detect_and_format_code(message: str) -> str:
        ...
        def format_block_code(match):
            ...
            highlighted_code = MessageFormatter.syntax_highlight_code(
        ...
        formatted_message = re.sub(
            block_code_pattern, format_block_code, formatted_message)
        ...
        formatted_message = re.sub(
            inline_code_pattern, lambda m: f'<code style="background-color: #2d2d2d; color: #dcdcaa; padding: 2px 4px; border-radius: 3px; font-family: \'Consolas\', \'Monaco\', \'Courier New\', monospace; font-size: 12px;">{m.group(1)}</code>', formatted_message)
    ...
    def _protect_code_blocks(message: str):
        ...
        protected_message = re.sub(
            r'<code[^>]*>.*?</code>', protect_code_blocks, message, flags=re.DOTALL)
        protected_message = re.sub(
            r'<pre[^>]*>.*?</pre>', protect_code_blocks, protected_message, flags=re.DOTALL)
        protected_message = re.sub(r'<div[^>]*style="background-color: #2d2d2d[^>]*>.*?</div>',
                                   protect_code_blocks, protected_message, flags=re.DOTALL)
    ...
    def format_markdown(message: str) -> str:
        ...
        protected_message, code_blocks = MessageFormatter._protect_code_blocks(
            message)
        ...
        protected_message = re.sub(
            r'---\s*---\s*---', '---', protected_message)
        protected_message = re.sub(r'---\s*###', '###', protected_message)

        # Format headers with consistent styling using local colors
        protected_message = re.sub(
            r'^###\s+(.+)$', f'<h3 style="font-size: 18px; font-weight: bold; color: {MessageFormatter.COLORS["header_secondary"]}; margin: 20px 0 10px 0; padding: 8px 0; border-bottom: 1px solid {MessageFormatter.COLORS["border"]};">\\1</h3>', protected_message, flags=re.MULTILINE)
        protected_message = re.sub(
            r'^##\s+(.+)$', f'<h2 style="font-size: 20px; font-weight: bold; color: {MessageFormatter.COLORS["header_primary"]}; margin: 25px 0 15px 0; padding: 10px 0; border-bottom: 2px solid {MessageFormatter.COLORS["border_light"]};">\\1</h2>', protected_message, flags=re.MULTILINE)
        protected_message = re.sub(
            r'^#\s+(.+)$', f'<h1 style="font-size: 24px; font-weight: bold; color: {MessageFormatter.COLORS["header_primary"]}; margin: 30px 0 20px 0; padding: 12px 0; border-bottom: 3px solid {MessageFormatter.COLORS["border_dark"]};">\\1</h1>', protected_message, flags=re.MULTILINE)
        ...
        protected_message = re.sub(
            r'`([^`]+)`', lambda m: f'<code style="background-color: {MessageFormatter.COLORS["inline_code_bg"]}; color: {MessageFormatter.COLORS["inline_code_text"]}; padding: 2px 4px; border-radius: 3px; font-family: \'Consolas\', \'Monaco\', \'Courier New\', monospace; font-size: 12px;">{m.group(1)}</code>', protected_message)

        # Add paragraph spacing for better readability
        protected_message = re.sub(
            r'\n\n+', '</p><p style="margin: 12px 0; line-height: 1.6;">', protected_message)
        protected_message = '<p style="margin: 12px 0; line-height: 1.6;">' + \
            protected_message + '</p>'
    ...
    def handle_html_tags(message: str) -> str:
        ...
        protected_message = re.sub(
            r'<ul>(.*?)</ul>', f'<ul style="list-style-type: disc; padding-left: 20px; margin: 10px 0;">\\1</ul>', protected_message, flags=re.DOTALL)
        protected_message = re.sub(
            r'<ol>(.*?)</ol>', f'<ol style="padding-left: 20px; margin: 10px 0;">\\1</ol>', protected_message, flags=re.DOTALL)
        protected_message = re.sub(
            r'<li>(.*?)</li>', f'<li style="margin: 5px 0;">\\1</li>', protected_message, flags=re.DOTALL)
        protected_message = re.sub(
            r'<b>(.*?)</b>', f'<b style="font-weight: bold; color: {MessageFormatter.COLORS["header_primary"]};">\\1</b>', protected_message, flags=re.DOTALL)
        protected_message = re.sub(
            r'<i>(.*?)</i>', f'<i style="font-style: italic; color: {MessageFormatter.COLORS["italic_text"]};">\\1</i>', protected_message, flags=re.DOTALL)
        protected_message = re.sub(
            r'<h1>(.*?)</h1>', f'<h1 style="font-size: 24px; font-weight: bold; color: {MessageFormatter.COLORS["header_primary"]}; margin: 15px 0 10px 0; border-bottom: 2px solid {MessageFormatter.COLORS["border_light"]}; padding-bottom: 5px;">\\1</h1>', protected_message, flags=re.DOTALL)
        protected_message = re.sub(
            r'<h2>(.*?)</h2>', f'<h2 style="font-size: 20px; font-weight: bold; color: {MessageFormatter.COLORS["header_primary"]}; margin: 12px 0 8px 0;">\\1</h2>', protected_message, flags=re.DOTALL)
        protected_message = re.sub(
            r'<h3>(.*?)</h3>', f'<h3 style="font-size: 16px; font-weight: bold; color: {MessageFormatter.COLORS["header_primary"]}; margin: 10px 0 6px 0;">\\1</h3>', protected_message, flags=re.DOTALL)
        protected_message = re.sub(
            r'<p>(.*?)</p>', f'<p style="margin: 8px 0;">\\1</p>', protected_message, flags=re.DOTALL)
    ...
    def format_chat_message(sender: str, message: str, is_code: bool = False) -> str:
        ...
        cleaned_message = MessageFormatter.cleanup_message(
            sender, message, is_code)
