...
class ChatTab(QWidget):
    ...
    # Emitted when a conversation is renamed (old_path, new_path)
    conversation_renamed = Signal(str, str)
    # Emitted when new conversation is requested
    ...
    def __init__(self, parent=None, conversation_manager=None, summarization_service=None, config_manager=None):
        ...
        # Track last system message widget (if using custom widgets)
        self.last_system_message_widget = None
        self.voice_mode = False  # Track voice mode state

        # Voice controls state tracking
        # Track if voice control signals are connected
    ...
    def setup_connections(self):
        ...
        try:
            ...
            logger.debug(
                "[VOICE DEBUG] Signal connections setup completed successfully")

        except Exception as e:
            logger.error(
                f"[VOICE ERROR] Error setting up signal connections: {e}")
    ...
    def _on_conversation_updated(self, conversation):
        ...
        logger.debug(
            f"[STATE_SYNC] _on_conversation_updated called. Conversation has {len(conversation)} messages.")
        for i, msg in enumerate(conversation):
            logger.debug(
                f"[STATE_SYNC] Message {i}: id={msg.get('id')}, role={msg.get('role')}, is_streaming={msg.get('is_streaming')}, content_len={len(msg.get('content', ''))}")
        prev_streaming = getattr(self, 'is_streaming', False)
        self.is_streaming = any(msg.get('is_streaming')
        ...
        input_components = self.input_controls.get_ui_components()
        logger.debug(
            f"[STATE_SYNC] send_button id: {id(input_components['send_button'])}, cancel_button id: {id(input_components['cancel_button'])}")
        logger.debug(
            f"[STATE_SYNC] InputControls send_button id: {id(self.input_controls.send_button)}, cancel_button id: {id(self.input_controls.cancel_button)}")
        if not self.voice_mode:
            ...
            logger.debug(
                f"[STATE_SYNC] send_button enabled: {input_components['send_button'].isEnabled()}, cancel_button visible: {input_components['cancel_button'].isVisible()}")
    ...
    def on_message_sent(self, message: str):
        ...
        import traceback
        logger.debug(
            f"[VOICE DEBUG] on_message_sent called with message: '{message}', voice_mode: {self.voice_mode}")
        logger.debug(
            f"[DUPLICATE_DEBUG] on_message_sent call stack: {traceback.format_stack()[-3:]}")
    ...
    def _ensure_voice_controls_initialized(self):
        ...
        if not self.voice_controls_initialized:
            try:
                ...
                logger.debug("Initializing voice controls",
                ...
                if voice_controls_layout:
                    ...
                    voice_controls_layout.addWidget(
                        voice_components['voice_button'])
                    voice_controls_layout.addWidget(
                        voice_components['voice_settings_button'])
                    voice_controls_layout.addWidget(
                        voice_components['audio_level_widget'])
                    voice_controls_layout.addStretch()  # Add stretch to push controls to the left

                    logger.debug(
                        "Voice control components added to existing layout")
                else:
                ...
                if not self._voice_controls_signals_connected:
                    ...
                    logger.debug(
                        "Connecting voice control signals to chat tab", print_to_terminal=True)

                    # Define signal-slot pairs with names for logging
                    signal_slot_pairs = [
                        (self.voice_controls.voice_input_received,
                         self.on_voice_input_received, "voice_input_received"),
                        (self.voice_controls.voice_input_error,
                         self.on_voice_input_error, "voice_input_error"),
                        (self.voice_controls.tts_started,
                         self.on_tts_started, "tts_started"),
                        (self.voice_controls.tts_finished,
                         self.on_tts_finished, "tts_finished"),
                        (self.voice_controls.tts_error,
                         self.on_tts_error, "tts_error"),
                        (self.voice_controls.recording_started,
                         self.on_recording_started, "recording_started"),
                        (self.voice_controls.recording_stopped,
                         self.on_recording_stopped, "recording_stopped"),
                        (self.voice_controls.recording_error,
                         self.on_recording_error, "recording_error"),
                        (self.voice_controls.voice_processing_started,
                         self.on_voice_processing_started, "voice_processing_started"),
                        (self.voice_controls.voice_processing_finished,
                         self.on_voice_processing_finished, "voice_processing_finished"),
                        (self.voice_controls.audio_level_changed,
                         self.on_audio_level_changed, "audio_level_changed"),
                        (self.voice_controls.eq_bars_changed,
                         self.on_eq_bars_changed, "eq_bars_changed"),
                        (self.voice_controls.user_interrupted,
                         self.on_user_interrupted, "user_interrupted"),
                        (self.voice_controls.request_cancelled,
                    ...
                    for signal, slot, signal_name in signal_slot_pairs:
                        try:
                            ...
                            logger.debug(
                                f"Connected signal {signal_name} to {slot.__name__}")
                        except Exception as e:
                            logger.error(
                                f"Failed to connect signal {signal_name} to {slot.__name__}: {e}")
                ...
                try:
                    # Connect signal directly (PySide6 handles duplicates gracefully)
                    voice_components['voice_settings_button'].clicked.connect(
                        self.open_voice_settings)
                    logger.debug("Connected voice settings button")
                except Exception as e:
                    logger.error(
                        f"Failed to connect voice settings button: {e}")
                ...
                logger.error(
                    f"Failed to initialize voice controls: {e}", print_to_terminal=True)
                self.voice_controls = None
                self.voice_controls_initialized = False
        else:
            print(f"[DEBUG] Voice controls already initialized")
            logger.debug("Voice controls already initialized",
                         print_to_terminal=True)
    ...
    def on_input_mode_changed(self, mode: str):
        ...
        logger.debug(
            f"[VOICE DEBUG] on_input_mode_changed called with mode: {mode}")
        ...
        if mode == "Chat":
            ...
            if self.voice_controls_initialized and self.voice_controls:
                ...
                if not self.voice_controls.is_voice_service_ready():
                    logger.info(
                        "Voice service not ready, starting initialization")
                ...
                if eq_mode != "None":
                    ...
                    self.eq_visualizer.switch_to_eq_visualizer(
                        self.chat_display.scroll_area, self.voice_mode)
                    logger.debug(
                        f"[VOICE DEBUG] Switched to Voice mode with EQ: {eq_mode}")
                else:
                    logger.debug(
            ...
            if hasattr(self.parent, 'get_service_manager'):
                ...
                if hasattr(service_manager, 'get_personality_service'):
                    ...
                    if personality_service:
                        success = personality_service.set_current_personality(
                            personality_name)
                        if success:
                            logger.debug(
                                f"Successfully set personality service to: {personality_name}")
                        else:
                            logger.warning(
                                f"Failed to set personality service to: {personality_name}")
        except Exception as e:
    ...
    def on_voice_input_received(self, text: str):
        try:
            ...
            if self.is_streaming:
                logger.debug(
                    "Stopping current streaming to process voice input")
                self.stop_streaming()
    ...
    def on_voice_input_received_direct(self, text: str):
        ...
        try:
            ...
            logger.debug(
                f"Direct voice input received in chat tab: {text}", print_to_terminal=True)

            # Stop any current streaming
            if self.is_streaming:
                logger.debug(
                    "Stopping current streaming to process direct voice input")
                self.stop_streaming()
            ...
            try:
                self.append_to_chat(
                    "System", f"Error processing direct voice input: {str(e)}")
            except:
                pass
    ...
    def on_tts_finished(self):
        ...
        QTimer.singleShot(100, lambda: setattr(
            self, '_tts_finished_handled', False))
        ...
        if not (self.voice_mode and
            ...
            try:
                self.eq_visualizer.switch_to_chat_display(
                    self.chat_display.scroll_area)
                logger.debug(
                    "Successfully restored chat display after TTS finished")
            except Exception as e:
                ...
                try:
                    ...
                    logger.debug(
                        "Forced chat display to be visible after TTS finished")
                except Exception as e2:
                    logger.error(f"Failed to force show chat display: {e2}")
        else:
            logger.debug(
                "TTS finished but EQ visualizer is active in voice mode, keeping EQ visualizer")
            ...
            if self.voice_mode and self.eq_visualizer.get_eq_mode() != "None":
                ...
                if self.voice_controls_initialized and self.voice_controls:
                    ...
                    if not tts_playing and audio_level > 0.05:
                        ...
                        tts_playing = True
                        logger.debug(
                            f"[EQ DEBUG] Inferred TTS playing from audio level: {audio_level:.4f}")

                    # Ensure EQ visualizer is properly initialized for voice mode
                    if not self.eq_visualizer.current_eq_widget:
                        logger.debug(
                            f"[EQ DEBUG] EQ widget not initialized, switching to EQ visualizer")
                        self.eq_visualizer.switch_to_eq_visualizer(
                            self.chat_display.scroll_area, self.voice_mode)

                    logger.debug(
                        f"[EQ DEBUG] TTS playing: {tts_playing}, audio_level: {audio_level:.4f}")
                    self.eq_visualizer.update_eq_visualizer(
                        audio_level, tts_playing)
            else:
                pass
        except Exception as e:
            logger.error(
                f"Error in on_audio_level_changed: {traceback.format_exc()}")
            raise (
                f"Error in on_audio_level_changed: {traceback.format_exc()}\n{e}")
    ...
    def on_eq_bars_changed(self, bar_values):
        ...
        try:
            if (self.voice_mode and
                ...
                logger.debug(
                    "[EQ DEBUG] EQ visualizer not available or not in voice mode")
        except Exception as e:
    ...
    def append_to_chat(self, sender: str, message: str, is_code: bool = False):
        ...
        logger.debug("Calling chat_display.append_to_chat",
                     print_to_terminal=True)
        self.chat_display.append_to_chat(sender, message, is_code)

        logger.debug("chat_display.append_to_chat completed",
                     print_to_terminal=True)
    ...
    def append_response_chunk(self, chunk: str, model_name: str = None, msg_id: str = None, chunk_index: int = None):
        ...
        logger.debug(
            f"[CHAT_TAB_DEBUG] append_response_chunk called with chunk: {chunk[:50]}, model_name: {model_name}, msg_id: {msg_id}, chunk_index: {chunk_index}")
        QTimer.singleShot(0, lambda: self._append_response_chunk_safe(
    ...
    def _append_response_chunk_safe(self, chunk: str, model_name: str = None, msg_id: str = None, chunk_index: int = None):
        ...
        try:
            logger.debug(
                f"[CHAT_TAB_DEBUG] _append_response_chunk_safe called with chunk: {chunk[:50]}, model_name: {model_name}, msg_id: {msg_id}, chunk_index: {chunk_index}")
            ...
            logger.debug(
                f"[CHAT_TAB_DEBUG] Calling chat_display.append_response_chunk with chunk: {chunk[:50]}, model_name: {model_name}, msg_id: {msg_id}, chunk_index: {chunk_index}")
            self.chat_display.append_response_chunk(
                chunk, model_name, msg_id, chunk_index=chunk_index)
        except Exception as e:
            logger.error(f"[PATCH] Error in _append_response_chunk_safe: {e}")
            logger.error(
                f"[PATCH] _append_response_chunk_safe traceback: {traceback.format_exc()}")
        ...
        try:
            ...
            if (self.voice_mode and
                ...
                self.eq_visualizer.current_eq_widget and
                    self.eq_visualizer.current_eq_widget.isVisible()):
                logger.debug(
                    "EQ visualizer is active in voice mode, not forcing chat display visibility")
                return
            ...
            if hasattr(self, 'chat_display') and self.chat_display.scroll_area:
                if not self.chat_display.scroll_area.isVisible():
                    logger.debug(
                        "Chat display was hidden, forcing it to be visible")
                    self.chat_display.scroll_area.show()
                    self.chat_display.scroll_area.setVisible(True)
                    self.chat_display.scroll_area.setEnabled(True)
    ...
    def stop_streaming(self):
        ...
        if hasattr(self, 'chat_controller') and hasattr(self.chat_controller, 'conversation_service'):
            try:
                ...
                logger.debug(
                    "[ID:STOP001] Finalized streaming message in conversation service")
            except Exception as e:
                logger.error(
                    f"[ID:STOP002] Error finalizing streaming in conversation service: {e}")
                logger.error(
                    f"[ID:STOP003] Stop streaming traceback: {traceback.format_exc()}")
        else:
            streaming_handler = self.chat_display.get_streaming_handler()
            if streaming_handler:
                streaming_handler.finalize_streaming_message()
                logger.debug(
                    "[ID:STOP001] Finalized streaming message in streaming handler")
            else:
    ...
    def _force_enable_send_button_safe(self):
        ...
        if not self.voice_mode:
            ...
            logger.debug(
                "[VOICE DEBUG] Text mode: force enabled send button (emergency UI reset)")
        else:
        ...
        safe_process_events_alternative()
        logger.debug(
            f"Send button enabled: {input_components['send_button'].isEnabled()}, visible: {input_components['send_button'].isVisible()}")

    def clear_chat(self):
        """Clear the chat display and reset all references"""
        try:
            logger.debug(
                "[CHAT_TAB] Clearing chat display and resetting references")
            ...
            logger.debug(
                "[CHAT_TAB] Chat display cleared and references reset successfully")
    ...
    def speak_ai_response(self, text: str):
        ...
        logger.debug(
            f"[CHAT_TAB] speak_ai_response called with text length: {len(text)}")
        logger.debug(
            f"[CHAT_TAB] voice_mode: {self.voice_mode}, voice_controls_initialized: {self.voice_controls_initialized}")

        # Ensure voice controls are initialized if in voice mode
        if self.voice_mode and not self.voice_controls_initialized:
            logger.debug(
                "Voice mode active but voice controls not initialized, initializing now")
            self._ensure_voice_controls_initialized()

        if self.voice_controls_initialized and self.voice_controls:
            try:
                logger.debug(
                    f"[CHAT_TAB] Calling voice_controls.speak_ai_response")
                self.voice_controls.speak_ai_response(text)
                logger.debug(
                    f"TTS request sent for AI response: {text[:50]}...")
            except Exception as e:
            ...
            logger.debug(
                f"[CHAT_TAB] voice_controls_initialized: {self.voice_controls_initialized}, voice_controls: {self.voice_controls}")
    ...
    def open_voice_settings(self):
        ...
        try:
            ...
            if not self.voice_controls_initialized or not self.voice_controls:
                logger.warning(
                    "Voice controls not available for settings dialog")
                QMessageBox.warning(self, "Voice Settings",
                                    "Voice controls are not available.")
                return
            ...
            dialog.eq_visualizer_changed.connect(
                self.on_eq_visualizer_changed_immediate)
            ...
            QMessageBox.critical(
                self, "Error", f"Failed to open voice settings: {str(e)}")
        ...
        if eq_mode != self.eq_visualizer.get_eq_mode():
            ...
            if self.voice_mode and eq_mode != "None":
                self.eq_visualizer.switch_to_eq_visualizer(
                    self.chat_display.scroll_area, self.voice_mode)
                logger.debug(
                    f"[EQ DEBUG] Switched to EQ visualizer: {eq_mode}")
    ...
    def on_eq_visualizer_changed_from_voice_controls(self, eq_mode: str):
        ...
        logger.debug(
            f"[EQ DEBUG] EQ visualizer change from voice controls: {eq_mode}")
        ...
        if eq_mode != self.eq_visualizer.get_eq_mode():
            ...
            if self.voice_mode and eq_mode != "None":
                self.eq_visualizer.switch_to_eq_visualizer(
                    self.chat_display.scroll_area, self.voice_mode)
                logger.debug(
                    f"[EQ DEBUG] Switched to EQ visualizer from voice controls: {eq_mode}")
            elif self.voice_mode and eq_mode == "None":
                # Switch back to chat display if EQ is disabled
                self.eq_visualizer.switch_to_chat_display(
                    self.chat_display.scroll_area)
                logger.debug(
                    "[EQ DEBUG] Switched back to chat display from voice controls")
    ...
    def load_conversation(self, filepath: str):
        ...
        logger.debug(
            f"[LOAD_DEBUG] Attempting to load conversation: {filepath}")
        ...
        try:
            ...
            if hasattr(self, 'chat_controller') and self.chat_controller and hasattr(self.chat_controller, 'conversation_service'):
                ...
                logger.debug(
                    f"[LOAD_DEBUG] Loading conversation {filename} through conversation service")
                conversation_service = self.chat_controller.conversation_service
                logger.debug(
                    f"[LOAD_DEBUG] Conversation service: {conversation_service}")

                # Load the conversation
                loaded_messages = conversation_service.load_conversation(
                    filename)
                logger.debug(
                    f"[LOAD_DEBUG] Loaded {len(loaded_messages)} messages from conversation service")

                # Ensure chat display is connected to conversation service
                if hasattr(self, 'chat_display'):
                    self.chat_display.set_conversation_service(
                        conversation_service)
                    logger.debug(
                        "[LOAD_DEBUG] Set conversation service in chat display")
                ...
                logger.debug(
                    f"[LOAD_FIX] Loaded conversation {filename} through conversation service")
            else:
                # Fallback to old method if conversation service not available
                logger.warning(
                    "[LOAD_FIX] Conversation service not available, using fallback method")
                logger.debug(
                    f"[LOAD_DEBUG] Chat controller: {getattr(self, 'chat_controller', None)}")
            ...
            try:
                ...
                if metadata.temperature is not None:
                    ...
                    input_components = self.input_controls.get_ui_components()
                    input_components['temperature_slider'].setValue(
                        int(metadata.temperature * 100))
            ...
            import traceback
            logger.error(
                f"Load conversation traceback: {traceback.format_exc()}")
            QMessageBox.critical(
                self, "Error", f"Failed to load conversation: {str(e)}")
        ...
        try:
            ...
            for message in conversation:
                if isinstance(message, dict):
                    sender_raw = message.get(
                        'sender') or message.get('role', 'Unknown')
                    sender = sender_raw[:1].upper(
                    ) + sender_raw[1:] if sender_raw else "Unknown"
    ...
    def set_chat_controller(self, chat_controller):
        ...
        if chat_controller and hasattr(chat_controller, 'conversation_service'):
            ...
            logger.debug(
                f"[CHAT_TAB] Conversation service from controller: {conversation_service}")
                ...
                if conversation_service and hasattr(conversation_service, 'conversation_updated'):
                    ...
                    if signal and hasattr(signal, 'disconnect'):
                        ...
                        if hasattr(signal, 'receivers') and signal.receivers() > 0:
                            ...
                            logger.debug(
                                "[CHAT_TAB] Signal has no receivers, skipping disconnect")
            except TypeError:
            ...
            if not hasattr(self, '_conversation_service_connected'):
                ...
                logger.debug(
                    "[CHAT_TAB] Connected to conversation service signals after controller set")
            else:
                logger.debug(
                    "[CHAT_TAB] Conversation service already connected, skipping duplicate connection")
            ...
            if hasattr(self, 'chat_display'):
                self.chat_display.set_conversation_service(
                    conversation_service)
                logger.debug(
                    "[CHAT_TAB] Set conversation service in chat display")
        else:
            logger.warning(
                f"[CHAT_TAB] Chat controller or conversation service not available: controller={chat_controller}")

    def verify_chat_controller_connection(self):
        """Verify that chat controller is properly connected"""
        if hasattr(self, 'chat_controller') and self.chat_controller:
            logger.debug(
                f"[CHAT_TAB] Chat controller is set: {self.chat_controller}")
            if hasattr(self.chat_controller, 'conversation_service'):
                logger.debug(
                    f"[CHAT_TAB] Conversation service is available: {self.chat_controller.conversation_service}")
                return True
            else:
                logger.warning(
                    "[CHAT_TAB] Chat controller has no conversation service")
                return False
        else:
    ...
    def finalize_streaming_and_start_tts(self, tts_text):
        ...
        logger.debug(
            "[VOICE] Finalizing streaming before starting TTS playback")
        ...
        def _finalize_and_start():
            try:
                ...
                if streaming_handler:
                    streaming_handler.finalize_streaming_message()
                logger.debug(
                    "[VOICE] Chat display finalized, starting TTS playback")
                self.speak_ai_response(tts_text)
            except Exception as e:
                logger.error(
                    f"[VOICE] Error finalizing streaming or starting TTS: {e}")
