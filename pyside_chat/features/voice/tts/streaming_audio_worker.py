...
class StreamingAudioWorker(QObject):
    ...
    def __init__(self, text: str, tts_service):
        ...
        logger.debug(f"StreamingAudioWorker initialized with text: {text[:50]}...")
    ...
    def run(self):
        try:
            ...
            for i, sentence in enumerate(sentences):
                ...
                logger.debug(f"Generating audio for sentence {i+1}/{total_sentences}: {sentence[:30]}...")
                audio_chunk = self._generate_audio_chunk(sentence)
                
                if audio_chunk is not None:
                    logger.debug(f"Generated audio chunk: shape={audio_chunk.shape}, dtype={audio_chunk.dtype}, max={np.max(audio_chunk)}, min={np.min(audio_chunk)}")
                    self.audio_chunk_ready.emit(audio_chunk)
                    progress = int((i + 1) / total_sentences * 100)
                    self.progress_updated.emit(progress)
                    logger.debug(f"Emitted audio chunk {i+1}/{total_sentences} (progress: {progress}%)")
                else:
                    logger.warning(f"Failed to generate audio for sentence {i+1}: {sentence[:30]}...")
                    
                time.sleep(0.1)
                
            if self.is_running:
                logger.debug("Streaming audio generation completed successfully")
            ...
            import traceback
            logger.error(f"Streaming audio generation error traceback: {traceback.format_exc()}")
    ...
    def _generate_audio_chunk(self, text: str) -> Optional[np.ndarray]:
        try:
            ...
            if isinstance(audio, list):
                ...
                logger.debug(f"Converted list audio to numpy array: shape={audio.shape}")
            ...
            if self.tts_service.speech_speed != 1.0:
                logger.debug(f"Adjusting audio speed to {self.tts_service.speech_speed}")
                audio = self._adjust_audio_speed(audio)
                
            logger.debug(f"Generated audio chunk: shape={audio.shape}, max={np.max(audio)}, min={np.min(audio)}")
            return audio
    ...
    def _adjust_audio_speed(self, audio: np.ndarray) -> np.ndarray:
        try:
            ...
            if self.tts_service.speech_speed > 1.0:
                ...
                logger.debug(f"Sped up audio: {len(audio)} -> {new_length} samples")
            elif self.tts_service.speech_speed < 1.0:
                new_length = int(len(audio) / self.tts_service.speech_speed)
                audio = signal.resample(audio, new_length)
                logger.debug(f"Slowed down audio: {len(audio)} -> {new_length} samples")
            return audio
        except ImportError:
