...
class EQVisualizer(QObject):
    ...
    def __init__(self, parent=None):
        ...
        # Store the original chat display when switching to EQ
    ...
    def setup_eq_visualizers(self):
        ...
        self.eq_widgets = {
            ...
            # Use CircularGradientEQWidget for Waveform Gradient
            "Waveform Gradient": CircularGradientEQWidget(),
            # Use CircularGradientEQWidget for Waveform Blue Gradient
            "Waveform Blue Gradient": CircularGradientEQWidget()
        }
    ...
    def switch_to_eq_visualizer(self, chat_display, voice_mode: bool):
        ...
        try:
            logger.debug(
                f"[EQ DEBUG] switch_to_eq_visualizer called - voice_mode: {voice_mode}, eq_visualizer_mode: {self.eq_visualizer_mode}")

            # Don't switch if already showing an EQ widget
            if self.current_eq_widget and self.current_eq_widget.isVisible():
                logger.debug(
                    f"[EQ DEBUG] EQ widget already visible, skipping switch")
                return
            ...
            if self.eq_visualizer_mode in self.eq_widgets:
                ...
                if scroll_area:
                    ...
                    if isinstance(scroll_area, QScrollArea):
                        ...
                        if scroll_area:
                            logger.debug(
                                f"[EQ DEBUG] Found QScrollArea in parent hierarchy, current widget: {scroll_area.widget()}")
                            scroll_area.takeWidget()
                            logger.debug(
                                f"[EQ DEBUG] Removed current widget from scroll area")
                    ...
                    try:
                        ...
                        logger.error(
                            f"[EQ DEBUG] Error starting EQ animation: {e}")
                    ...
                    logger.debug(
                        f"[EQ DEBUG] Successfully switched to EQ visualizer: {self.eq_visualizer_mode}")
                else:
                    logger.error(
                        f"[EQ DEBUG] No scroll area found for chat display")
            else:
                logger.error(
                    f"[EQ DEBUG] EQ visualizer mode '{self.eq_visualizer_mode}' not found in available widgets: {list(self.eq_widgets.keys())}")
        except Exception as e:
    ...
    def switch_to_chat_display(self, chat_display):
        ...
        logger.debug(
            f"[EQ DEBUG] switch_to_chat_display called - current_eq_widget: {self.current_eq_widget is not None}")

        # Don't switch if already showing chat display
        if chat_display and chat_display.isVisible():
            logger.debug(
                f"[EQ DEBUG] Chat display already visible, skipping switch")
            return
        ...
        if self.current_eq_widget:
            ...
            logger.debug(
                f"[EQ DEBUG] EQ widget parent found: {scroll_area is not None}, type: {type(scroll_area)}")
            ...
            if scroll_area:
                ...
                if isinstance(scroll_area, QScrollArea):
                    ...
                    scroll_area.takeWidget()
                    logger.debug(
                        f"[EQ DEBUG] Removed EQ widget from scroll area")
                    ...
                    if hasattr(self, 'stored_chat_display') and self.stored_chat_display:
                        ...
                        logger.debug(
                            f"[EQ DEBUG] Restored stored chat display in scroll area")
                        ...
                        logger.debug(
                            f"[EQ DEBUG] Set chat display in scroll area")
                else:
                    logger.debug(
                        f"[EQ DEBUG] Parent is not QScrollArea, it's: {type(scroll_area)}")
                    # Try to find the scroll area in the parent hierarchy
                ...
                logger.debug(
                    f"[EQ DEBUG] Hid EQ widget, showed and enabled chat display")
                ...
                self.current_eq_widget = None
                logger.debug(
                    "[EQ DEBUG] Successfully switched back to chat display")
            else:
    ...
    def update_eq_visualizer(self, audio_level: float, tts_playing: bool = False):
        ...
        logger.debug(
            f"[EQ DEBUG] update_eq_visualizer called with audio_level: {audio_level:.4f}, tts_playing: {tts_playing}")
        logger.debug(
            f"[EQ DEBUG] current_eq_widget exists: {self.current_eq_widget is not None}")
        logger.debug(
            f"[EQ DEBUG] current_eq_widget type: {type(self.current_eq_widget)}")
        ...
        try:
            ...
            if tts_playing:
                ...
                logger.debug(
                    f"[EQ DEBUG] TTS playing - base level: {base_level:.4f}")
                ...
                logger.debug(
                    f"[EQ DEBUG] Microphone input - base level: {base_level:.4f}")
            ...
            for i, (low_freq, high_freq) in enumerate(frequency_bands):
                ...
                if i < 6:  # Bass frequencies (0-5) - 25% of bars
                    ...
                    # Increased response for bass
                    freq_multiplier = 1.8 + (i * 0.15)
                ...
                # More variation for higher levels
                variation_factor = 0.6 + (audio_level * 0.8)
            ...
            logger.debug(
                f"[EQ DEBUG] Generated {len(bar_values)} bar values: {[f'{v:.3f}' for v in bar_values]}")

            # Use QTimer.singleShot to ensure UI update happens in main thread

            QTimer.singleShot(
                0, lambda: self._update_eq_widget_safe(bar_values))
            logger.debug(
                f"[EQ DEBUG] Scheduled EQ widget update for main thread")

        except Exception as e:
            logger.error(f"[EQ DEBUG] Error updating EQ visualizer: {e}")
            import traceback
            logger.error(
                f"[EQ DEBUG] EQ error traceback: {traceback.format_exc()}")
    ...
    def _update_eq_widget_safe(self, bar_values):
        ...
        try:
            if self.current_eq_widget and hasattr(self.current_eq_widget, 'set_eq_bars'):
                ...
                logger.debug(
                    f"[EQ DEBUG] Updated EQ widget with {len(bar_values)} bar values in main thread")
            else:
            ...
            import traceback
            logger.error(
                f"[EQ DEBUG] Safe update error traceback: {traceback.format_exc()}")
    ...
    def update_eq_visualizer_mode(self, mode: str):
        ...
        logger.debug(
            f"[EQ DEBUG] update_eq_visualizer_mode called - new_mode: {mode}, current_mode: {self.eq_visualizer_mode}")
