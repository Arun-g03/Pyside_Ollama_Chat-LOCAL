...
class SemanticSearchFallback(QObject):
    ...
    def _load_memories(self):
        ...
        try:
            if os.path.exists(self.metadata_file):
                logger.info(
                    "[ID:0335] Loading existing memories from cache", print_to_terminal=True)
                ...
                logger.info(
                    f"[ID:0334] Loaded {len(self.memories)} memories", print_to_terminal=True)

        except Exception as e:
            logger.error(
                f"[ID:0333] Error loading memories: {e}", print_to_terminal=True)
            self.memories = []
    ...
    def _save_memories(self):
        ...
        try:
            ...
            logger.debug(
                f"[ID:0332] Saved {len(self.memories)} memories to cache", print_to_terminal=True)

        except Exception as e:
            logger.error(
                f"[ID:0331] Error saving memories: {e}", print_to_terminal=True)
        ...
        try:
            ...
            try:
                ...
                existing_memory = next(
                    (m for m in self.memories if m.memory_id == memory_id), None)
                if existing_memory:
                    logger.debug(
                ...
                # Save to cache
                logger.debug(
                    f"[ID:0326] Saving memories to cache for memory: {memory_id}")
                self._save_memories()

                logger.debug(
                    f"[ID:0325] Added memory {memory_id}", print_to_terminal=True)
            ...
            logger.error(
                f"[ID:0323] Error adding memory: {e}", print_to_terminal=True)
            return False
        ...
        try:
            ...
            try:
                ...
                if memory_to_remove:
                    ...
                    logger.debug(
                        f"[ID:0320] Saving memories after removal for memory: {memory_id}")
                    self._save_memories()
                    logger.debug(
    ...
    def search_semantic(self, query: str, max_results: int = 10, min_similarity: float = 0.3,
        ...
        try:
            logger.debug(
                f"[ID:0313] Starting keyword search - Query: {query[:50]}...")
            logger.debug(
            ...
            if memory_types:
                memories_to_search = [
                    m for m in self.memories if m.memory_type in memory_types]
                logger.debug(
            ...
            logger.debug(
                f"[ID:0309] Keyword search completed - Found {len(results)} results")
            ...
            logger.error(
                f"[ID:0307] Error in keyword search: {e}", print_to_terminal=True)
            return []
        ...
        try:
            ...
            try:
                ...
                if memory:
                    ...
                    logger.debug(
                        f"[ID:0297] Saving memories after importance update for memory: {memory_id}")
                    self._save_memories()
                    logger.debug(
