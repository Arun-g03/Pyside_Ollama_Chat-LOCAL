...
class StreamingHandler(QObject):
    ...
    def __init__(self, render_callback: Callable = None, ai_name: str = "Assistant"):
        ...
        # 10ms delay for smoother typewriter effect
        self._typewriter_timer.setInterval(10)
    ...
    def append_message(self, sender: str, content: str, is_code: bool = False, tag: str = "ai"):
        ...
        try:
            ...
            logger.debug("[DEBUG][append_message] messages: %s", ', '.join(
                f"{m['sender']} streaming={m['is_streaming']} len={len(m['content'])}" for m in self.messages))
    ...
    def edit_message(self, message_index: int, new_content: str) -> bool:
        ...
        if 0 <= message_index < len(self.messages):
            ...
            logger.debug(
                f"[DEBUG][edit_message] Edited message {message_index}: '{old_content}' -> '{new_content}'")
    ...
    def update_streaming_message(self, content: str, sender: str, message_id: str = None,
        ...
        try:
            ...
            self._pending_chunk = (
                content, sender, message_id, is_code, tag, append)
        ...
        try:
            ...
            for msg in reversed(self.messages):
                if msg['is_streaming']:
                    ...
                    logger.debug(
                        f"[DEBUG] Finalized streaming message: {msg.get('message_id', 'unknown')}")
                    break
