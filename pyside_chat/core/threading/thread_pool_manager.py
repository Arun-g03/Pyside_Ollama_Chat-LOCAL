...
class ThreadPoolManager(QObject):
    ...
    def __init__(self, max_threads: int = 4, parent=None):
        try:
            ...
            logger.debug(
                f"[ID:TP001] ThreadPoolManager created - Max threads: {max_threads}")
        except Exception as e:
            logger.error(
                f"[ID:TP001A] Exception in __init__: {e}\n{traceback.format_exc()}")
    ...
    def start_task(self, task, task_id: Optional[str] = None) -> str:
        ...
        try:
            ...
            if len(self.active_tasks) >= self.max_queue_size:
                logger.warning(
                    f"[ID:TP011] Queue full ({len(self.active_tasks)} tasks), rejecting task: {task_id}")
                return task_id
            ...
            logger.debug(
                f"[ID:TP002] Starting task: {task_id} ({task_info['task_type']})")
        ...
        try:
            ...
            while task_id in self.active_tasks:
                if time.time() - start_time > timeout:
                    logger.warning(
                        f"[ID:TP005] Timeout waiting for task: {task_id}")
                    return False
            ...
            logger.error(
                f"[ID:TP007] Error waiting for task {task_id}: {e}\n{traceback.format_exc()}")
            return False
    ...
    def wait_for_all_tasks(self, timeout: float = 60.0) -> bool:
        ...
        try:
            ...
            while self.active_tasks:
                if time.time() - start_time > timeout:
                    logger.warning(
                        f"[ID:TP008] Timeout waiting for {len(self.active_tasks)} tasks")
                    return False
            ...
            logger.error(
                f"[ID:TP010] Error waiting for all tasks: {e}\n{traceback.format_exc()}")
            return False
    ...
    def cancel_task(self, task_id: str) -> bool:
        ...
        try:
            if task_id in self.active_tasks:
                ...
                logger.warning(
                    f"[ID:TP012] Task not found for cancellation: {task_id}")
                return False

        except Exception as e:
            logger.error(
                f"[ID:TP013] Error cancelling task {task_id}: {e}\n{traceback.format_exc()}")
            return False
    ...
    def cancel_all_tasks(self) -> int:
        ...
        try:
            ...
            logger.error(
                f"[ID:TP015] Error cancelling all tasks: {e}\n{traceback.format_exc()}")
            return 0
    ...
    def get_pool_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"[ID:TP016] Error getting pool status: {e}\n{traceback.format_exc()}")
            return {}
    ...
    def get_task_info(self, task_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                f"[ID:TP017] Error getting task info for {task_id}: {e}\n{traceback.format_exc()}")
            return None
    ...
    def cleanup_old_tasks(self, max_age_hours: float = 24.0):
        ...
        try:
            ...
            logger.debug(
                f"[ID:TP018] Cleaned up old tasks (max age: {max_age_hours}h)")

        except Exception as e:
            logger.error(
                f"[ID:TP019] Error cleaning up old tasks: {e}\n{traceback.format_exc()}")
    ...
    def _update_pool_status(self):
        ...
        try:
            ...
            self.stats['pool_utilization'] = (
                active_count / max_count) * 100 if max_count > 0 else 0
            ...
            logger.error(
                f"[ID:TP020] Error updating pool status: {e}\n{traceback.format_exc()}")
    ...
    def on_task_completed(self, task_id: str):
        ...
        try:
            ...
            logger.error(
                f"[ID:TP022] Error handling task completion for {task_id}: {e}\n{traceback.format_exc()}")
    ...
    def on_task_failed(self, task_id: str, error_message: str):
        ...
        try:
            if task_id in self.active_tasks:
                ...
                logger.error(
                    f"[ID:TP023] Task failed: {task_id} - {error_message}")
                self.task_failed.emit(task_id, error_message)

        except Exception as e:
            logger.error(
                f"[ID:TP024] Error handling task failure for {task_id}: {e}\n{traceback.format_exc()}")
    ...
    def shutdown(self):
        ...
        try:
            ...
            logger.error(
                f"[ID:TP027] Error during ThreadPoolManager shutdown: {e}\n{traceback.format_exc()}")
...
def get_global_thread_pool_manager() -> ThreadPoolManager:
    ...
    try:
        if _global_thread_pool_manager is None:
            ...
            logger.debug(
                f"[ID:TP028] Created global ThreadPoolManager with {recommended_threads} threads")
        return _global_thread_pool_manager
    except Exception as e:
        logger.error(
            f"[ID:TP028A] Error getting global ThreadPoolManager: {e}\n{traceback.format_exc()}")
        raise
...
def shutdown_global_thread_pool_manager():
    ...
    try:
        ...
        logger.error(
            f"[ID:TP029A] Error shutting down global ThreadPoolManager: {e}\n{traceback.format_exc()}")
