        ...
        for i, message in enumerate(conversation):
            try:
                ...
                if role == 'assistant' and thought:
                    ...
                    thought_html = self._generate_single_message_html(thought_sender, thought, message_idx)
                    if thought_html:
                        html_parts.append(thought_html)
                    
                    # Then render the main assistant message with extra top margin
                    message_html = self._generate_single_message_html(display_sender, content, message_idx)
                    if message_html:
                        # Add extra top margin to separate thought from main message
                        message_html = message_html.replace('margin: 8px 0;', 'margin: 16px 0 8px 0;')
    ...
    def _generate_single_message_html(self, sender: str, content: str, message_idx: int) -> str:
        ...
        try:
            ...
            if sender == "You":
                ...
                style_type = self.system_message_manager.get_style_for_message(content)
                ...
                formatted_content = ChatStyles.format_message_content(content, sender)
