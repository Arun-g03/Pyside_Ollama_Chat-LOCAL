...
class StreamingWorker(QObject):
    ...
    def _log_thread_info(self, action: str):
        ...
        logger.debug(
            f"[ID:TH002] StreamingWorker {action} - Thread: {thread_name} (ID: {thread_id})")
    ...
    def configure_streaming(self, **kwargs):
        ...
        try:
            ...
            if 'streaming_message_id' in kwargs:
                logger.debug(
                    f"[ID:TH003] StreamingWorker configured with streaming_message_id: {kwargs['streaming_message_id']}")
            else:
                logger.debug(
                    f"[ID:TH003] StreamingWorker configured with: {list(kwargs.keys())}")
            ...
            if 'streaming_message_id' in self._configuration:
                logger.debug(
                    f"[ID:TH003A] Final configuration has streaming_message_id: {self._configuration['streaming_message_id']}")
            else:
                logger.debug(
                    f"[ID:TH003A] Final configuration has no streaming_message_id")
    ...
    def reset_state(self):
        ...
        try:
            ...
            streaming_message_id = self._configuration.get(
            ...
            if streaming_message_id:
                ...
                logger.debug(
                    f"[ID:TH005] StreamingWorker state reset - preserved streaming_message_id: {streaming_message_id}")
            else:
                logger.debug(
                    "[ID:TH005] StreamingWorker state reset - no streaming_message_id to preserve")

        except Exception as e:
            logger.error(
                f"[ID:TH006] Error resetting streaming worker state: {e}")
    ...
    def start_streaming(self):
        ...
        try:
            ...
            if 'streaming_message_id' in self._configuration:
                logger.debug(
                    f"[ID:TH007A] Configuration has streaming_message_id: {self._configuration['streaming_message_id']}")
            else:
                logger.warning(
                    f"[ID:TH007A] Configuration missing streaming_message_id! Available keys: {list(self._configuration.keys())}")
    ...
    def _stream_operation(self):
        ...
        raise NotImplementedError(
            "Subclasses must implement _stream_operation")
...
class ChatStreamingWorker(StreamingWorker):
    ...
    def configure_streaming(self, context_messages: List[Dict], model: str, temperature: float, config_manager, streaming_message_id: str = None):
        ...
        try:
            ...
            logger.debug(
                f"[ID:TH009] ChatStreamingWorker configured for model: {model}, streaming_message_id: {streaming_message_id}")

        except Exception as e:
            logger.error(
                f"[ID:TH010] Error configuring chat streaming worker: {e}")
    ...
    def _stream_operation(self):
        ...
        try:
            ...
            if not config_manager:
                raise ValueError(
                    "Configuration manager is required for chat streaming")
            ...
            logger.debug(
                f"[ID:TH011] Starting chat streaming to: {stream_url}")
            ...
            chunk_count = 0
            streaming_message_id = self._configuration.get(
                'streaming_message_id', "msg_id_placeholder")
            logger.debug(
                f"[WORKER_DEBUG] Using streaming_message_id: {streaming_message_id}")

            # Log whether we're using a real ID or placeholder
            if streaming_message_id == "msg_id_placeholder":
                logger.warning(
                    f"[WORKER_DEBUG] Using placeholder streaming_message_id - this indicates a configuration issue!")
                logger.debug(
                    f"[WORKER_DEBUG] Available configuration keys: {list(self._configuration.keys())}")
            else:
                logger.debug(
                    f"[WORKER_DEBUG] Using real streaming_message_id: {streaming_message_id}")
                    ...
                    try:
                        ...
                        if 'message' in data:
                            ...
                            if content:
                                chunk_count += 1
                                logger.debug(
                                    f"[WORKER_DEBUG] Emitting chunk {chunk_count}: content='{content[:20]}...', msg_id='{streaming_message_id}'")

                                # Validate content before emitting
                                if not content.strip():
                                    logger.warning(
                                        f"[WORKER_DEBUG] Skipping empty chunk {chunk_count}")
                                    continue

                                # Validate streaming message ID
                                if not streaming_message_id or streaming_message_id == "msg_id_placeholder":
                                    logger.error(
                                        f"[WORKER_DEBUG] Invalid streaming message ID: '{streaming_message_id}', this will cause chunk processing to fail")
                                    logger.error(
                                        f"[WORKER_DEBUG] Available configuration keys: {list(self._configuration.keys())}")

                                self.chunk_received.emit(
                                    content, model, streaming_message_id, chunk_count)
                                ...
                                logger.debug(
                                    f"[WORKER_DEBUG] Skipping empty content from Ollama response")

                        # Check for done signal
                        if data.get('done', False):
                            logger.debug(
                                f"[ID:TH014] Chat streaming completed - Total chunks: {chunk_count}")
                            break
                        ...
                        logger.error(
                            f"[ID:TH016] Error processing streaming chunk: {e}")
                        self.error.emit(f"Error processing chunk: {str(e)}")
                        break

            self.progress_updated.emit(
                f"Chat streaming completed - {chunk_count} chunks")
            logger.debug(
                f"[ID:TH017] Chat streaming finished - Total chunks: {chunk_count}")
            ...
            logger.error(
                f"[ID:TH020] Chat streaming error traceback: {traceback.format_exc()}")
            self.error.emit(error_msg)
        finally:
            self._running = False
            self.finished.emit()
...
class AudioStreamingWorker(StreamingWorker):
    ...
    def configure_audio_streaming(self, audio_source: str, sample_rate: int = 16000, chunk_size: int = 1024):
        ...
        try:
            ...
            logger.debug(
                f"[ID:TH021] AudioStreamingWorker configured for source: {audio_source}")

        except Exception as e:
            logger.error(
                f"[ID:TH022] Error configuring audio streaming worker: {e}")
    ...
    def _stream_operation(self, audio_source: str, sample_rate: int = 16000,
        ...
        try:
            ...
            # Get configuration
            audio_source = self._configuration.get(
                'audio_source', audio_source)
            ...
            while self._running and not self._should_stop:
                try:
                    ...
                    self.chunk_received.emit(
                        simulated_chunk, "simulated_model", "simulated_msg_id", chunk_count)

                    # Update progress periodically
                    if chunk_count % 10 == 0:
                        self.progress_updated.emit(
                            f"Processed {chunk_count} audio chunks...")
                    ...
                    logger.error(
                        f"[ID:TH025] Error processing audio chunk: {e}")
                    self.error.emit(f"Error processing audio chunk: {str(e)}")
                    break

            self.progress_updated.emit(
                f"Audio streaming completed - {chunk_count} chunks")
            logger.debug(
                f"[ID:TH026] Audio streaming finished - Total chunks: {chunk_count}")

        except Exception as e:
            error_msg = f"Unexpected error during audio streaming: {str(e)}"
            logger.error(f"[ID:TH027] {error_msg}")
            logger.error(
                f"[ID:TH028] Audio streaming error traceback: {traceback.format_exc()}")
            self.error.emit(error_msg)
        finally:
            self._running = False
            self.finished.emit()
...
class MonitoringWorker(StreamingWorker):
    ...
    def configure_monitoring(self, monitor_interval: float = 1.0, alert_threshold: float = 0.8):
        ...
        try:
            ...
            logger.debug(
                f"[ID:TH029] MonitoringWorker configured - interval: {monitor_interval}s")

        except Exception as e:
            logger.error(
                f"[ID:TH030] Error configuring monitoring worker: {e}")
    ...
    def _stream_operation(self, monitor_interval: float = 1.0,
        ...
        try:
            logger.debug(
                f"[ID:TH032] Monitoring started - Interval: {monitor_interval}s")
            self.progress_updated.emit("Starting system monitoring...")

            # Get configuration
            monitor_interval = self._configuration.get(
                'monitor_interval', monitor_interval)
            alert_threshold = self._configuration.get(
                'alert_threshold', alert_threshold)
            ...
            logger.error(
                f"[ID:TH036] Monitoring error traceback: {traceback.format_exc()}")
            self.error.emit(error_msg)
        finally:
            self._running = False
            self.finished.emit()
