...
class ThreadCalculator:
    ...
    def get_system_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Error getting psutil.cpu_count(logical=True): {e}")
                logical_cores = cpu_count
            try:
                physical_cores = psutil.cpu_count(logical=False)
            except Exception as e:
                logger.error(f"Error getting psutil.cpu_count(logical=False): {e}")
    ...
    def _calculate_recommendations(self, cpu_count: int, logical_cores: int, 
        ...
        try:
            # Base thread counts
            try:
                max_worker_threads = min(logical_cores * 2, 16)  # Cap at 16 for stability
            except Exception as e:
            ...
            try:
                cpu_bound_threads = max(2, logical_cores - 1)  # Leave one core free
            except Exception as e:
            ...
            try:
                ...
                logger.error(f"Error calculating chunk_processing_threads: {e}")
                chunk_processing_threads = 2
            
            # Memory-based adjustments
            try:
                memory_safe_threads = self._calculate_memory_safe_threads(memory_gb, logical_cores)
            except Exception as e:
            ...
            try:
                ...
                logger.error(f"Error applying system-specific thread adjustments: {e}")
    ...
    def _calculate_memory_safe_threads(self, memory_gb: float, logical_cores: int) -> int:
        ...
        try:
            ...
            memory_based_threads = int(available_memory_mb / estimated_memory_per_thread_mb)
    ...
    def print_system_analysis(self):
        ...
        try:
            ...
            print(f"  Max Worker Threads: {recommendations.max_worker_threads}")
            print(f"  Default Worker Threads: {recommendations.default_worker_threads}")
            ...
            print(f"  Background Threads: {recommendations.background_threads}")
            print(f"  Streaming Threads: {recommendations.streaming_threads}")
            print(f"  Chunk Processing Threads: {recommendations.chunk_processing_threads}")
            print(f"  Memory-Safe Threads: {recommendations.memory_safe_threads}")
            print(f"  Conservative Threads: {recommendations.conservative_threads}")
            
            print(f"\nUsage Examples:")
            print(f"  calculator.get_recommendations_for_pool('worker') -> {self.get_recommendations_for_pool('worker')}")
            print(f"  calculator.get_recommendations_for_pool('streaming') -> {self.get_recommendations_for_pool('streaming')}")
            print(f"  calculator.get_recommendations_for_pool('memory_safe') -> {self.get_recommendations_for_pool('memory_safe')}")
