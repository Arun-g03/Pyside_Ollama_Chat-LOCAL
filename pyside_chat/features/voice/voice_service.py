...
class VoiceService(QObject):
    ...
    def start_voice_input(self):
        ...
        try:
            ...
            try:
                ...
                LoggingHelpers.log_exception_with_context("set_speech_detection_parameters", e, {
                    "silence_duration": self.silence_duration,
                    "silence_threshold": self.silence_threshold,
                    "min_speech_duration": self.min_speech_duration
                })
            ...
            try:
                ...
                LoggingHelpers.log_exception_with_context("recording_service.start_recording", e, {})
                raise
            ...
            LoggingHelpers.log_audio_operation("start_voice_input", True, details=f"silence: {self.silence_duration}s, threshold: {self.silence_threshold}")
            logger.debug(f"Voice input started with audio gate (silence: {self.silence_duration}s, threshold: {self.silence_threshold})")
        except Exception as e:
            self.is_recording = False
            LoggingHelpers.log_audio_operation("start_voice_input", False, e)
    ...
    def stop_voice_input(self):
        ...
        try:
            if not self.recording_service:
                LoggingHelpers.log_audio_operation("stop_voice_input", False, Exception("Recording service not available"))
                logger.error("Recording service not available")
                return
                
            if not self.is_recording and not self.recording_service.frames:
                LoggingHelpers.log_warning_with_context("No voice recording in progress", {"is_recording": self.is_recording})
            ...
            try:
                ...
                LoggingHelpers.log_exception_with_context("recording_timer.stop", e, {})
            
            try:
                result = self.recording_service.stop_recording()
            except Exception as e:
                LoggingHelpers.log_exception_with_context("recording_service.stop_recording", e, {})
                raise
            
            self.is_recording = False
            
            if result is None:
                LoggingHelpers.log_warning_with_context("No audio file generated", {"result": result})
            ...
            if audio_file and os.path.exists(audio_file) and speech_detected:
                ...
                if self.response_queue:
                    try:
                        ...
                        LoggingHelpers.log_exception_with_context("response_queue.put", e, {})
                
                if self.stt_service:
                    try:
                        self.stt_service.convert_audio_to_text(audio_file)
                    except Exception as e:
                        LoggingHelpers.log_exception_with_context("stt_service.convert_audio_to_text", e, {"audio_file": audio_file})
                        self.is_processing_voice = False
                        self.voice_input_error.emit(f"STT processing failed: {str(e)}")
                else:
                    LoggingHelpers.log_audio_operation("stop_voice_input", False, Exception("STT service not available"))
                    logger.error("STT service not available")
                    self.is_processing_voice = False
                    self.voice_input_error.emit("STT service not available")
            else:
                LoggingHelpers.log_info_with_context("No speech detected during recording", {"audio_file": audio_file, "speech_detected": speech_detected})
            ...
            if self.is_recording:
                self.is_recording = False
            self.is_processing_voice = False
            LoggingHelpers.log_audio_operation("stop_voice_input", False, e)
    ...
    def is_voice_available(self) -> bool:
        ...
        stt_available = self.stt_service and self.stt_service.is_available() if self.stt_service else False
        tts_available = self.tts_service and self.tts_service.is_available() if self.tts_service else False
        recording_available = self.recording_service and self.recording_service.is_available() if self.recording_service else False
        
        return stt_available and tts_available and recording_available
