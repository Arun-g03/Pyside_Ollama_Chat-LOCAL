...
class SettingsDialog(QDialog):
    ...
    def load_current_settings(self):
        ...
        if not self.ui_ready:
            logger.debug(f"[ID:0139] SETTINGS: UI not ready yet, skipping settings load",print_to_terminal=False)
            return
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"[ID:0138] SETTINGS: UI elements deleted during load: {e}",print_to_terminal=True)
                return
            ...
            logger.debug(f"[ID:0137] SETTINGS: Error loading settings: {e}",print_to_terminal=True)
