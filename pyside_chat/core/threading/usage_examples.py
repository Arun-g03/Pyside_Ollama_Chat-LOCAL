...
class ChatApplicationExample:
    ...
    def start_chat_streaming(self, messages: List[Dict], model: str, temperature: float,
        ...
        try:
            logger.debug(
                f"[ID:EX002] Starting chat streaming for model: {model}")
            ...
            self.chat_streaming_thread.setObjectName(
                f"ChatStreamingThread_{id(self.chat_streaming_thread)}")
            ...
            self.chat_streaming_thread.finished.connect(
                self._on_chat_streaming_thread_finished)
            ...
            logger.debug(
                f"[ID:EX003] Chat streaming thread started: {self.chat_streaming_thread.objectName()}")

        except Exception as e:
            logger.error(f"[ID:EX004] Error starting chat streaming: {e}")
            logger.error(
                f"[ID:EX005] Chat streaming error traceback: {traceback.format_exc()}")
        ...
        try:
            ...
            if self.chat_streaming_thread:
                try:
                    ...
                    logger.debug(
                        "[ID:EX007] Chat streaming thread marked for deletion")
                except Exception as e:
                    logger.error(
                        f"[ID:EX008] Error deleting chat streaming thread: {e}")
                finally:
                    self.chat_streaming_thread = None
                    self.chat_streaming_worker = None
            else:
            ...
            logger.error(
                f"[ID:EX010] Error handling chat streaming thread finished: {e}")
    ...
    def stop_chat_streaming(self):
        ...
        try:
            ...
            if self.chat_streaming_thread and self.chat_streaming_thread.isRunning():
                ...
                # 5 second timeout
                if not self.chat_streaming_thread.wait(5000):
                    logger.warning(
                        "[ID:EX012] Chat streaming thread did not quit within timeout")
    ...
    def process_message_spell_check(self, message: str):
        ...
        try:
            logger.debug(
                f"[ID:EX015] Processing message spell check: {message[:50]}...")
    ...
    def process_message_formatting(self, message: str):
        ...
        try:
            logger.debug(
                f"[ID:EX018] Processing message formatting: {message[:50]}...")
    ...
    def process_file_operation(self, file_path: str, operation: str, **kwargs):
        ...
        try:
            logger.debug(
                f"[ID:EX021] Processing file operation: {operation} on {file_path}")
    ...
    def _on_chat_streaming_finished(self):
        ...
        try:
            ...
            logger.error(
                f"[ID:EX029] Error handling chat streaming finished: {e}")
    ...
    def _on_chat_streaming_error(self, error: str):
        ...
        try:
            ...
            logger.error(
                f"[ID:EX031] Error handling chat streaming error: {e}")
    ...
    def on_message_processed(self, task_type: str, result: Dict[str, Any]):
        ...
        try:
            logger.debug(
                f"[ID:EX032] Message processing completed: {task_type}")

            if task_type == "spell_check":
                if result.get('has_corrections', False):
                    logger.info(
                        f"[ID:EX033] Spell check found corrections: {result['corrections']}")
                else:
                    logger.debug(
                        "[ID:EX034] Spell check completed - no corrections needed")

            elif task_type == "formatting":
                if result.get('changes_made', False):
                    logger.info(
                        f"[ID:EX035] Formatting applied: {result['formatted']}")
                else:
                    logger.debug(
                        "[ID:EX036] Formatting completed - no changes needed")

            # Handle the result (e.g., update UI, apply corrections)

        except Exception as e:
            logger.error(
                f"[ID:EX037] Error handling message processing result: {e}")

    def on_message_processing_error(self, task_type: str, error_message: str):
        """Handle message processing error."""
        try:
            logger.error(
                f"[ID:EX038] Message processing error ({task_type}): {error_message}")
            # Handle the error (e.g., show error message to user)

        except Exception as e:
            logger.error(
                f"[ID:EX039] Error handling message processing error: {e}")

    def on_file_processed(self, operation: str, file_path: str, result: Dict[str, Any]):
        """Handle file processing completion."""
        try:
            logger.debug(
                f"[ID:EX040] File processing completed: {operation} on {file_path}")

            if 'error' in result:
                logger.error(
                    f"[ID:EX041] File processing error: {result['error']}")
            else:
            ...
            logger.error(
                f"[ID:EX043] Error handling file processing result: {e}")

    def on_file_processing_error(self, operation: str, file_path: str, error_message: str):
        """Handle file processing error."""
        try:
            logger.error(
                f"[ID:EX044] File processing error ({operation} on {file_path}): {error_message}")
            # Handle the error (e.g., show error message to user)

        except Exception as e:
            logger.error(
                f"[ID:EX045] Error handling file processing error: {e}")
    ...
    def cleanup(self):
        ...
        try:
            ...
            logger.error(
                f"[ID:EX049] Error during ChatApplicationExample cleanup: {e}")
...
def demonstrate_threading_usage():
    ...
    app.process_file_operation(
        "output.json", "write", content='{"test": "data"}')
