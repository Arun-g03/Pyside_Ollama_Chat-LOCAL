...
class ConversationMetadata:
    ...
    # AI-generated name for the conversation
...
class ConversationManager(QObject):
    ...
    def update_conversation_name(self, filepath: str, ai_generated_name: str) -> Optional[str]:
        ...
        try:
            logger.debug(
                f"[ID:0087] Updating conversation name for {filepath} to '{ai_generated_name}'")

            # Load the conversation
            conversation, metadata = self.load_conversation(filepath)
            logger.debug(
                f"[ID:0086] Loaded conversation with {len(conversation)} messages")
            logger.debug(
                f"[ID:0085] Original metadata ai_generated_name: {metadata.ai_generated_name}")

            # Update the AI-generated name
            metadata.update_ai_generated_name(ai_generated_name)
            logger.debug(
                f"[ID:0084] Updated metadata ai_generated_name: {metadata.ai_generated_name}")

            # Create a safe filename from the AI-generated name
            safe_filename = self._create_safe_filename(ai_generated_name)
            logger.debug(f"[ID:0083] Created safe filename: {safe_filename}")

            # Generate new filepath
            new_filepath = os.path.join(self.history_dir, safe_filename)
            logger.debug(f"[ID:0082] New filepath: {new_filepath}")
            ...
            if os.path.exists(new_filepath) and new_filepath != filepath:
                ...
                new_filepath = os.path.join(self.history_dir, safe_filename)
                logger.debug(
                    f"[ID:0081] File exists, using timestamped filename: {new_filepath}")
            ...
            logger.debug(
                f"[ID:0080] Saving with metadata: {save_data['metadata']}")
            ...
            if new_filepath != filepath:
                try:
                    os.remove(filepath)
                    logger.debug(f"[ID:0079] Deleted old file: {filepath}")
                except Exception as e:
                    logger.warning(
                        f"[ID:0078] Failed to remove old file {filepath}: {e}")
            ...
            if self.metadata.current_conversation_file == filepath:
                ...
                logger.debug(
                    f"[ID:0077] Updated current conversation file reference to: {new_filepath}")

            self.metadata_updated.emit()
            logger.debug(
                f"[ID:0076] Successfully updated conversation name, returning: {new_filepath}")
            return new_filepath

        except Exception as e:
            logger.error(
                f"[ID:0075] Failed to update conversation name: {str(e)}")
            return None
    ...
    def auto_save_conversation(self, conversation: list) -> Optional[str]:
        ...
        """
        logger.debug(
            f"[ID:0074] Auto-save attempt - enabled: {self.metadata.auto_save_enabled}, conversation length: {len(conversation) if conversation else 0}")

        if not self.metadata.auto_save_enabled:
            logger.debug(
                "[ID:0073] Auto-save skipped: auto_save_enabled is False")
            return None
        ...
        if not conversation and self.metadata.current_conversation_file and os.path.exists(self.metadata.current_conversation_file):
            try:
                with open(self.metadata.current_conversation_file, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                    existing_conversation = data.get(
                        "conversation", []) if isinstance(data, dict) else data
                    if len(existing_conversation) > 0:
                        logger.debug(
                            "[ID:0072] Auto-save skipped: conversation is empty and existing file has content")
                        return None
            except:
        ...
        if not self.metadata.current_conversation_file:
            if self.metadata.ai_generated_name:
                safe_filename = self._create_safe_filename(
                    self.metadata.ai_generated_name)
            ...
            try:
                with open(self.metadata.current_conversation_file, 'r', encoding='utf-8') as f:
                    ...
                    if isinstance(data, dict) and "metadata" in data:
                        existing_ai_name = data["metadata"].get(
                            "ai_generated_name")
                        if existing_ai_name and not self.metadata.ai_generated_name:
                            logger.debug(
                                f"[ID:0071] Preserving existing AI name: {existing_ai_name}")
                            self.metadata.update_ai_generated_name(
                                existing_ai_name)
            except Exception as e:
                logger.debug(
                    f"[ID:0070] Could not read existing file for AI name preservation: {e}")
        ...
        try:
            ...
            self.metadata_updated.emit()
            logger.debug(
                f"[ID:0069] Auto-save successful: {self.metadata.current_conversation_file}")
            return self.metadata.current_conversation_file

        except Exception as e:
            logger.debug(
                f"[ID:0068] Auto-save failed: {e}", print_to_terminal=True)
            return None
    ...
    def list_conversations(self) -> list[tuple[str, ConversationMetadata]]:
        ...
        for filename in os.listdir(self.history_dir):
            if filename.endswith('.json'):
                ...
                try:
                    ...
                    logger.debug(
                        f"[ID:0067] Error reading {filename}: {e}", print_to_terminal=True)
        ...
        conversations.sort(
            key=lambda x: x[1].last_modified or "", reverse=True)
        return conversations
    ...
    def delete_conversation(self, filepath: str) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"[ID:0066] Failed to delete {filepath}: {e}", print_to_terminal=True)
            return False
    ...
    def rename_conversation(self, old_filepath: str, new_filepath: str) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"[ID:0065] Failed to rename {old_filepath} to {new_filepath}: {e}", print_to_terminal=True)
            return False
    ...
    def create_new_conversation(self) -> Optional[str]:
        ...
        try:
            ...
            logger.debug(
                f"[ID:0143] Created new conversation file: {filepath}")
            return filepath

        except Exception as e:
            logger.error(
                f"[ID:0144] Failed to create new conversation: {str(e)}")
            return None
