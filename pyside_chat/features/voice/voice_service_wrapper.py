...
class VoiceServiceWrapper(QObject):
        ...
        try:
            ...
            if hasattr(self.process_manager, 'user_interrupted'):
                self.process_manager.user_interrupted.connect(
                    self.user_interrupted.emit, Qt.ConnectionType.QueuedConnection)
            if hasattr(self.process_manager, 'request_cancelled'):
                self.process_manager.request_cancelled.connect(
                    self.request_cancelled.emit, Qt.ConnectionType.QueuedConnection)

            logger.info(
                "[ID:0225] Voice service wrapper initialized with process manager")

        except Exception as e:
            logger.error(
                f"[ID:0224] Failed to initialize process manager: {e}")
    ...
    def _init_direct_service(self):
        ...
        try:
            ...
            logger.info(
                "[ID:0222] Voice service wrapper initialized with direct service")
            ...
            logger.error(
                f"[ID:0221] Failed to initialize direct voice service: {e}")
    ...
    def set_continuous_voice_mode(self, enabled: bool):
        ...
        if self.use_separate_process and self.process_manager:
            self.process_manager.send_command(
                "set_continuous_voice_mode", enabled)
        elif self.direct_service:
    ...
    def _on_voice_service_ready(self):
        ...
        logger.info("Voice service ready signal received in wrapper",
        ...
        if self.direct_service and self.direct_service.is_voice_available():
            logger.info(
                "Direct voice service is ready, emitting wrapper ready signal", print_to_terminal=True)
            ...
            logger.warning(
                "Direct voice service not ready yet, waiting...", print_to_terminal=True)

    def _check_service_readiness(self):
        """Check if the voice service is ready and emit signal if it is"""
        if self.direct_service and self.direct_service.is_voice_available():
            logger.info(
                "Voice service is now ready, emitting ready signal", print_to_terminal=True)
            self.voice_service_ready.emit()
            # Stop the timer since we're ready
            self._ready_check_timer.stop()
        else:
            logger.debug(
                "Voice service still not ready, checking again in 1 second")
