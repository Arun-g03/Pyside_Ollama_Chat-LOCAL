...
class CoquiTTSService(QObject):
    ...
    def _initialize_service(self):
        ...
        try:
            ...
            if hasattr(self, '_loaded_models') and self._loaded_models:
                logger.info(
                    f"Using cached Coqui TTS service with {len(self._loaded_models)} cached models")
            else:
    ...
    def _get_tts_model_cache_dirs(self):
        ...
        if sys.platform == 'darwin':
            cache_dirs.append(os.path.expanduser(
    ...
    def get_downloaded_models(self) -> list:
        ...
        logger.info(
            f"[CoquiTTS] Checking model cache directories: {cache_dirs}")
            ...
            if os.path.exists(base_dir):
                for item in os.listdir(base_dir):
                    ...
                    if os.path.isdir(item_path):
                        ...
                        has_weights = any(f.endswith(".pth")
                                          for f in os.listdir(item_path))
                        if os.path.exists(config_path) and has_weights:
                            model_name = item.replace('--', '/')
                            logger.info(
                                f"[CoquiTTS] Found downloaded model: {model_name} in {item_path}")
                            downloaded.add(model_name)
        logger.info(
            f"[CoquiTTS] Downloaded models: {list(downloaded)}", print_to_terminal=True)
    ...
    def set_voice(self, voice_name: str) -> bool:
        ...
        try:
            ...
            if voice_name == "No speakers available":
                logger.warning(
                    "Invalid voice name 'No speakers available', using default")
                voice_name = "ED"  # Default voice for VCTK model
            ...
            if hasattr(self, 'available_voices') and self.available_voices:
                if voice_name not in self.available_voices:
                    logger.warning(
                        f"Voice {voice_name} not available, using first available voice")
    ...
    def speak_text(self, text: str, use_streaming: bool = True):
        ...
        try:
            logger.debug(
                f"Converting text to speech with Coqui TTS: {text[:50]}...")
    ...
    def _ensure_cleanup_before_start(self):
        ...
        try:
            ...
            if self.is_streaming:
                logger.debug(
                    "Stopping existing streaming before starting new one")
                self.stop_playback()
    ...
    def _speak_text_streaming(self, text: str):
        ...
        try:
            ...
            import traceback
            logger.error(
                f"Streaming TTS error traceback: {traceback.format_exc()}")
    ...
    def _on_streaming_finished(self):
        ...
        try:
            logger.debug(
                "[VOICE] Streaming TTS playback finished - starting cleanup")
            ...
            if hasattr(self, 'streaming_player') and self.streaming_player:
                try:
                    logger.debug(
                        "[VOICE] Disconnecting streaming_player signals")
                    safe_disconnect(self.streaming_player.playback_finished)
                    logger.debug(
                        "[VOICE] Disconnected playback_finished signal")
                    ...
                    logger.debug(
                        "[VOICE] Disconnected audio_level_changed signal")
                    ...
                    if self.streaming_player.isRunning():
                        ...
                        # 500ms timeout
                        if not self.streaming_player.wait(500):
                            logger.warning(
                                "[VOICE] Streaming player cleanup timeout in finished callback")
                            self.streaming_player.terminate()
                            # Additional 200ms timeout
                            self.streaming_player.wait(200)
                except Exception as e:
                    logger.error(
                        f"[VOICE] Error cleaning up streaming player in finished callback: {e}")
            # Clean up streaming worker and thread
            if hasattr(self, 'streaming_worker') and self.streaming_worker:
                try:
                    logger.debug(
                        "[VOICE] Disconnecting streaming_worker signals")
                    safe_disconnect(self.streaming_worker.audio_chunk_ready)
                    logger.debug(
                        "[VOICE] Disconnected audio_chunk_ready signal")
                    safe_disconnect(self.streaming_worker.progress_updated)
                    logger.debug(
                        "[VOICE] Disconnected progress_updated signal")
                    safe_disconnect(self.streaming_worker.streaming_finished)
                    logger.debug(
                        "[VOICE] Disconnected streaming_finished signal")
                    safe_disconnect(self.streaming_worker.streaming_error)
                    logger.debug("[VOICE] Disconnected streaming_error signal")
                except Exception as e:
                    logger.error(
                        f"[VOICE] Error cleaning up streaming worker in finished callback: {e}")
            if hasattr(self, 'streaming_thread') and self.streaming_thread:
                try:
                    if self.streaming_thread.isRunning():
                        ...
                        # 1 second timeout
                        if not self.streaming_thread.wait(1000):
                            logger.warning(
                                "[VOICE] Streaming thread quit timeout, forcing termination")
                            self.streaming_thread.terminate()
                            # Additional timeout
                            if not self.streaming_thread.wait(500):
                                logger.error(
                                    "[VOICE] Streaming thread termination failed")
                        else:
                            logger.debug(
                                "[VOICE] Streaming thread stopped successfully")
                    self.streaming_thread.deleteLater()
                except Exception as e:
                    logger.error(
                        f"[VOICE] Error cleaning up streaming thread: {e}")
            # Reset streaming state
            self.is_streaming = False
            self.streaming_player = None
            self.streaming_worker = None
        ...
        if self.is_streaming and self.streaming_player:
            ...
            try:
                ...
                if self.streaming_player.isRunning():
                    self.streaming_player.quit()
                    # 1 second timeout
                    if not self.streaming_player.wait(1000):
                        logger.warning(
                            "Streaming player quit timeout, forcing termination")
                        self.streaming_player.terminate()
                        # Additional 500ms timeout
                        self.streaming_player.wait(500)
            except Exception as e:
            ...
            if hasattr(self, 'streaming_thread') and self.streaming_thread.isRunning():
                try:
                    ...
                    if not self.streaming_thread.wait(1000):
                        logger.warning(
                            "Streaming thread quit timeout, forcing termination")
                        self.streaming_thread.terminate()
                        # Additional 500ms timeout
                        if not self.streaming_thread.wait(500):
                            logger.error(
                                "Streaming thread termination failed in stop_playback")
                        else:
                            logger.debug(
                                "Streaming thread terminated successfully")
                    else:
        ...
        try:
            ...
            with self._model_loading_lock:
                if model_name == self.current_model and self.tts_model is not None:
                    logger.debug(
                        f"Model {model_name} already loaded, skipping")
                    return True
                ...
                if model_name in self._loaded_models:
                    ...
                    self.current_voice = cached_data.get(
                        'current_voice', self.available_voices[0] if self.available_voices else None)
                    logger.debug(
                        f"Model {model_name} loaded from cache with {len(self.available_voices)} voices")
                    return True
                ...
                if hasattr(self.tts_model, 'list_voices'):
                    self.available_voices = self.tts_model.list_voices()
                    logger.debug(
                        f"Voices for model {model_name}: {self.available_voices}")
                else:
                    self.available_voices = []
                    logger.debug(
                        f"Model {model_name} has no list_voices method.")
                ...
                if not self.available_voices and self.current_model:
                    ...
                    speakers_file = config.get(
                        'speakers_file') if config else None
                    if speakers_file and os.path.exists(speakers_file):
                        try:
                            with open(speakers_file, 'r', encoding='utf-8') as f:
                                ...
                                logger.debug(
                                    f"Loaded speakers from speakers_file: {self.available_voices}")
                        except Exception as e:
                ...
                if self.available_voices:
                    ...
                    self.current_voice = None
                    logger.debug(
                        f"No voices available, using single-speaker mode")
    ...
    def is_multi_speaker(self) -> bool:
        ...
        print(
            f"[DEBUG] Checking if model {self.current_model} is multi-speaker...")
        if hasattr(self, 'available_voices') and self.available_voices:
            print(f"[DEBUG] available_voices: {self.available_voices}")
            if len(self.available_voices) > 1:
                print(
                    f"[DEBUG] Model {self.current_model} is multi-speaker (voices > 1)")
                return True
            # If only one, check config for speakers/num_speakers
            if self.current_model:
                config = self.get_model_config(self.current_model)
                print(
                    f"[DEBUG] Model config for {self.current_model}: {config}")
                if config:
                    if 'speakers' in config and isinstance(config['speakers'], list) and len(config['speakers']) > 1:
                        print(
                            f"[DEBUG] Model {self.current_model} is multi-speaker (config speakers > 1)")
                        return True
                    if config.get('num_speakers', 1) > 1:
                        print(
                            f"[DEBUG] Model {self.current_model} is multi-speaker (config num_speakers > 1)")
                        return True
        ...
        if self.current_model:
            ...
            if config:
                if 'speakers' in config and isinstance(config['speakers'], list) and len(config['speakers']) > 1:
                    print(
                        f"[DEBUG] Model {self.current_model} is multi-speaker (config speakers > 1, fallback)")
                    return True
                if config.get('num_speakers', 1) > 1:
                    print(
                        f"[DEBUG] Model {self.current_model} is multi-speaker (config num_speakers > 1, fallback)")
                    return True
    ...
    def _generate_audio(self, text: str) -> Optional[str]:
        ...
        try:
            ...
            if self.available_voices and self.current_voice:
                ...
                logger.debug(
                    f"Generating audio file with voice: {self.current_voice}")
    ...
    def _play_audio(self, audio_file: str, text: str):
        ...
        try:
            ...
            logger.debug(
                f"Started Coqui TTS playback, estimated duration: {estimated_duration:.1f}s")
