...
class SettingsDialog(QDialog):
    ...
    def create_general_tab(self):
        ...
        personality_layout.addRow(
            "Default Personality:", self.personality_combo)
    ...
    def create_chat_tab(self):
        ...
        self.spellcheck_checkbox.setToolTip(
            "Enable spell checking in chat input")
        ...
        self.enhancement_checkbox.setToolTip(
            "Automatically enhance AI responses with additional details")
        ...
        self.memory_checkbox.setToolTip(
            "Enable or disable LLM memory management features")
        ...
        self.typewriter_checkbox.setToolTip(
            "Show AI responses with a typewriter animation")
    ...
    def create_session_tab(self):
        ...
        self.history_checkbox.setToolTip(
            "Enable conversation history (/set history)")
        ...
        self.wordwrap_checkbox.setToolTip(
            "Enable word wrapping (/set wordwrap)")
        ...
        self.json_format_checkbox.setToolTip(
            "Force responses in JSON format (/set format json)")
        ...
        self.verbose_checkbox.setToolTip(
            "Show detailed LLM statistics (/set verbose)")
        ...
        # Add description
        description = QLabel(
            "These settings control Ollama session behavior using /set commands. Changes take effect immediately for new conversations.")
    ...
    def load_current_settings(self):
        ...
        if not self.ui_ready:
            logger.debug(
                f"[ID:0139] SETTINGS: UI not ready yet, skipping settings load", print_to_terminal=False)
            return
        ...
        try:
            ...
            self.memory_checkbox.setChecked(
                self.config_manager.get("memory_enabled", True))
            self.typewriter_checkbox.setChecked(
                self.config_manager.get("typewriter_enabled", True))
            ...
            try:
                ...
                logger.debug(
                    f"[ID:0138] SETTINGS: UI elements deleted during load: {e}", print_to_terminal=True)
                return

            # Load chat parameters
            chat_settings = self.config_manager.get_chat_settings()
            self.max_tokens_spin.setValue(
                chat_settings.get("max_tokens", 2048))
            self.top_p_spin.setValue(
                int(chat_settings.get("top_p", 0.9) * 100))

            # Load developer settings
            self.logging_checkbox.setChecked(
                self.config_manager.get("logging_enabled", True))

        except Exception as e:
            logger.debug(
                f"[ID:0137] SETTINGS: Error loading settings: {e}", print_to_terminal=True)
            QMessageBox.warning(
                self, "Warning", f"Some settings could not be loaded: {str(e)}")
    ...
    def save_settings(self):
        ...
        try:
            ...
            self.config_manager.set(
                "typewriter_enabled", self.typewriter_checkbox.isChecked())
